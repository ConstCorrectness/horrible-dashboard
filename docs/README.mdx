...
flowchart TB
    subgraph FE["Frontend — one React app, two layouts"]
        ...
        Mods["Modules<br/>agent · dashboard · observability ·<br/>marketplace · settings · clubhouse · scratch"]
    ...
    subgraph BE["backend/ — FastAPI (the brain)"]
        ...
        Other["clubhouse · settings · plugins"]
    end
...
backend/) exists and the **dashboard**, **agent**, **observability**,
**clubhouse**, **marketplace**, and **settings** modules are implemented. The
remaining module pages (editor, terminal, file-explorer) still document design
only — update each to match reality when it gets implemented. An experimental
Electron shell lives on the `electron-shell` branch
