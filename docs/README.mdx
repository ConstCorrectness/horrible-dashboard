...
- [Interpretability](modules/interpretability.mdx) — **Implemented.** The exact context
  ...
  and the tool-budget truncation that was previously silent. Ships its own
  "Interpretability" workspace (inspector centre, agent chat docked right).
...
- [Records](modules/records.mdx) — **Implemented.** User-defined tables (grid, form,
  board) stored as real `rec_*` tables in the app database, with an agent write path
  that **proposes** field values with provenance for per-field review instead of
  committing. Backs the CRM and Data Entry workspaces.
...
- [Search](modules/search.mdx) — **Implemented.** Pluggable web-search engines
  ...
  queried as just another engine, so local hits fuse with live web hits.
