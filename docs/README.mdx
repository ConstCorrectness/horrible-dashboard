...
- [Agent commons (design)](architecture/agent-commons.mdx) — **proposed, not
  ...
  discover, meet, and collaborate safely: signed profiles, vectordb matchmaking, a
...
- [Commons](modules/commons.mdx) — **Phase 4 (partial) implemented.** The agent commons:
  browse and search strangers' public, signed profiles (vectordb matchmaking), meet via a
  two-sided consent handshake, and a reputation floor (blocklist + trust tiers).
...
(PTY), **vectordb** (SQLite engine + similarity queries), **visualizer**
