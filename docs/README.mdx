...
- [Layout shell](architecture/layout-shell.mdx) — the shared frontend layout
  ...
  versions differ.
- [Data directories](architecture/data-directories.mdx) — where a node keeps its
  files, resolved once in `backend/paths.py` per launcher and per OS. Read before
  adding storage: the old inline `.data` default was relative to the *cwd*, so it
  silently meant a different directory depending on who started the backend.
- [HorribleAssault's two clients](architecture/hassault-two-clients.mdx) — the
  browser pane and the native Rust client render the same game from the same
  server: what is duplicated, how each duplication is pinned, and the checklist
  for adding a feature to one without silently leaving the other behind
- [Releases and remote updates](architecture/releases.mdx) — bundling, the signed
  update manifest, stable/beta channels, and why an update never touches
  `$HORRIBLE_DATA_DIR`. Read before generating the signing key: losing it means no
  existing install can ever be updated again.
- [Theming](architecture/theming.mdx) — every colour, radius and elevation as a CSS
  token, and the `data-theme` switch that selects a block of them. Read before
  adding a colour to a component: a hardcoded literal is invisible to the switcher
  and survives into a theme it was never designed for.
- [Windowing](architecture/windowing.mdx) — the frame: how views (panels and
  widgets) open as tabbed/split panes and windows, views vs pane instances, the
  `FrameState` model, and layout persistence.
- [Desktop shell](architecture/desktop-shell.mdx) — the OS-style desktop on top of
  that frame: the single shell view, windows as an interaction model, edge snapping,
  tab merging, the pluggable backdrops and wallpaper routes, the taskbar and start
  menu, the boot splash and first-run flow, the spotlight, and the per-desktop
  tiling/floating mode switch.
...
- [Share relay](architecture/share-relay.mdx) — the public half of screen sharing:
  WHIP ingest, SFU fan-out, WHEP playback, link tokens, and why the ingest URL
...
- [OpenTelemetry (OTLP)](modules/otel.mdx) — **Implemented.** An OTLP/HTTP collector
  for agents you write (received traces become trajectory runs), GenAI-semconv spans
  for the built-in agents, and optional export to Phoenix/Langfuse/Jaeger/Tempo.
...
- [Agentpedia](modules/agentpedia.mdx) — **Implemented (stepper + harness pages).**
  One agent turn, steppable: what the model was shown, what actually went over the
  wire, what the agent did, and what it cost, scrubbed round by round. Owns no store —
  it joins `agent_turns`, the telemetry ring and `traj_steps`, which became possible
  once every request carried a `turn_id`. Forks are not built yet.
...
- [llama.cpp](modules/llamacpp.mdx) — **Implemented.** The node serving its own
  weights: an upstream `llama-server` build fetched and hash-checked on demand, a GGUF
  catalog spanning our managed directory and the files Ollama and LM Studio already
  have, and a supervised process the agent talks to over the OpenAI dialect. Adds
  per-agent provider selection, so one agent can run here while the rest stay on Ollama.
- [Hardware](modules/hardware.mdx) — **Implemented.** One capability probe deciding
  the llama.cpp build, GPU offload, thread count and trace cap, with three states
  rather than two: found it, looked and found nothing, and *could not ask*. No pane —
  a settings section and three overrides.
...
- [Doc viewer](modules/docviewer.mdx) — **Implemented.** Capture a whole
  documentation site and read it offline at full fidelity — real CSS *and* real
  JavaScript, in an opaque-origin sandbox — with a page tree and semantic search over
  the set.
...
- [Audio](modules/audio.mdx) — **Implemented.** Default input, default output and
  a VoiceMeeter-shaped routing matrix over both: sources are strips, outputs are
  buses, and a strip can feed several buses at once — which is what makes "play
  this into my microphone *and* my headphones" expressible. Per-platform virtual
  device support (PipeWire creates them, Voicemeeter is driven, BlackHole is
  detected), and `audio.*` agent tools.
...
- [People](modules/people.mdx) — **Implemented.** One pane for everyone you know,
  replacing nine across three modules. Introduces `@username` as a person's single
...
- New panel, command, capability, or backend route on an existing module → update
  that module's page.
- Changes to the workspace/docking system, spotlight, keybindings, platform
  capability service, or either app entry → update
  `docs/architecture/layout-shell.mdx`.
- Changes to the desktop surface — backdrops, the taskbar, boot/first-run, windows,
  snapping — → update `docs/architecture/desktop-shell.mdx`.
