...
For a media element there is a hook, `useMediaStrip(element, decl)`, which handles
the once-per-element rule below. Pass the element held in state through a callback
ref, not a `RefObject`: a ref object never changes, so an effect keyed on it does
not re-run when a `<video>` mounts after the first render, and that element is
never attached.

**Until a bus exists, a strip plays straight to the default output.** Buses come
from the saved routing, which loads at launch (`audio.startOnBoot`, on by default)
or when the mixer is first opened. A strip connected before that, or with the
backend down, used to reach a fader wired to nothing. `createMediaElementSource`
had already taken the element's audio, so a karaoke song played in total silence.
The engine now wires such a fader to `ctx.destination` and unwires it the moment
`apply` builds a bus.
...
`openMicrophone()` wraps that: it asks for the chosen input, retries the system
default if that fails (unless the failure was a refusal), and otherwise throws a
message from `describeMicFailure` that names the error **and lists the inputs the
window can see**. Chromium's "Requested device not found" is the same text for
"no microphone", "the OS default input is a headset that is switched off" and
"this webview enumerates nothing"; only the device list tells them apart.
Clubhouse voice uses it.

### The desktop app runs WebView2's audio service unsandboxed

On Windows the desktop shell passes `AudioServiceSandbox` in WebView2's
`--disable-features` (`media::WEBVIEW2_ARGS` in `apps/desktop/src-tauri`, and
`additionalBrowserArgs` for `main` in `tauri.conf.json`). On a machine where the
sandboxed audio service crash-loops inside WebView2, and only there (Edge on the
same build was fine), `enumerateDevices()` returned **no audio devices at all**:
microphone permission was granted, the profile was fine, and every capture failed
with "Requested device not found". With the sandbox off, all devices came back.
The renderer stays sandboxed; this is what the enterprise
`AudioSandboxEnabled=false` policy does.

Two constraints on that string: setting arguments **replaces** wry's defaults, so
it restates them (including `--autoplay-policy=no-user-gesture-required`, or
mixer playback waits for a click); and every webview sharing the profile must use
**identical** arguments or WebView2 refuses to create it, which is why a test
pins the conf against the constant.
...
2. **`ctx.close()` is not cleanup.** The context is shared now. Closing it when a
   ...
   It is worse than "until the next join", because `getContext()` caches the one
   context for the life of the page and never rebuilds a closed one: every later
   `createGain()` / `createMediaStreamDestination()` throws `InvalidStateError`,
   and the resume guard does not catch it either — a closed context is `closed`,
   not `suspended`. hassault was fixed and Clubhouse's room teardown was missed,
   so leaving one Clubhouse room left the microphone dead in every room after it
   while the button still read *Mic Active*. The rule is now enforced across the
   tree by `modules/audio/__tests__/shared-context-is-never-closed.test.ts`,
   which reads the source the way `control-padding.test.ts` does — a closed
   context is invisible at runtime, so there is nothing else to assert against.
...
A shared session's guests are an output like any other. The
[share module](share.mdx) adds a bus whose `deviceId` is the sentinel
`horrible:viewers` and whose output is a `MediaStreamDestination` rather than a
device, so "what the viewers hear" is an ordinary column in the matrix — karaoke
to the viewers but not to your headphones, or your microphone to both, are one
checkbox each.

Two things about it are deliberate:

- **Nothing is routed to it when it appears.** The same rule `addBus` follows, for
  a stronger reason: this output is another person's ears, and defaulting it to
  carry whatever is playing would make starting a screen share silently broadcast
  the host's music and notifications.
- **It has no `<audio>` element.** Every other bus plays locally; this one must
  not, or the host hears every sound twice and their own microphone echoes back.

- **The sentinel never goes through `resolveDeviceId`.** `horrible:viewers` is not
  a device id, so looking it up in the OS device list fails and returns `''` —
  "system default". A bus resolved that way connects to `ctx.destination` and
  builds no `MediaStreamDestination` at all, which breaks the two rules above at
  once: guests get silence, and the host hears the viewer mix out of their own
  speakers. `resolveTarget` short-circuits the sentinel for exactly this reason.

`mixer.busStream(id)` is how the share module reads it. Going through the mixer
rather than opening its own context is this module's central rule: a private
`AudioContext` is a module the mixer cannot see, and nothing on screen would
say so.

Note that a bus **always carries exactly one audio track** once its sink exists,
silent or not, so track-counting cannot answer "is anything being sent". That
question is answered from the routing document by `busHasSource`.
