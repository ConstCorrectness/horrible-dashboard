...
- **Panels** — `mcp.servers` ("MCP Servers", 🔌, `role: 'document'` + `dockable: 'left'` — five sections including an authoring surface do not fit a 280px rail, and `dockable` keeps the rail glyph for consulting one beside your work,
  singleton). Configuration you consult while working, not a document you work in.
  Three **sections**: `servers` (<kbd>s</kbd>, default), `discover` (<kbd>d</kbd>) and
  `author` (<kbd>a</kbd>).
...
One pane with three sections rather than three panes: finding a server, writing one and
running one are the same object at three moments. Discover hands its result straight to
