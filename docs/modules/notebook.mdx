...
Kernels are process-global, and dockview unmounts inactive panes — so a pane can reopen a
...
- **Panel group** — `notebook.workbench` (primary `notebook.editor`, companion browser).
...
- **Phase 3** *(done, minus the reactive bridge)* — custom `anywidget` (`_esm`/`_css`)
  ...
  seed), and agent tools (`nb.list_cells`, `notebook.read_cell`/`insert_cell`/`edit_cell`/
