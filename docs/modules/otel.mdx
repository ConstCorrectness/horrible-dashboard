...
It is **HTTP only**. There is no gRPC on :4317.
...
| `GET /api/otel/ingest` | endpoint, received-trace count, last-received time; the token for loopback only |
...
- **gRPC (`:4317`)**: only OTLP/HTTP is received.
- **`traceparent` on outbound HTTP**: this was planned as opt-in for LAN targets, and
  is not implemented.
- **Games agents**: `agent_code` runs in-process and is not wrapped in spans yet. Only
  the `chat` calls it makes through `providers` are traced.
