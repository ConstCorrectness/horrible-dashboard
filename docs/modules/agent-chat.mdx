...
(open panes), `/clear` (new session), `/llm` (query, set, or reset orchestrator LLM
...
| client→server | `ask`               | `{turnId, prompt, history?, context?}`        |
...
`registry.layoutController` seam (installed by `Workspace.tsx`), then replies. This
...
`maximize_pane`/`restore_pane(instanceId)`. `split_pane`'s `direction` takes the
model-friendly orientations **`vertical`** (panes side by side) and
**`horizontal`** (panes stacked) as well as the four concrete sides
(`left`/`right`/`above`/`below`); the orientations are resolved to a side
(`vertical`→`right`, `horizontal`→`below`) in `tool-exec.ts`, so the
`LayoutController` and the UI corner-grip stay four-way.
...
### Local Reasoning Models & Tool Count Thresholds

Local reasoning models (e.g., `gemma4:e2b` under Ollama) exhibit a strict tool count capacity limit when handling streamed turns:
* **Threshold Limit:** If a request payload lists **40 or more tool definitions**, the model silently skips its thinking phase entirely (bypassing the `<think>` tag / `thinking` field) and immediately streams the final content starting with its first token. If the list contains **39 or fewer tools**, reasoning streams normally.
* **Dynamic Pruning & Prioritization:** To prevent exceeding this limit while supporting the full catalog of layout and dynamic tools (46+ tools in total), the backend employs a dynamic selection algorithm in `_tools_for()` to cap the tool count at exactly **38 tools**:
  * **Always Kept:** All 15 static layout tools (`LAYOUT_TOOLS`) are preserved.
  * **High Priority Core Tools:** Core workspace integrations (`files.*`, `editor.*`, `terminal.*`) are kept by default.
  * **Contextual Promotion:** Optional plugin/widget tool groups (`vectordb.`, `visualizer.`, `clubhouse.`, `stub.`) are dynamically searched against the user's prompt and chat history keywords. If intent is detected (e.g., matching `"visualizer"`, `"vector"`, or `"room"`), the corresponding tools are prioritized.
  * **Pruning:** Low-priority/inactive optional tools are pruned to satisfy the 38-tool ceiling, ensuring reasoning remains active.
