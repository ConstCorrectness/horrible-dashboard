...
reads the active session the **first** time it talks to an agent, lazily creates a
session titled from the first prompt, and **auto-saves each completed turn**. The session bar (a `<select>` + new/delete) switches between them,
...
### The transcript lives outside the component

`packages/core/src/modules/agent/chat-state.ts` holds the transcript, the in-flight
turn, the draft and the active session id in a small module-level store keyed by
agent id; `ChatWidget` reads it through `useSyncExternalStore` and owns none of it.

That is not tidiness, it is the fix for "the chat resets when I prompt". The shell
unmounts this pane routinely — a workspace switch tears down every pane, an inactive
tab is unmounted rather than hidden ([Unmount is not close](../architecture/desktop-shell.mdx)),
and the agent's **own layout tools** (`open_pane`, `split_area`) restructure the tree
it is rendered in. With the conversation in `useState`, each of those blanked it: the
pane came back showing the starter prompts, re-read the node, and got the last
_saved_ state — which for a turn still streaming is the conversation as it was before
the question. So the most reliable way to lose a conversation was to ask the agent to
open something, which is the one thing this agent is for.

Two consequences follow, and both are the point. A remount is **invisible** — same
transcript, same draft, no refetch and no flash of the empty state. And a turn
**survives its pane**: `askAgent`'s streaming callbacks write to the store, not into
a dead component's `setState`, so tokens that arrived while the pane was unmounted are
there when it comes back rather than silently dropped.

The mount effect therefore loads only when that agent has not been read yet
(`loaded`), and `loadSessions` will not overwrite a transcript it already holds for
the active session — a reload landing mid-stream would otherwise truncate the answer
back to the previous turn. State is per app session and deliberately not persisted:
the node is still the durable store, this is only what makes the *pane* stateless.

**A failed read is never rendered as an empty transcript.** `loadSessions` records
`loading` / `ok` / `failed` rather than catching into a blank pane, because empty is
indistinguishable from deleted — and it was worse than cosmetic: with no `activeId`,
the next message `ensureSession` sent created a _second_ session, orphaning the real
history rather than merely hiding it. So `ensureSession` re-reads before creating
anything when the last read failed, the restore retries once on its own (boot races
the backend), and the pane shows what happened with a Retry instead of a chat that
looks wiped.

A **model picker** (`ModelPicker`) sits in the same bar, listing every reachable
provider's models — so a hosted provider appears there the moment a key is saved, and
switching models does not mean leaving the conversation for the settings page.

It is a **searchable combobox, not a `<select>`**. One OpenRouter key puts several
hundred models in this list (465 on a typical node), and a native select can only be
walked: its type-ahead matches a *prefix*, so the queries people actually have — every
`:free` model, everything with `qwen` in it — are unreachable by typing. The filter is
a substring match over the provider label and the model id **together**, word by word
and ANDed, so `free` narrows to the free tier, `lm studio` to one provider, and
`free qwen` to the intersection. Arrow keys and Enter pick; the panel is
`position: fixed` and measured off the trigger, because the session bar sits inside a
pane that clips its overflow. It writes the _provider and the model together_: they are two settings
(`agent.<id>.provider`, `agent.<id>.model`), and the settings page edits them as two
controls, but here one option carries both, because a model name means nothing on a
server that does not have it — `minimax/minimax-m3:free` selected while the provider
still says Ollama is not half-configured, it is broken, and it fails mid-turn rather
than at the click. The blank choice clears both back to the configured model. A saved
override the live list does not contain (a provider that went down, an id typed on the
settings page) stays selectable, so the control never silently renders as something
...
3. **Turn setup (backend).** `handle_agent_message` routes the `ask` to a **detached**
   ...
   awaits `tool_result`s that arrive on the _same_ receive loop; awaiting inline would
...
4. **The loop (≤ `agent.maxRounds`).** Each round streams the provider via
...
### The step budget, and how a turn that runs out ends

A turn may spend `agent.maxRounds` rounds of tool calls (`MAX_ROUNDS`, 24, when the
setting is 0). It is a **guard against a model that never stops calling tools**, not
a work limit — but at its old value of 8 it was acting as one, invisibly. In the
recorded turns the counts tapered off to a single turn at 7, then **32 sat at exactly
8**: the shape of a budget being hit rather than of work finishing.

Two things make that shape a bug rather than a threshold doing its job:

- **Discovery used to be charged as work.** Under progressive disclosure the model
  pays a round for each `load_tools` call before it can touch the task — one observed
  turn spent rounds 3 and 5 loading groups — so a third of a short budget could go on
  reading the menu. A round whose calls were *all* meta tools is now free, up to
  `META_ROUND_ALLOWANCE` (6). Bounded rather than free, because a model that loops
  loading groups for ever must still stop.
- **Running out threw the work away.** The loop returned a literal
  `"(stopped after too many steps)"`, so a turn that had read six files and was ready
  to report ended with a sentence about the machinery. Exhaustion now spends one
  last **tool-less** round (`_final_answer`): the model is told the budget is gone and
  asked to answer with what it has. Offering no tools is what makes it terminate. If
  that round returns nothing, or fails, the turn still says plainly that it stopped
  early and how many steps it used — an honest stop beats an invented answer, and
  both beat silence.

Passing no tools is a real request, so `providers` now **omits `tools` entirely when
the list is empty** rather than sending `[]`, which several OpenAI-compatible servers
reject outright.
...
6. **Completion.** When a round returns no tool calls, the backend sends the
   ...
   _narrated_ an action without emitting a call, the loop first grants **one** nudged
...
one glued string would make that impossible. The forced-tool nudge adds a _fourth_,
mid-conversation.
...
delivered at all. Because the recorder captures in the orchestrator, _before_ the
...
continuing; otherwise it sends the final (authoritative) `answer`. Capped by
[the step budget](#the-step-budget-and-how-a-turn-that-runs-out-ends). Ollama gets `think:true` best-effort — models without a thinking mode
...
higher temperatures small local models _narrate_ an action ("I'll call …") instead
...
the browser — returning a message the model can actually act on. An _empty_ payload
...
the selected provider's live models** from `/agent/status`, not a static enum); the blank
...
| client→server | `cancel`            | `{turnId}` — Stop; only the asking socket may    |
...
| `trainer`    | training, recipe, cells, evals, localtrack, datasets, llamacpp, wandb, editor, files | training                 | session mode  |
...
and shows the _other_ half of that trap. `hardware` is pointedly **not** in its scope,
even though the workspace reads a hardware probe: a group is a tool name's prefix
(`_group_of`), that namespace holds settings keys rather than tools, so naming it
would permit nothing at all and say nothing about it. Check that a group you list has
tools under it, not just a module with that name.

Its scope is long because the loop is long, and because `training` was
[split into four](training.mdx#resolved-training-was-split). That group had reached 34
tools — a third of them registered by the *frontend*, which is why every count in a
comment said 14 — and with a ~9-tool core there was no room for `evals` beside it: the
loop this agent's own prompt prescribes did not fit under `TOOL_BUDGET`, and the
overflow was dropped with nothing but a log line to show for it. Only `training` is
preloaded; the rest cost a `load_tools` round rather than schema space on every turn.

`datasets` was missing entirely, which is the same dead-button trap one level in: the
agent that fills in a recipe's `dataset_id` could not search, peek at or token-stat a
dataset, and `can_delegate` is False, so it had no way to ask anything that could.
...
- **Per-agent provider** (`roster.resolve_provider`). `provider`/`endpoint` used to be
  ...
  unexpressible: a model _name_ means nothing on a server that does not have that
...
calls `get_pane_context` — pull, not push). As a convenience the frontend _also_
...
rather than cosmetic**: the normal message tells the model to write the _complete_
...
`context.panes` — a snapshot of every _visible_ pane that exposes one
...
`agent.workspaceContextBudget` (1500 chars per pane, clipping oversized _fields_
...
_beside_ another one — without it, `move_pane` could only tab into an existing area
...
delete it" arrived with nothing to delete it _with_. `_carried` / `_remember_groups`
...
A provider is either a **local server** reached over HTTP on this machine, or a
**hosted API** reached over the internet with a key. `backend/modules/agent/providers.py`
is the source of truth; it collapses every one of them onto one of three **dialects**:

| Provider      | Dialect   | Default endpoint         | Pull | Spawn | Key |
| ------------- | --------- | ------------------------ | ---- | ----- | --- |
...
| OpenAI        | `litellm` | —                        | no   | no    | yes |
| Anthropic     | `litellm` | —                        | no   | no    | yes |
| Google Gemini | `litellm` | —                        | no   | no    | yes |
| OpenRouter    | `litellm` | —                        | no   | no    | yes |
...
provider whose model _path_ is known rather than inferred. See
...
The dialect decides the wire format and the request paths — it is **not** the
online/offline switch, `hosted` is. "OpenAI-compatible" refers to LM Studio's,
llama.cpp's and vLLM's wire format; it does not mean any request leaves the machine.
Everything on the `ollama` and `openai` dialects runs offline against a localhost
server; only the `litellm` dialect calls out.

### Hosted providers and their API keys

Four things make a hosted provider work, and each exists because the obvious
alternative fails silently:

- **The key is never a setting.** `GET /api/settings` hands the whole settings bag
  to the browser and to every plugin, so a credential there is a credential given
  away. Keys are Fernet-encrypted in `secrets.db` under the provider kind (the same
  store [connectors](./connectors.mdx) use) and written through
  `PUT /agent/providers/<kind>/key`, which returns only `has_api_key`. Nothing reads
  a key back out, which is why the settings field is write-only: it shows a key as
  _saved_, never as its value.
- **A key already in the environment counts.** litellm reads `OPENAI_API_KEY`,
  `OPENROUTER_API_KEY` and friends on its own, so `api_key_for` falls back to the
  variable. Without that check a key that _works_ would be reported as missing, and
  the user would be sent to fix something that isn't broken. `DELETE` says so too:
  it can return `has_api_key: true`, because an exported variable is not ours to
  delete.
- **"Reachable" means "we hold a key".** `list_models` raises `MissingApiKey` (an
  `httpx.HTTPError`, which is what the probe already catches) for a keyless hosted
  provider. Before this, a keyless OpenAI reported itself reachable with a list of
  models, so onboarding cheerfully offered a provider that failed on the first turn.
- **The model id needs a routing prefix.** `qualify_model` adds `ProviderInfo.model_prefix`
  — `openrouter/` for OpenRouter, `gemini/` for Google AI Studio, nothing for OpenAI
  and Anthropic, whose ids are unambiguous. A bare `minimax/minimax-m3:free` is not a
  model litellm can place, and a bare `gemini-2.5-pro` routes to the wrong vendor.
  Applied at the call, so the id stored in settings stays the one the provider's own
  dashboard shows.

**OpenRouter's model list is fetched live** from its public catalog
(`https://openrouter.ai/api/v1/models`, no key needed to read) rather than pinned in
code: its range — and which entries are `:free` — changes weekly, so a hardcoded list
would be wrong within the month. Ids come back **tool-capable first**, because a model
without tool support does not merely do worse in the orchestrator loop, it never calls
a tool at all — which reads as the agent ignoring you rather than as a model that
cannot do this. The fetch is cached for 10 minutes and carries its own 10s timeout
(the probe client's 2s budget is sized for a loopback port, not for a few hundred KB
from across the internet), and a failed fetch falls back to `static_models` rather
than raising: the key decides usability, and an empty dropdown for a working provider
is the worse failure. The other three hosted providers have no public catalog, so
they list `static_models` — a starting point for the dropdown, never a claim to be
the provider's full range. Every model field in the UI stays free text.

**Free models are listed without a key.** The catalog is public, and onboarding is the
moment someone has no key yet and wants to know what they could run for nothing, so
`free_models(client, info)` never checks for one and `/agent/status` reports
`free_models` on every provider (empty for providers without a catalog). It says
nothing about usability — a free OpenRouter model still needs a key to call, and
`reachable` still means "we hold one". An entry is free when both `pricing.prompt` and
`pricing.completion` parse to exactly zero, or its id ends in `:free`; the auto-router
is priced `-1` and is not. The free list shares the catalog cache with the model list,
so a keyed provider costs no second fetch. Onboarding picks a free model by default
when switching to a provider, lists free ids first and tags them.

**Every model field is a substring-searchable combobox.** The onboarding field was a
`<datalist>` and the settings page's model override a `<select>`, and both effectively
match from the start of the id — so with several hundred OpenRouter ids, typing `free`
found nothing. Both are now `ModelCombobox`, filtering through `matchesModelQuery`, the
same match the Agent pane's `ModelPicker` uses: every word anywhere in the id. A free
model also matches the word `free`, which finds zero-priced ids without the suffix.
The field still accepts an id that is not listed (Enter uses what was typed), and it
commits on pick, Enter or blur rather than per keystroke, because on the settings page
each commit is a settings write and a half-typed id is a broken agent until the typing
stops.

**A remote listing is cached and warmed; a local one never is.** NVIDIA NIM's
`/v1/models` (and any `vllm`/`peer` pointed at another host) is fetched across the
internet, so `list_models` gives it its own `REMOTE_TIMEOUT` (10s) rather than the
probe client's 2s budget — that budget is sized for a loopback port, and `_probe`
swallows `httpx.HTTPError`, so inheriting it made a slow network indistinguishable
from a provider with no models. The answer is cached under **(kind, endpoint)** for
the same 10 minutes as the OpenRouter catalog, keyed by the endpoint too because a
vLLM can be repointed and a list cached under the bare kind would then describe a
machine nobody is talking to. A **local** server's list is deliberately never cached:
`ollama pull` changes it, and a stale list is a worse lie than a slow one — loopback
is decided by `is_loopback_endpoint`, for which an empty endpoint (a `peer` with no
lease) counts as local because it is unreachable either way.

Every provider is probed in parallel, so the slowest one sets the response time, and
the probe client's **connect** timeout is 0.75s (the overall budget stays 2s). That is
the number that matters on Windows, where a connect to a *closed* loopback port does
not fail fast — the OS retries the refused SYN for about two seconds — so each
provider that simply is not running (Ollama, llama.cpp, vLLM on most machines) used
to hold the whole response for the full budget. A live local server accepts in about
a millisecond, or ~250ms through `localhost` when the IPv6 attempt is refused first.
Remote listings are unaffected: they pass their own explicit `timeout=`. On the
frontend, `getAgentStatus()` shares one in-flight request between callers (several
components ask within a few ms of each other at boot); nothing is cached past it.

Those remote listings are **warmed at startup**: the app lifespan calls
`agent.routes.start_warm_model_lists()`, which probes only the remote providers in
the background so the first model dropdown opens filled instead of waiting on a round
trip. It is never awaited and every failure is swallowed — a warm is not worth a
second of boot, and an unwarmed provider is simply fetched on demand as before.
`GET /agent/status?refresh=true` drops both caches first, and the **refresh button**
in the model picker's panel — beside the filter field — is its only caller. It is a
button rather than a fetch-on-open because a refresh on open would put that download
in front of every use of the dropdown to serve the one time in a hundred the list
actually changed, and the TTL is otherwise invisible from the UI: enabling a model on
the vendor's dashboard and not finding it here reads as the model not existing. While
it is in flight the button spins and the count line says so; a failure says
`Refresh failed — showing the last list` rather than being swallowed, because a
refresh that silently did nothing is indistinguishable from a vendor with no new
models. A refreshed status **outranks** the one the pane handed the picker (`own ??
status`), until the pane itself refetches — a new object from the pane is in turn
newer than our copy, so the effect drops ours and defers to it again.

**A strict provider's tool names are translated at the wire, not renamed at the
source.** NVIDIA NIM validates function names against OpenAI's documented grammar
(`a-zA-Z0-9_-`) and rejects the *whole request* if one fails — `400 Validation:
Function at index 7 has an invalid name: "agent.ask_peer"`, returned in 281ms before
any inference, so every turn failed identically. Every tool in this codebase is
namespaced with a dot, and the orchestrator groups tools by that prefix (a connector's
`id` must equal its tool prefix), so the names cannot be changed at the source.
`sanitize_tool_names` rewrites them for `ProviderInfo.strict_tool_names` providers and
`restore_tool_names` undoes it on the way back. Three things it must do or it
half-works: rewrite the **history** too (an assistant turn already in the transcript
carries `tool_calls[].function.name`, so round two fails where round one passed);
restore the name on what the model called (otherwise the orchestrator is handed
`library_search`, matches no registered tool, and reports the model hallucinating a
tool it was in fact given); and **disambiguate collisions rather than merge them** —
`library.search` and `library_search` both sanitize to the same string, and merging
would route one tool's call to the other, a wrong action taken confidently.

**A provider's error body survives to the message.** `raise_for_status` produces the
status line and a link to MDN, so a provider that said exactly what was wrong is
reported as an unexplained 400 — diagnosing the NIM failure above meant digging the
response body out of the telemetry ring, which is not something a user can do.
`raise_for_status_with_body` puts the (truncated — some providers echo the whole
request back) body in the exception message instead.

`orjson` is a hard dependency because litellm's OpenRouter transformation imports it;
without it every OpenRouter call fails as an `APIConnectionError` naming a missing
module, which reads as a network fault.

**A hosted model that never answers ends the turn.** litellm's default request timeout
is `litellm.request_timeout` = **6000 s**, and nothing passed a smaller one, so a free
OpenRouter model sitting in its queue left the chat on its `…` bubble for up to 100
minutes with the input disabled. Every litellm call now carries `timeout`
(`HOSTED_FIRST_CHUNK_S`, 120 s), and the streamed paths run through `_watch_stream`, an
explicit watchdog: no first chunk within 120 s (connect included) or a 90 s gap
(`HOSTED_IDLE_S`) once streaming raises `StreamStalled` — a `TimeoutError`, so the
turn's catch-all reports it as `error` + `done`. The transport's read timeout alone is
not enough: OpenRouter keeps a queued request's connection warm with SSE comment lines,
which reset a read deadline without ever yielding a chunk.

**The chat can stop a turn.** While a turn is in flight the send button is a **Stop**
button: it aborts `askAgent`'s `signal`, which sends `cancel` and resolves at once; the
backend cancels the task (`_turns_by_id`, owner-checked — a turn id is not a secret on
a shared socket hub) and still emits `done` for any other listener. The abort handle
lives at module level beside the transcript, so a remounted pane can stop a turn it
did not start.

Keys are entered in two places, both writing the same route: the **Model provider API
keys** section of the Settings page (`ApiKeysSettings`), and inline in onboarding —
where an unreachable hosted provider offers a key field in the same slot an
unreachable local one offers "install and start it". A provider with a key then
appears in the per-agent **provider override** dropdown like any local one, and the
**model override** dropdown lists that provider's models (not the configured
provider's — an agent pointed at OpenRouter used to be offered Ollama's model names,
every one of which means nothing on that server, and the failure only showed up
mid-turn).
...
and most agents never rearrange anything. The five _read_ verbs
...
`show` is how it puts something there. The sixteen _arrangement_ verbs are loaded on
demand via `load_tools("layout")`.

**Including for `main`.** They used to stay in `main`'s core on the reasoning that
driving the shell is its job — but that made the _loosely prompted_ agent the most
...
The cap is **resolved per agent**, not global: `tool_budget_for(agent_id)` returns 38
for a model running on this machine and `TOOL_BUDGET_HOSTED = 96` for one reached
through a hosted API. The 38 ceiling is a measured property of *small local models*,
not of tool calling, and applying it to a frontier model bought nothing while costing
a `load_tools` round per group past the third.

The signal is `ProviderInfo.hosted` — an API reached with a key, versus a server on
this machine — and deliberately not the model name: parsing a parameter count works
for `llama-3.2-3b-instruct` and not at all for `gpt-5`, so a name-based rule would
have left every hosted model on the floor, which is the case it exists for. **Anything
unidentifiable resolves to the floor.** Being wrong low costs a round trip; being wrong
high costs a model that silently stops reasoning.

:::caution Watch the core, not the cap
The cap was 44 — deliberately _above_ the ≤39 ceiling, so that a legitimate
