...
provider's models as `<optgroup>`s — so a hosted provider appears there the moment a
key is saved, and switching models does not mean leaving the conversation for the
settings page. It writes the _provider and the model together_: they are two settings
