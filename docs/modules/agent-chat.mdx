...
4. **The loop (≤ `MAX_ROUNDS`).** Each round streams the provider via
   ...
   answer `content` token-deltas relay live as `reasoning`/`token` events the widget
   renders as they arrive. A round ends with either `tool_calls` or a final answer.
