...
store (`agent.orchestrator.temperature`). When the model still answers in prose that
...
a stronger one. Both knobs live in the **Agent orchestrator** section of the Settings
...
"Configured agent model" choice clears the override. The ghost-text `/agent/complete`
path likewise samples at a low fixed temperature so completions are stable, not
creative.
