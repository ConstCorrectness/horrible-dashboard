...
| `crm`        | records, browser, search, github, google, library | records            | session mode  |
...
- **A workspace picks the agent.** A `FramePreset` may declare `agent: '<id>'`, and
  ...
  CRM as `crm`, Data Entry as `intake`. Choosing another from the picker is a
...
than furniture: the CRM agent already knows which record is open, and the intake
agent which document is on screen, without spending a `list_open_panes` +
...
`get_pane_context`) stay in every agent's core — any agent may need to see what the
user is looking at. The sixteen *arrangement* verbs stay in core only for `main`
(driving the shell is its job) and for any spec that lists `layout` in its
`tool_groups`; every other agent gets them on demand via `load_tools("layout")`.
Measured with the interpretability recorder's own counter, that takes a scoped
agent's always-on core from **26 tools / 2351 tool-schema tokens to 7 / 563** —
the 16 arrangement verbs alone were 1455 tokens on every round of every turn.
