...
`registry.layoutController` seam (installed by `Workspace.tsx`), then replies.

**`LAYOUT_TOOLS`** (first slice, app-level verbs; the model discovers ids via the
read tools, so the catalog stays frontend-owned):
`list_available_panes`, `list_workspaces`, `open_pane(id)`, `close_pane(id)`,
`create_workspace(name)`, `switch_workspace(id)`.
...
**Next slices:** per-widget `agentTools` + a frontend→backend capability manifest,
`getAgentContext()` state-reads and widget interop, streamed tokens, and
parameterized commands-as-tools.
