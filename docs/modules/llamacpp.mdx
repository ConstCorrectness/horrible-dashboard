...
Without it, `llama-server` ignores the model's own chat template and serves a
generic one **that cannot emit tool calls**. The request still returns 200 with a
perfectly fluent answer, and every tool in the agent's schema is simply never
called — which looks exactly like "this small model is bad at tool use", the most
expensive wrong conclusion available here. It rides on every spawn and is asserted
by a test.
...
- `--jinja` present on every spawn (its absence = tool calls stop happening, with a
  200 and a fluent answer).
