...
| --- | --- | --- | --- |
| `records.grid` | **Rows** | document | The rows as an editable table. Double-click a cell to edit. Two sections: **Rows** and **Table setup**. |
...
| `records.list` | **Tables** | tool (`embedded`) | Table switcher, search, "+ New table", and the review queue. The **left region strip** (`r`) of all three documents above — a per-document switcher, not an [Explorer](explorer.mdx) section, which is where it briefly lived. |
...
**Table setup** is the `setup` section of the Rows pane (`records.grid`), reached
from the **+ New table** button in the Tables rail, the ⚙ beside the active table,
or the `records.newTable` command — all through `openTableSetup` (`open-setup.ts`).
It was a window of its own (`records.schema`) until the pane consolidation: a
one-off form in the launcher beside the views it configures, opening a fresh
document for every column you added. Saving or deleting hands back to the Rows
section. The designer's target rides `params.setupSchemaId`, deliberately not
`schemaId`, which *pins* a grid (below) — and "new" would pin it to nothing.
`records.schema` still resolves, via `RENAMED_VIEWS` and `VIEW_ALIASES`.
