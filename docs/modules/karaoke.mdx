...
| Queue | `karaoke.queue` | `embedded` — the stage's **left strip** | Running order, reorder, history. |
| Find songs | `karaoke.search` | `embedded` — the stage's **right strip** | YouTube search + the downloaded library, in one pane. |

The queue and search were dock tools until the pane consolidation. As docks they were
two more launcher entries for one machine and outlived the stage in whatever
workspace you left them in; as strips on `karaoke.stage` (both `defaultOpen`) they
come and go with it, and `openPanel('karaoke.queue')` reveals the strip.
...
**YouTube needs a JavaScript runtime.** yt-dlp solves YouTube's player challenges by
running scripts from `yt-dlp-ejs` (pulled in by the `yt-dlp[default]` dependency) in a
JS runtime, and it enables only **deno** by default. Without one, YouTube withholds
most formats and 403s the rest — measured 2026-09-10 on yt-dlp 2026.7.4, the pre-muxed
mp4 died with `HTTP Error 403: Forbidden`. `download_opts` therefore passes
`js_runtimes` for deno, node and bun; a missing runtime is skipped. The table is
**copied per download**, because yt-dlp mutates it in place.

**Video is two streams joined by ffmpeg.** YouTube no longer serves a single
pre-muxed mp4 for most songs — measured 2026-09-10 on yt-dlp 2026.8.19 with a runtime,
the format list held separate video-only and audio-only streams and nothing combined,
so the old `best[ext=mp4]/best` failed with "Requested format is not available". So
`download_opts` picks the format by whether ffmpeg is present, looked up **per
download** (`transpose.ffmpeg_path`, the binary pitch shift uses) so installing it
takes effect on the next song without a restart:

| Download | Format | Result |
| --- | --- | --- |
| Video, ffmpeg present | `VIDEO_FORMAT`: `bv*[vcodec^=avc1][height<=720]+ba[ext=m4a]`, then any mp4 video ≤720p + m4a, then a single file, then **last** video ≤720p (H.264 preferred) + any audio, for a video whose only audio is Opus/WebM — ffmpeg copies Opus into the mp4 | merged `.mp4`, `ffmpeg_location` passed explicitly |
| Video, no ffmpeg | `NO_FFMPEG_FORMAT`: `b[ext=mp4]/b/ba[ext=m4a]/ba` | a pre-muxed file if the video still has one, else **audio only** |
| Audio only (`start_download(..., audio_only=True)`) | `AUDIO_FORMAT`: `ba[ext=m4a]/ba/b` | `.m4a`; ffmpeg is never consulted |

H.264 comes first because it decodes in every browser and WebView without depending
on hardware codec support, which AV1 and VP9 do; 720p is plenty for lyrics on a TV.
Verified with a real `--test` download: with ffmpeg the selector took `136+140` (720p
H.264 + m4a) and merged them into one `.mp4`; without, it fell through to `140` and
wrote an `.m4a`.

Without ffmpeg the song still downloads, because a song you can sing to with no
picture beats a failed download. `download_song` recognises the fallback
(`got_audio_only`: the result's `vcodec` is `"none"`, or the extension is an audio one
when codecs are missing) and lands the row as **`ready` with a note in `error`**
(`AUDIO_ONLY_NOTE`). The Library row renders `error` inline and only disables a song
whose status is `failed`, so the note explains the black stage without blocking the
song. The Search tab shows an install hint whenever `/status` reports no ffmpeg. An
audio-only *request* — the Clubhouse room agent's `play_music` — carries no note: it
asked for audio on purpose.
...
| `GET /status` | What this node can do (yt-dlp / ffmpeg present). No ffmpeg means audio-only downloads and no pitch shift. |
...
build; its `[default]` extra brings `yt-dlp-ejs`, and YouTube additionally needs a JS
runtime (deno, node or bun) on `PATH`. ffmpeg is optional but gates two things:
**video** downloads (without it songs download as audio only, with a note on the row)
and pitch shift. Install it however your OS prefers and make sure it's on `PATH`.
