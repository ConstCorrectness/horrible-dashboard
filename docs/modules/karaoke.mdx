...
Downloads also have an **audio-only** mode (`start_download(..., audio_only=True)`,
format `ba[ext=m4a]/ba/b`), used by the Clubhouse room agent's `play_music`: music
that is heard rather than watched needs no video, and a single audio stream needs no
ffmpeg merge. That matters because YouTube's current format set often has **no
pre-muxed mp4 at all** — only separate video and audio streams — so the video path's
`best[ext=mp4]/best` can fail with "Requested format is not available" even with a
runtime present. Fixing video downloads (a merged selector, which needs ffmpeg) is
tracked separately.
