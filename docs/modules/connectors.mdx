...
Scopes: `read:user` (identify the account), `repo` (search code, read files, list
issues — private repos included) and `gist` (for
[notebook publishing](notebook.mdx#publishing)). GitHub has **no read-only `repo`
scope** for OAuth Apps, so the grant also permits writes; the agent still routes
writes through the permission gate.

`gist` was added after the first sign-ins, and a token only carries the scopes it was
issued with. `github.granted_scopes()` reads them off the stored credential rather
than assuming `SCOPES`, so a feature that needs a newer scope can say *reconnect
GitHub* instead of relaying GitHub's bare 404. The HTTP call itself lives in
`github_api.request`, shared by the agent tools and the publisher; its error dicts
carry a `status` so a caller can tell "not found" (create it) from "forbidden". Configure it **in the app** (the tile's Set up form), or with
...
a "Google Drive" root appear in Explorer's **Files** section, folders expand lazily, and files
...
### A browser for the human, over the same helpers

`huggingface_routes.py` puts those four capabilities on HTTP for the
[Lab](./lab.mdx)'s Hugging Face pane. It lives beside the connector rather than in a
module of its own, for the same reason as `github_routes.py` and `google_routes.py`:
the token is held by `huggingface.py` and reached through `huggingface_tools._request`,
and a separate module would have to import another module's private helpers.

| Method | Path | Purpose |
| --- | --- | --- |
| GET | `/api/connectors/huggingface/search` | `q`, `type`, `task`, `sort` — Hub search |
| GET | `/api/connectors/huggingface/mine` | the connected account's own repos |
| GET | `/api/connectors/huggingface/repo` | metadata + file list for one repo |
| GET | `/api/connectors/huggingface/file` | one text file (README, config.json, card) |

The routes call `_search` / `_list_repos` / `_repo_info` / `_read_file` directly, so
"find me a dataset" and "show me a dataset" are one implementation seen from two ends.
Everything the agent is protected from, the pane inherits for free: the 100 KB cap, the
binary refusal, the `type` split.

Three things differ because the caller is a browser, not an agent loop:

- **Errors become status codes.** `huggingface_tools` returns errors as values because
  an agent reads the message and decides what to do next; a fetch cannot. `_check` maps
  "isn't connected" to **409** and everything else to **502** — the pane offers a
  connect prompt for the first and a retry for the second, so collapsing them would
  produce the wrong prompt, not just the wrong number.
- **`repo` is a query parameter, not a path segment.** A Hub id is `owner/name`; as a
  path it needs escaping at every call site, and one caller forgetting is a 404 that
  looks like a missing repo.
- **Short-TTL caching** (60s searches, 300s repos/files, 200 entries), because browsing
  is repetitive in a way an agent turn is not. Failures are never cached — `_check`
  raises before the store — or a "not connected" banner would outlive the connection.

:::note `gated` is three-state
`true` / `"auto"` / `"manual"` all mean a licence must be accepted; `false` means open;
**`null` means the Hub didn't say**. The pane renders a chip only for the first group.
Coercing a missing value to `false` would tell the user a repo is open on no evidence.
:::
...
granted scopes and a Disconnect, or the connect flow when unconnected. Here it is
anchored under the tile that opened it; reached from anywhere else it is the same
component centred in a shell dialog — see below.
...
- `ConnectorPopover` is the **only** connect UI, but the row is not the only place
  it appears. `requestConnect(id)` from anywhere is answered by `ConnectorDialog`
  (`packages/ui/src/home/ConnectorDialog.tsx`), mounted at the shell next to
  `Dialogs`, which renders the same component as a centred modal
  (`variant="modal"`). So a pane never grows a second copy of the flow **and**
  never has to navigate to find it.

  It works this way because the alternative was tried: the taskbar clock flyout's
  integration tiles used to run `shell.setup`, which switched the desktop backdrop
  to the splash surface — where the row is mounted — and reset the setup card,
  dropping the connector id on the way. Clicking "Experiment trackers" from the
  clock landed you on the greeting with nothing open and the tile to find again.
  A connect request should never be a navigation.
...
An **api-key** connector (NGC issues personal keys and has no device flow) holding one
credential with two uses: the **NGC model catalog** and **NIM**, NVIDIA's hosted
inference.

**A NIM endpoint is one `PROVIDERS` entry with `dialect="openai"`, not a new dialect.**
NIM speaks the OpenAI chat API; a bespoke dialect would mean six new branches in
`providers.py` and would silently lose the `tool_choice="required"` retry, which is
gated on `info.dialect == "openai"` — the same reasoning the llama.cpp integration
records. It is the first openai-dialect provider that is *remote*, so `auth_headers`
was added to the shared request path: empty for every local server, a `Bearer` for NIM.

It is an ordinary **hosted** provider in `GET /api/agent/status`: listed with or
without a key, unreachable until one exists, pointing at build.nvidia.com. It used to
be filtered out until the connector held a key, following the `peer` precedent — but
`peer` is a provider nobody can obtain on purpose, whereas anyone can get an NVIDIA
key, so hiding it meant nothing anywhere in the agent settings said this node can talk
to NVIDIA at all. The request that filter was avoiding — an unauthenticated call on
every status poll — is instead prevented by the `MissingApiKey` check in
`list_models`, which now fires **before** the dialect branch rather than inside the
`litellm` one, because "a hosted provider without a key is not reachable" was never a
litellm-only rule.

`ProviderInfo.key_connector` is how the provider borrows the credential: non-empty
means `api_key_for` reads that connector's store, and means the key-write routes
(`PUT/DELETE /api/agent/providers/<kind>/key`) **refuse** this provider with a 400
naming the connector. One home — NVIDIA's key reaches NIM inference *and* the NGC
catalog, so a second copy under the provider kind would be a second thing to rotate,
a second thing to leak, and would skip the connector's own verification, which is what
proves a pasted key actually works. The API-keys settings section renders such a
provider as a status row saying where the credential lives, rather than a field the
backend would reject.

**`store.save` takes a `Credential` dataclass, not a dict.** This connector was
written against a dict-shaped store that does not exist, so pasting a key raised
`TypeError: asdict() should be called on dataclass instances` before anything was
written — the connector had never worked, and nothing caught it because nothing
tested it (`backend/tests/test_connectors_nvidia.py` now does).

**NVIDIA publishes no per-model price signal.** `/v1/models` is the plain OpenAI
shape (`id`, `owned_by`, `root`) with no pricing, and the *Free Endpoint* badge on
build.nvidia.com is rendered server-side with no JSON API behind it — NGC's catalog
search is a different universe (downloadable registry artifacts). So unlike
OpenRouter, whose `pricing.prompt`/`pricing.completion` decide `free_models`, a free
tag for NIM cannot be derived, and one is not invented. What it gets instead is
`ProviderInfo.tier_note` — one sentence about the tier, rendered **once beneath the
model list** in the picker (for the providers whose rows are actually in view) rather
than on each row. The distinction is the point: the note is true of the provider, and
repeating it per model would read as a claim about that model in particular, which is
exactly the claim nothing supports. `free_models` stays empty for NIM.

Connecting NVIDIA has nothing to do with the [hardware](./hardware.mdx) probe. That
reads the card in this machine with `nvidia-smi`; this is an account.

## Hugging Face gained a write scope

`write-repos` was added so a finished checkpoint or a dataset built here can be pushed
to the Hub (`huggingface.createRepo`, `huggingface.uploadFolder`).

**`manage-repos` is still deliberately absent**, and that distinction is the whole
safety argument the original "read-only" rule was really protecting: `write-repos` can
create a repo and upload files; `manage-repos` can **delete** one. A confused agent
that uploads a bad checkpoint has wasted bandwidth; one that can delete a repo has
destroyed work.

Adding a scope forces re-consent. A connection made before `write-repos` existed keeps
working for reads, and a push tells it to reconnect rather than failing with a bare 403
several megabytes in. The check order matters: **not connected** is reported before
**read-only**, or a disconnected account goes looking for a scope problem it does not have.
