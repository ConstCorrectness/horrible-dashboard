...
It is listed in `GET /api/agent/status` **only while a key is connected**, following
the `peer` precedent plus one more reason: probing it unconditionally would mean an
unauthenticated request to NVIDIA on every status poll, to be told what we already knew.
