...
- **Commands (C3):** `editor.newNote` (create + open a note, `/new`),
  `editor.newBuffer` (an untitled scratch **file**, `/scratch` — see
  [Untitled buffers](#untitled-buffers-and-language-detection)),
  ...
  **keybinding → `editor.save`** routed through the shell keybinding service — the
  ...
  quick-open is still to come.) **`ctrl+shift+space` → `editor.toggleSuggestions`**
  opens the completion popup, or closes it if it is open — a shell binding rather than
  a CodeMirror keymap so it is rebindable and listed with the rest, scoped
  `when: textInput` and acting on whichever view holds the caret (a buffer or a
  notebook cell). App-level keys route through the shell service;
  **editor-internal keys are CodeMirror keymaps scoped to the view** (they act on
  ...
  open completion popup, else **opens one** where completions could plausibly come from,
  else indents (**Shift-Tab** dedents); **F12** go-to-definition
...
- **Services for other modules:** `editor.openBuffer(source, { language? })` is the
  ...
  position, and region strips. When there is nothing to reuse but the editor is
  already on screen, the new buffer opens as **another tab in that same area**
  (`areaHostingView` + `openPaneInArea`) — role-routed `openPane` would split off a
  fresh area instead, which meant every file clicked in the tree carved the frame
  up further. Only a *first* editor buffer takes new space.
  ...
  live-only), `getBufferSelection` (live-only **by nature** — a selection exists in a
  CodeMirror view and nowhere else, so this returns null for an unmounted tab rather
  than falling back to bytes), `setBufferContent`, `getActiveBufferSource`, and
  `listBuffers`. The visualizer uses this to bridge templates ⇄ buffers (see
  [visualizer › Editor integration](visualizer.mdx#editor-integration)), and
  [llama.cpp](llamacpp.mdx#tracing-what-you-are-actually-writing) uses it to trace the
  selection through a model.

  Note what the service **cannot** reach: an untitled buffer. `getActiveBufferSource`
  is keyed by source URI and an unsaved buffer has none, so it is invisible to every
  cross-module consumer until it is saved. Callers should say that specifically rather
  than reporting "no open buffer" about a buffer that is plainly open.
- **Left region strip:** `editor.recentNotes` (**`embedded`**) — the most-recently-updated notes
...
### Untitled buffers and language detection

`editor.newBuffer` (`/scratch`) opens a buffer with **no source** — an unsaved *file*,
not a note. It appears as `untitled.md`, and `editor.buffer` supported this shape all
along (`saveDir` resolution, `adoptSource` retargeting on Save As); nothing opened it.

One resolver, `editor/language.ts`, answers "what language is this?" for both the
grammar and the LSP. It used to be two maps that disagreed: a regex over the tab title
chose the highlighting (JS/TS, Python, else Markdown — so a `.rs` or `.cpp` file
rendered as **Markdown** and nothing said so), and a separate extension switch chose the
LSP language id. Filename matching now comes from `@codemirror/language-data`, so the
grammar set is CodeMirror's rather than a table we maintain, and each grammar is a
dynamic import — nothing loads until a buffer needs it.

Precedence: **a manual pin, then the opener's `language` hint, then the filename, then a
content sniff, then Markdown.**

The sniff (`sniffLanguage`, pure and unit-tested) runs **only for a sourceless
buffer**. That gate is the whole design: a buffer with a name has already been told
what it is, and a `.py` file momentarily holding prose is still Python. A shebang is
decisive on its own; every other family needs **two** corroborating signals and a clear
winner, because a language that flickers mid-word under the cursor is worse than one
that arrives a beat late — `import x` alone is Python, JavaScript, Rust and Java at
once. It reads a debounced snapshot (600ms), not every keystroke.

Saving pins the answer: Save As proposes an extension from the *resolved* language (a
sniffed-Python scratch buffer offers `.py`), and once the file has a name the name
decides. The buffer header carries a compact language control that reports the resolved
language and lets you override it — the answer is sometimes a guess now, and a buffer
silently highlighting as the wrong language with the wrong completions is exactly what
saying so out loud prevents.

Adding a language is now **two** places, not three: `language.ts` and `LSP_SERVERS` in
`backend/modules/lsp/manager.py` (which gained `cpp` and `clangd`).

### Completion is no longer gated on the language server

`autocompletion()` used to be configured in exactly one place — inside `lspExtension` —
which meant only a `workspace-file:` in one of five languages had **any** completion
source. A note, an unsaved buffer, a `.json`, a Drive file had none, even though the
`code_symbols` index that would serve them was already built.

`editor/completion.ts` owns the stack instead, and the language server is one optional
source among several. `BufferView` mounts it through its own compartment for buffers
with no server; `lspExtension` calls the same builder with the server's source added.

:::caution One `autocompletion()` at a time, swapped in one transaction
CodeMirror merges configuration across live instances, but `override` is a
**replacing** field it has no combiner for: two live instances that both set it do not
degrade quietly, they make the *dispatch* throw
`Config merge conflict for field override`. The LSP compartment and the standalone
compartment are therefore configured from one predicate (`usesLanguageServer`) and are
never both non-empty.

That predicate is not enough on its own, because the two compartments live in one
buffer and a compartment is only as exclusive as the moment it is reconfigured. They
must swap in the **same transaction**. They used to be reconfigured from two separate
effects — the LSP one when the content arrived, the standalone one when the resolved
language changed — and between the two, the stack resolved while the tab title was
still the `…` placeholder (no language, so: standalone) was live alongside the one
resolved from the loaded filename (`gqa.py` → python, so: LSP). The throw landed
inside the load's `.then`, so the buffer's own `.catch` swallowed it into the status
bar and **the content transaction was rolled back with it**: every source file opened
from the Explorer came up blank.

Hence the load applies content on its own transaction first and reconfigures second,
guarded — a buffer that loaded is more use with no completions than one that shows
nothing.
:::

### Completing an import statement

`from <Tab>` and `from vllm import <Tab>` ask questions the identifier prefix query
cannot answer: the first has no prefix at all, the second wants one module's importable
names rather than a global scan. `editor/importContext.ts` classifies the cursor
(`importContextAt`, pure) and queries two routes:

| Route | Answers |
| --- | --- |
| `GET /api/editor/complete/modules` | Module names for a `from` / `import` statement. |
| `GET /api/editor/complete/members` | The names importable from one module. |

Both accept an **empty prefix** — unlike `/complete`, whose "no prefix, no rows" fast
path is correct for the hot identifier lookup and would be the exact bug here. With no
prefix the module query returns top-level modules only; listing every dotted submodule
of every indexed package is thousands of rows and none of them is the question.

Two rules do real work:

- **The member query filters on `imp`, not `module`.** For a method, `module` holds the
  *class* it belongs to (`extract_packages` stores `member_of or module`), so filtering
  on `module` would answer `from vllm import …` with `LLM`'s methods and miss the
  package's own names. `imp` is by construction "the module this symbol can be imported
  from", and is empty for methods.
- **A member completion applies as a bare name**, never through `applyImport`. We are
  already inside the import statement; routing through it would insert a second
  `from vllm import LLM` line above the one being typed.
- **A package with no rows of its own falls back to its submodules.** `imp` is where a
  symbol is *defined*, never where it is re-exported from — `transformers` re-exports
  `AutoModel` out of `transformers.models.auto` — so the exact match answered
  `from transformers import <Tab>` with **nothing** while the index held 530
  transformers symbols. Same for numpy and torch: for every package that assembles its
  public surface in `__init__.py`, which is most large ones. Only packages that define
  their names directly there (huggingface_hub, sympy) ever worked, which is what made
  this read as a flaky index rather than a structural miss.

  The fallback fires **only when the exact match is empty**, and that is the whole of
  its safety. `vllm` really does define `LLM` and really does not re-export
  `LoRARequest`, so blending submodule names into a package the index already describes
  correctly would offer an import that fails. Where there is nothing to be wrong about
  a guess costs nothing; where there is, the precise answer stands alone. Fallback rows
  carry their defining module in `detail`, so the guess is legible as one.

Modules rank by import **depth** first, so `vllm` beats `vllm.lora.request` — the same
rationale that tiers the identifier query.

### Opening the popup: three gestures, and why it needed all three

An empty prefix is the case this whole path exists for, and it is also the one nothing
can auto-trigger on — there is no word to match. So it is answered when the user asks
(`explicit`), or at the one moment they are unambiguously waiting to be told:

- **Typing the space after the keyword.** `importContextAt` reports `justOpened` when
  the cursor sits directly after `import`, `from`, a comma or an open paren, and one
  space at most. That is where VS Code opens its suggest widget, and where a person
  reaches for it. A *second* space, or a space following a half-typed name, is not
  that moment — opening on every space in the statement is what the empty-prefix guard
  originally prevented, and still does.
- **Tab** — see below.
- **`ctrl+shift+space`**, the `editor.toggleSuggestions` command. Deliberately **not**
  CodeMirror's own `ctrl+space`: `keymap/reserved.ts` lists that as the IME toggle on
  Windows and the input-source switch on macOS, both `preventable: false`. It is exactly
  the shape of binding that table exists to catch — it never reaches the page, and
  nothing says why. Toggling rather than only opening, because the same key then
  dismisses; a popup you can summon but must reach for Escape to close is half a
  binding. `when: textInput` scopes it to a focused editor surface, and
  `focusedEditorView()` resolves which one via `EditorView.findFromDOM` — a notebook has
  one view per cell, and the only one the command can mean is the one holding the caret.

### Tab opens the popup

Tab accepts an open popup, then tries to open one, then indents. The middle rung is the
whole design risk: `startCompletion` returns true whether or not it finds anything, so
calling it unconditionally would swallow indentation everywhere. `tabStartCompletion`
therefore declines — falling through to `indentWithTab` — unless the selection is empty,
the text before the cursor on this line is **not** entirely whitespace (that is
indentation, unambiguously), and either a word character precedes the cursor or the
cursor is in an import context. Ghost-text autosuggest keeps its `Prec.highest` Tab, so
an inline suggestion is still accepted ahead of all of it.

### Notebook cells get the same stack

They did not. `buildCompletion` only ran inside `lspExtension`, and the Tab keymap was
added by `BufferView` alone — so a **notebook cell** had no completion source unless a
language server was attached, and no cell ever had the keymap. The visible symptom was
the one this section is about: `from transformers import ` in a cell offered nothing,
Tab escaped to browser focus traversal instead of asking, and `ctrl+space` was eaten by
the IME. Three separate gaps, one indistinguishable outcome.

Both halves now reach a cell, and which one is a real decision:

- With a server, `NotebookLspDoc.cellExtension` returns the LSP extension **plus**
  `completionKeymap`.
- Without one, `EditorService.bareCompletion('python')` returns the standalone stack
  plus the same keymap — that is every cell on a node with no Python server, every cell
  of a notebook opened before its absolute path resolves, and every cell during the
  seconds a cold server takes to come up.

They are alternatives and never both, for the reason in the caution above: two live
`autocompletion()` instances make the dispatch that installs the second one throw.
`useNotebookLsp` picks one and the cell's compartment swaps between them.

`bareCompletion` is on `EditorService` rather than deep-imported because the notebook
kit is domain-neutral — it knows this contract and nothing else about this module.

`bareCompletion` and `openNotebookLsp` both return a **promise**. Their
implementations are CodeMirror, and the service is registered at boot, so a static
import in `service.ts` put all of CodeMirror (1.7 MB) on the boot path of every
session, notebook or not. They import it on first use instead; `useNotebookLsp` hands
a cell `[]` for the few milliseconds that takes, and disposes a handle that resolves
...
### Save As writes a file, not a note

`saveAs()` always writes a **`workspace-file:`** — an untitled buffer is an unsaved
*file*, and turning it into a `note:` (as this used to) put the work somewhere the
file tree, git, and every other tool couldn't see it. Notes remain a first-class
source; they're just something you open deliberately (`/new`), not what Save
silently produces.

Three consequences worth knowing:

- **The destination is shown, not implied.** The prompt is prefilled with the full
  absolute path and names the directory a bare name would land in. That directory is
  the first non-virtual **workspace root**, which is also what the backend anchors a
  relative path to (`_resolve_relative` in `backend/modules/files/routes.py`), so the
  path shown is the path written. The header carries the same information for every
  buffer: the file's full path, or — while untitled — the file it would become. With
  no workspace root configured at all (`HORRIBLE_NO_DEFAULT_ROOT`) Save says so
  instead of silently inventing a destination.
- **An untitled buffer adopts what it just wrote.** It `retargetPane`s onto the new
  `workspace-file:` source, so the title bar, the tab, and every later `mod+s` follow
  the file — the buffer stops being "Untitled" the moment it stops being untitled.
  Retargeting rather than reopening keeps the pane's area, tab position and region
  strips. If that file is already open in another buffer the retarget is refused and
  the save is simply reported.
- **A buffer that already has a source keeps it.** Save As there writes a *copy* to
  the new path and leaves the buffer pointed at its own source — which is what the
  close prompt needs, since re-pointing a closing buffer would be meaningless.
...
Servers spawn only when installed (else the editor silently degrades); each
language maps to an **ordered list of candidate servers** — the first whose executable
resolves wins. An executable resolves from the **backend interpreter's own scripts
directory first**, then `PATH`. The scripts directory is not optional: the desktop
supervisor launches `.venv\Scripts\python.exe` directly, which puts nothing on `PATH`,
so a PATH-only lookup never found the dev dependency `basedpyright` and silently
picked a globally installed `pylsp` instead — whose pyflakes reports `!pip install` in
a notebook cell as `invalid syntax`, where basedpyright understands IPython lines. Python prefers **`basedpyright`** (`basedpyright-langserver --stdio`;
...
### Notebook cells get the same language server

Both notebook panes — the `notebook` module's and `training`'s — attach a language
server to their **code cells**, so a fine-tuning notebook completes against the
`torch`/`trl` in its own project venv rather than against nothing.

A notebook is **one document made of cells**, not N little files: `import torch` in
cell 1 is what makes `torch.` complete in cell 7. That is exactly what LSP 3.17's
`notebookDocument/*` notifications describe, and basedpyright implements them, so
`editor/notebook-lsp.ts` is a *coordinator* rather than a second client. One
`NotebookLspDoc` per open notebook owns the session and the ordered cell array; each
cell's CodeMirror view gets an ordinary `lspExtension` whose **`DocumentBinding`**
routes sync through the notebook instead of through `textDocument/*`. Panes reach it
through `EditorService.openNotebookLsp` and the `useNotebookLsp` hook — the notebook
kit stays domain-neutral and never deep-imports the editor module.

```mermaid
flowchart LR
  Pane["notebook pane<br/>(cells in order)"] -->|sync| Doc["NotebookLspDoc"]
  Cell["CellEditor<br/>(one per cell)"] -->|DocumentBinding| Doc
  Doc -->|"notebookDocument/didOpen<br/>didChange · structure"| Session["shared LspSession"]
  Session -->|"publishDiagnostics<br/>(per cell URI)"| Cell
```

Four things here are **silent** when wrong — the server stays up and answers, it just
answers about a document that isn't yours. All four were measured against
basedpyright 1.39.9 rather than read off the spec:

- A cell document must use the **`vscode-notebook-cell:` scheme**. Under `file:` with
  a `#cell0` fragment the server accepts the notebook and then resolves nothing: no
  diagnostics, empty completions, no error. The fragment itself is free-form, so it
  carries the nbformat cell id (stable across reordering).
- A cell's edits must ride **`notebookDocument/didChange`**. A plain
  ...
  the server analyzing the text the cell was opened with — which reads as "completions
  are stale" rather than as a sync bug.
- **Only code cells** belong to the array. A markdown cell shifts every later cell's
  index, so a structural edit would land in the wrong place.
- `didOpen` must carry the **real cells**. `fetchPythonEnv` is cached per directory, so
  on a second open the session is ready before React has run the effect that describes
  the cells; opening anyway sent `cells: []`, and a notebook the server believes is
  empty resolves nothing in any of it. The cells are the trigger for `didOpen`, not the
  session.

The pane must also hand the coordinator an **absolute** path. Both modules address
notebooks relatively (to the notebook root, or to a training project), but the
interpreter and project root are resolved by walking *up* from the file — so each pane
joins its own root first. `useNotebookLsp` disables itself on a relative path rather
than resolving some unrelated directory's environment.

Cells deliberately get **no F12/F2**: a definition or rename can land in another cell,
and jumping there means mapping a cell URI back to a mounted editor the coordinator
does not own. A key that silently does nothing across cells is worse than no key, so
cross-cell navigation waits for the panes to expose a "reveal cell" callback. Hover
still works, and the cell's docs chain narrows to the **kernel** source — the one thing
a language server cannot do is ask the live kernel what the object actually is.
...
`plt`, `nn`, `DataFrame`, `Tensor`, `AutoModelForCausalLM`, `LLM`, `AsyncLLMEngine`, …)
offers it in any `.py` buffer, and accepting **inserts the symbol and prepends the matching `import`**
...
Declared in the editor manifest and edited on the [settings](settings.mdx) page. The
language-server ones are captured when a buffer opens (the LSP extension lives in a
compartment reconfigured on load), so a change takes effect for **buffers opened after
it**. The last two below reach the *live* completion compartment instead and apply
immediately.
...
| `editor.completionTrigger` | `auto` | When the popup opens. `auto` opens it as you type (and on Tab/Ctrl-Space); `manual` keeps the buffer quiet until you ask. Tab still indents wherever there is nothing to complete. Applies live. |
| `editor.importCompletions` | `true` | Complete module names and members inside `import` statements, including on an empty prefix. Python only. Applies live. |
