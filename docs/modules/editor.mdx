...
:::caution One `autocompletion()` at a time
CodeMirror merges configuration across instances, but `override` is a **replacing**
field — a second live instance's override silently wins and the first one's sources
vanish with no error. The LSP compartment and the standalone compartment are therefore
configured from one predicate (`usesLanguageServer`) and are never both non-empty.
:::
...
`autocompletion()` instances mean one `override` silently replaces the other's sources.
