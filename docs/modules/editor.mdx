...
- **Gated:** `editor.applyEdit(uri, edits)` and `editor.save(uri)`, matched by the
  same filesystem `Edit`/`files.write` rules the tree uses. Both auto-allow under
  the `acceptEdits` mode (Claude Code parity). See the
  [permission system](../architecture/agent-tools.mdx#the-permission-system).
