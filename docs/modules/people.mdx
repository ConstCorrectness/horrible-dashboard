...
| **Discover** | Username search, and the agent commons directory | `commons.directory` |
| **Requests** | The inbound inbox | `commons.requests` |
| **Me** | Your username, friend code, devices, profile, diagnostics | `commons.profile` |
...
## `@username` — one name for a person
...
account is the identity**. A username is chosen at sign-up and is unique; the
server holds one person key per account and signs the certificate of every machine
that signs in to it, so all of them are one person. See
[One account, every machine](social.mdx#one-account-every-machine).

```mermaid
sequenceDiagram
    participant N as Your machine
    participant G as Game server
    participant P as Someone looking for you
    N->>G: POST /account/devices {node_id, node key, ts, node signature}
    Note over G: bearer proves the ACCOUNT<br/>signature proves the MACHINE
    G-->>N: certificate signed by the account's person key
    P->>G: GET /directory/resolve?handle=rob
    G-->>P: @rob → person_id + pubkey + machines
    Note over P: fingerprint(pubkey) == person_id?<br/>each machine's certificate verifies?
    P->>N: friend request over the fabric (relay)
```

### What the two proofs are for

The bearer token proves *account*; the node signature proves *machine*. **Both** are
required: without the machine proof, a signed-in user could list someone else's
machine under their own username. The signed challenge includes the account id and
a timestamp, so it cannot be replayed into another account or reused later.
...
the key it came with, and each machine's certificate verifies under it. Since the
server holds that key, it vouches for the whole entry; the username is how people
add each other, and trusting the server that issues it is the design.
...
key it names, which says nothing about who owns the username. Resolve a username
...
`roster.resolve_target` is the shared entry point: `@username` first (exact and
...
against the node's implementations, and `test_games_account_identity.py` asserts
the two sides build byte-identical enrollment challenge bytes. The fixture pins *agreement*; each side's own tests pin correctness.
...
- **Friends** — one row per person: face, name, level, what they're up to, an
  ...
  friends. Adding a friend and renaming yourself moved into a
...
card: a friend with no username, a signed-out node and an unreachable game server
...
- **Game server:** `POST /account/devices`, `GET /directory/resolve`,
