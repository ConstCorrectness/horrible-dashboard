...
Once minted, the link row carries a **copy button** — `CopyableLink`'s opt-in
`showCopy`, off for every other caller. Elsewhere in the app a link is somewhere
to *go*, and clicking it is the point; here the URL **is** the artefact being
handed to somebody else, so opening it yourself is the least useful thing you can
do with it. It duplicates the `share.copyLink` command deliberately: the command
is for someone who already knows it exists, and the pane is where everyone else
is looking.
