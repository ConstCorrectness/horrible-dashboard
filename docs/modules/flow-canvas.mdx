...
- **Panels:** `flow.editor` (center, multi-instance — one pane per open flow keyed
  by `flowId` param; falls back to the active flow when opened with no param),
  `flow.library` (left, singleton). `flow.runs` is Phase 2.
- **Panel group:** `flow.studio` — `flow.library` is the primary (the entry point
  that appears in the command palette). Opening Flows renders a `PaneGroupShell`
  with a **`⬡ Canvas`** toggle in the strip; clicking it opens `flow.editor` as a
  resizable companion pane showing the active flow. The companion falls back to
  the active flow via `getFlows().active` (same logic the standalone editor uses
  when opened with no param). See [panel groups](../architecture/panel-groups.mdx).
