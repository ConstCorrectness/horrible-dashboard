...
- **Tool node**: a single deterministic call to one existing agent tool or command
  (`files.read`, `terminal.exec`, `database.query`, `open_pane`, `visualizer.*`…).
...
- **Data nodes**: database query/semantic-search, read a pane's `getAgentContext`, write to a
...
- **Panels:**
  - `flow.editor` — the canvas, **multi-instance** (a Panel keyed by `flowId` param,
    so you can open several flows at once), role `document`. With no param
    it loads the active flow (creating a first one if none).
  - `flow.library` — list of saved flows; create/open/delete. `embedded`, hosted by the **left region strip of `flow.editor`** — its only home. It was briefly also a section of [Explorer](explorer.mdx); that was a second home for a switcher that only means anything beside the canvas it re-points, and it is gone.
...
- **Panels:** `flow.editor` (document, multi-instance — one pane per open flow
  keyed by `flowId` param; falls back to the active flow when opened with no
  param), `flow.library` (tool, `embedded` — the canvas' left region strip). `flow.runs` is Phase 2.
- **Regions:** the canvas is the document and hosts the library as its **left
  [region strip](../architecture/panel-groups.mdx)** (`f` while the canvas is
  focused) — inverted from the old grouping where the library was the hub, to
  match how every node editor centers the canvas (Blender's shader editor,
  ComfyUI).
- **Commands:** `flow.new` (create + open a new flow), `flow.openLibrary` (open
  the library). (Run/stop live on the canvas toolbar; agent flow-CRUD tools are Phase 3.)
- **Frame preset:** an **Orchestration** workspace (library dock · canvas) in the
  workspace tab strip.
