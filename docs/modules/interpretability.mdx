...
cache contents, and gradients are all unavailable through the HTTP API of every
provider it can talk to, which returns tokens and counts — nothing else.

The model explorer narrows that gap on one side: reading the GGUF directly gives the
real **weights inventory** — every tensor's name, shape and quantization — so the
structure shown is measured rather than described. It stops at the file. Nothing here
runs a forward pass, so there are still no activations and no attention maps.

Getting those is a planned, separate piece of work: a llama.cpp-backed *inspect*
provider that coexists with Ollama and captures ggml's per-node `cb_eval` tensors
into a **snapshot trace on disk**, scrubbed offline rather than streamed. That path
deliberately avoids autograd — the backend has no torch and should not grow one; heavy
dependencies live in per-project uv envs (`backend/modules/training/envs.py`) or
optional extras. Until it lands, take this module as inspecting the **prompt** and the
**weights**, never the computation between them.
...
| --- | --- | --- |
| The model's own **GGUF header** | Ollama, LM Studio, or any server with `interpretability.ggufPath` set | The **inventory**: every tensor's name, shape, ggml quant type and byte size |
...
2. **Ollama's blob store** — its manifest names the layer whose `mediaType` is
   `…image.model`; that digest is the blob. `OLLAMA_MODELS` relocates the root.
3. **LM Studio's index** (`~/.lmstudio/.internal/model-index-cache.json`), where
...
Step 3 is not gated on the provider being LM Studio — llama.cpp and other
OpenAI-dialect servers are often pointed at a model LM Studio also has indexed, and
a hit there is a hit on the same file.
...
`tool` panes are dock-only — so both panes are `role: 'widget'` even though they read
like tools. Same inversion as the observability module.
