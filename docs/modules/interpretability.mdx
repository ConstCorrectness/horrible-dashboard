...
- **Retention is in-process.** 25 turns, cleared on backend restart. Nothing is
  persisted to `app.db`; replay/diff across sessions would need that. The ring counts
  delegated sub-turns and peer nodes as entries, so a heavily-delegating session fills
  it faster; an orphaned child (parent already evicted) is promoted to a root rather
  than dropped.
