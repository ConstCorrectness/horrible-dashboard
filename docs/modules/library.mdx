...
- A **source** is one ingested item — a `blog`, a `note`, or a media asset
  (`image` / `video`). Catalog rows live in the `library_sources` table in the app's
  SQLite database (`$HORRIBLE_DATA_DIR/app.db`).
...
- **PDF/EPUB parsing** as a first-class source `type` + a **file-upload dropzone** and
  `POST /library/upload` multipart route. The `type` enum and pipeline are built to
  absorb `pdf`/`epub` by adding an extractor branch.

  Partially overtaken: the [connectors](connectors.mdx#drive--library-sync) module
  already extracts PDF text (`pypdf`, in `providers/drive_api.py`) to sync Drive PDFs
  in as `note` sources. That's the extractor a `pdf` source type would reuse — what's
  missing is the upload route and the `type` branch, not the parsing.
