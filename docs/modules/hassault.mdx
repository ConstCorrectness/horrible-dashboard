...
#### Five things it exists to get right
...
| `carbine-m4a1` | **687k**, 25 meshes | `x` | needs decimating first |
...
Three weapons now render as real models — pistol, shotgun, sniper. `assault` and
`knife` do not, and that is the answer rather than a gap: the M4A1 is 687k
triangles and there is no knife model. **A weapon with no prop keeps its
procedural boxes**, which is a complete working weapon and was the only kind
there was. `tests/browser_parity.rs` pins the two clients' lists equal, because a
...
The GLBs are compiled in with `include_bytes!`, like the operator, and one prop
is resident at a time — a swap re-uploads a few thousand vertices and a megabyte
of texture, against four times the VRAM to avoid an upload nobody can perceive.
...
well), the fog's density, and the sample count. **High is the browser's own fog
density**, not a value picked to look close — it is the level at which the two
clients are meant to be the same picture, and the lower levels trade distance for
fill rate from there.
...
fog, which is the more visible difference anyway. Note that Low returns the
...
| Quick-toggle buttons | `F1`–`F8`, drawn as live state chips |
