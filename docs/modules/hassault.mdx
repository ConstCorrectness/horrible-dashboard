...
## The workspace: one game, several panes

The game is one pane, but almost everything you do *around* it is another one —
the model studio, the armory, the tactical radar, the match companion, the
developer console. `workspace-bus.ts` is the channel between them, and it exists
because the obvious implementation does not survive contact with the frame engine.
...
The panes are arranged by `frames:` presets rather than left to the user to
assemble, since the useful combinations are few and specific:

| Preset | Layout |
| --- | --- |
| `hassault_dev` | Play beside the studio (55/45), console below, companion + radar right. |
| `hassault_mapmaker` | Studio at 68% with a live match beside it to test what you just built, radar right. |
| `hassault_armory_studio` | Armory and studio 50/50; console present but closed. |
| `hassault_play` | Just the match, with companion + radar + voice stacked right. |
...
- **Panels:**
  ...
  | --- | --- | --- | --- |
  | `hassault.play` | document | ✅ | The match. `fullscreen: true`. |
  | `hassault.studio` | document | — | Model studio / level editor / scene outliner. |
  | `hassault.modelViewer` | document | — | Asset inspector. |
  | `hassault.modelEditor` | document | — | Mesh editing. |
  | `hassault.animEditor` | document | — | Animation clips. |
  | `hassault.armory` | document | — | Inventory, catalog, trade-up forge. |
  | `hassault.console` | tool | — | The developer console. |
  | `hassault.companion` | tool | — | Telemetry, vitals, team scoreboard. |
  | `hassault.radar` | tool | — | 2D tactical minimap and callouts. |
  | `hassault.voice` | tool | — | Voice comms. |

  `hassault.play` is the only singleton, and the only `fullscreen: true`. It is a
  `document` because it wants the centre area and a lot of pixels, and it grabs
  pointer lock, which would be hostile in a narrow dock. The `fullscreen` opt-in
  earns it the `⛶` titlebar control and Escape-able presentation over the taskbar
  and the workspace strip — and on the desktop build that escalates to OS-window
  ...
  The other nine are deliberately **not** singletons: two studio panes on two
  monitors, or a radar beside a companion, is the arrangement the
  [workspace presets](#the-four-workspaces) exist to set up. They talk to each
  other over the [workspace bus](#the-workspace-one-game-several-panes), not by
  reaching into one another.
- **Workspace presets:** `hassault_dev`, `hassault_mapmaker`,
  `hassault_armory_studio`, `hassault_play` — see
...
10. **A workspace, not a pane** — done: ten panes (play, studio, model viewer /
    editor / animation editor, armory, console, companion, radar, voice) wired
    together by a typed
    [event bus](#the-workspace-one-game-several-panes) that parks an intent for a
    pane that has not mounted yet, and four
    [workspace presets](#the-four-workspaces) that arrange them.
