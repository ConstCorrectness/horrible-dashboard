...
- **Training in the native client has no dummy range.** `training.ts` — the
  ammo, the reloads and the static dummies that make Train a place to practise
  the shoot-jump — has no Rust port, so native Train is a map, a body and a
  weapon that does not fire. The pane's Train is still the complete one.
