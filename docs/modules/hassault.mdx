...
to the version. The browser's CacheStorage name is derived from the same `version`
(`models/assetCache.ts`), which is what invalidates GLBs cached under an unchanged
filename. **v2** added `hassault-operator-t`, `hassault-weapon-knife` and the three
