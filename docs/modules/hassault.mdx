...
0.7 above). `generate_office.py` was authored in metres — a 4-unit storey — so the
body could not fit under the ceiling and every spawn resolved onto the roof, with
no error anywhere. Its footprint was already in cube units, so the fix scales
**only Z** (`VERTICAL_SCALE = 3.0`, ceiling at 12, what `hd_office.json` declares),
inside the two primitives everything is built from. Five of its eight client
spawns and six of its ten items were also inside desks, a pillar or cubicle
partitions; they were moved to the nearest clear floor.
...
world built from the map's JSON brushes, with the JSON's `spawns`. For hd_office
the two describe different layouts; that divergence is not resolved here.
