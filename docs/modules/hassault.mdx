...
`hassault.sensitivity` setting and also adjustable live from the pause menu — the
two are the same value, so the settings page and the in-game slider can never
disagree.
...
- **No weapon sounds.** There is a procedural first-person model with a walk bob,
  recoil kick and reload animation (`viewmodel.ts`), and a tracer and crosshair
  kick for everyone else's shots, but nothing to hear yet.
...
7. **Bot players** — done: reactive AI at three skill levels, producing ordinary
   ...
   simulation.
8. **Textures and map models** — the largest remaining gap between this and
   something that looks like AssaultCube. Needs the `.cfg` parser that maps
   texture slots to files.
9. **Pickups** — ammo and health entities are already in the map file; placing
   them turns the ammo economy from a countdown into a reason to move.
10. **Game modes** — team deathmatch is what the scoreboard currently describes.
