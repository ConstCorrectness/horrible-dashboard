...
| **B4** | Play/Train/Host route through it | not started |

It plays: it loads any map the node serves, joins a real match, renders it, and
predicts your movement locally. What it does not yet do is *replace* the browser
client as the way in — Play, Train and Host still open the pane, and the native
client is launched from a Settings row. That is B4, and it is deliberately last:
routing people into it before the prediction was trustworthy would have meant
shipping the rubber-banding as the default experience.
...
`hassault.nativeClient` routes **Play, Train and Host** to the native window
instead of this pane; `hassault.nativeBinaryPath` says where the binary is when it
is not in the usual build output. Off by default, and the Play section says so
while it is on — the two clients do not look alike (the native one has no
sound and no scope), so a button that quietly opened a different window would
read as the game having broken.
...
- **Settings:** `hassault.installPath` — the folder containing `packages/maps`.
  ...
  instead of playing in the pane, and `hassault.nativeBinaryPath` — where that
  binary is, when it is not in the usual build output. Both are declared rows: the
  path was read by the launch route and named in its own error message long before
  it was editable anywhere.
...
- **The native client is not yet a way *in*.** It renders, joins and predicts, but
  Play/Train/Host still open the browser pane; the native client is launched from
  a Settings row. That is stage B4 — see
  [the native client](#the-native-client-rust--wgpu).
