...
Five maps ship with the app — `hd_atrium`, `hd_crossing`, `hd_pit`,
`hd_facility` and `hd_junkflea` — so the pane is playable on a clean clone. They
are ours, CC0, and named `hd_*` so the bundled and installed catalogs share one
namespace without either being able to shadow the other.

The last two are **`gltf` maps** rather than Cube heightfields; see
[Two level formats](#two-level-formats-cube-and-gltf) for what that changes. The
adjudicable set is pinned exactly — not as a subset — in
`test_games_hassault_rooms.py`, because the point of that assertion is that the
set a rated match may be played on is *closed*, so a map appearing in it without
being added deliberately is precisely what it should catch. `assets.load_map` resolves bundled names first: an install directory is
...
`assets.py` is the map catalog and nothing else. The 3D asset cache — the
GLB download/verify pipeline behind `/api/hassault/assets/*` — is
`asset_cache.py`, deliberately a separate module: it was briefly written *over*
`assets.py`, which removed `install_root`/`list_maps` from under six call sites in
one commit. `GET /api/hassault/status` then 500'd, and because that call is the
loader's `install` stage, the pane froze at 17% showing a bare fetch error rather
than anything naming the module that had gone missing.

The manifest (`packages/core/src/modules/hassault/assets.manifest.json`, mirrored
to `assets/manifest.json` by `pnpm assets:pack`) is **one release**: its `sha256`
values describe the files on the `assets-v<version>` GitHub release its `baseUrl`
points at. Adding or rebuilding a GLB therefore means bumping `version`, moving
`baseUrl` to the new tag, and publishing that release with every file — not just the
changed ones, since a client pulls everything from one URL. Neither `assets:pull` nor
`asset_cache.sync_assets` refuses a hash mismatch (both warn and install), so a stale
`baseUrl` silently serves the previous release; `assets-manifest.test.ts` pins the tag
to the version. The browser's CacheStorage (`models/assetCache.ts`) keys each entry by
the asset's own **`sha256`**, and caches **only manifest assets** — CacheStorage never
revalidates, and keying by `version` alone let the `hd_*.glb` maps (committed files,
not release assets, so they have no manifest entry) be rebuilt under v3 while every
browser that had opened one kept drawing the old geometry. The maps now ride the
ordinary HTTP cache, which revalidates. **v2** added `hassault-operator-t`, `hassault-weapon-knife` and the three
`hassault-grenade-*` props. **v3** introduced the procedural PBR tactical knife redesign
and 5 rare knife archetypes (`karambit`, `butterfly`, `bayonet`, `skeleton`, `huntsman`).
...
### GLB map surfaces: UVs are derived, colour is authored

The modelled maps (`hd_*.glb`, written by `tools/blender/generate_*.py`) carry a
base colour per material and **no textures and no `TEXCOORD_0`**. The second was a
silent bug: the native loader read UVs off the mesh and fell back to `(0, 0)`, so
every triangle in a map sampled one texel — each surface one flat colour, and that
colour the corner of its tile (a concrete seam, which is why dust2's sandstone drew
near black). The browser drew the bare base colour with no texture at all.

Both clients now do the same three things:

- **Classify** the material by name against `glb-surfaces.json` — ordered
  lowercase-substring rules, first match wins, read by `glb-surfaces.ts` and by
  `textures3d.rs` via `include_str!`, so there is one table, not two. `none`
  (emissive, liquid, foliage, printed surfaces) gets no tile. Rule order is
  load-bearing: `mat_ceiling_tile` must hit plaster before marble's `tile`, and
  `mat_wood_crate` crate before wood.
- **Project UVs** from the world position along the dominant axis of the **face**
  normal (a box with shared corners has diagonal vertex normals, which smear every
  face), ties broken like the shader's `detail_uv`, scaled by the kind's
  `tileScale`. The browser de-indexes the geometry to do it.
- **Treat the tile as detail, not colour.** Every multiplied layer is normalised to
  grey luminance with a linear mean of `detailMean` (0.667), and the albedo is
  multiplied by `detailGain` (1.5 — the native shader's literal), so a surface
  averages out to the mapper's colour. A high-contrast tile (hazard) is
  *compressed* about the mean rather than scaled into clipping, which would drag the
  mean back down. Glass and the site decals are blended, not multiplied, and stay as
  drawn.

Six tiles exist for these maps: masonry (running-bond ashlar), plaster, cobblestone
(jittered Voronoi), barrel roof tile, carpet and brick. They are ported in both
languages and use a *tiling* fBm (`fbmTiled`, each octave's period dividing the
tile) — the older tiles' `fbm` does not tile and seams at every repeat.

`glb-surface-vectors.json` pins agreement: the classification of every material in
every bundled GLB (read out of the files at generation time) and a set of UV cases,
written by `glb-surfaces.test.ts` (`UPDATE_GLB_SURFACE_VECTORS=1`) and replayed by
`textures3d.rs`. A new generator material that falls through to the `concrete`
default is caught by a count in the same suite.

### GLB maps are in cube units, and a spawn needs room for a body

One world unit is one cube and a standing player is **5.2** of them (eye 4.5 +
0.7 above) — about a 1.75 m person, so roughly **three cubes to the metre**. Every
`tools/blender/generate_*.py` is authored in **metres** (a 70 m Dust II, a 4 m
office storey, a 2.6 m shipping container), and for a long time the result was
exported as-is and read as cubes. That made every modelled map a third the size
it was drawn at relative to the player: Dust II was thirty body-widths across,
its 4.4-unit "grand" archways barely cleared a 5.2-unit body, and its 4.2-unit
tunnel was shorter than the player standing in it.

The conversion now lives in one place, **`tools/blender/maplib.py`**:
`export_map_glb(name)` bakes modifiers (so a bevel's width scales with the mesh
it rounds) and scales the finished scene by `MAP_SCALE = 3.0` about the origin
before export. It is **uniform** — scaling only the footprint would turn every
crate into a slab — and it keeps the node tree intact (for a uniform scale,
multiplying each translation in the chain and each mesh once is exactly
`D · world`), so every node name the clients classify by survives. The
generators keep authoring in metres; everything that places something *on* a
map is in cubes and was multiplied once: the JSON's brushes, spawns, items and
sites (`sfactor` 6 → 8, since 210 cubes no longer fits a 64 grid), the
`MAP_CONFIGS` table in `world3d.ts`, its twin in `world3d.rs`, and
`bots.TACTICAL_COVER_NODES`. The procedural fallbacks (`createProcedural*`,
`create_procedural_*`) still build the metre layout and are scaled on the way
out (`scaleFallback` / `scale_fallback`), so a fallback lands where the GLB
would have.

`hd_office` was the one exception before this: it had been fixed by scaling
**only Z** (`VERTICAL_SCALE = 3.0`) inside its primitives, which kept a metre
footprint under a cube ceiling. It now authors plain metres like the rest; its
heights come out identical and its footprint triples. Its tables' heights were
already in cubes, so only their x/y were multiplied. In hd_bank's JSON, the
knee-high cover blocks stay at floor 1 rather than 3 — the brush world is a
rough stand-in for the GLB, and a three-cube block is a jump, which the on-foot
reachability check rightly counts as sealing the cells on top off.

`HASSAULT_MAP_SCALE=1 HASSAULT_MAP_OUT=<dir>` runs a generator unscaled into a
scratch directory. At 1.0 every generator reproduces the committed pre-scale GLB
(same node and material sets, same bounds), which is how the refactor onto
`maplib` was checked.

Five of office's eight client spawns and six of its ten items were once inside
desks, a pillar or cubicle partitions; they were moved to the nearest clear floor.

`tests/test_spawn_clearance.rs` probes every GLB map's spawns against the collision
mesh with rays (floor below, headroom over the footprint, nothing within the
radius at chest height), and every map passes it. When it was written, five more
failed: bank's south spawns stood where there is no floor at `z = 0` (the slab is
at ~0.85), assault's RVSF spawns were on a mezzanine with 4.6 units of headroom
(moved to the ground floor below — there is nowhere on that level a body fits),
and inferno, mirage and nuke each had spawns under an overhang or on a crate.
Each was moved to the nearest clear point on its own level, `z` set to the floor
it actually stands on, both tables at once. Items are checked on office only:
elsewhere they sit in crouch-height spaces a standing probe rejects.

Note the **two spawn sources** for a GLB map: Train and the GLB render use the
tables in `world3d.ts`/`world3d.rs`, while a hosted match simulates on the cube
world built from the map's JSON brushes, with the JSON's `spawns`. The JSON's
placements are now copied from those tables, then nudged by
`tools/hassault_fit_placements.py` wherever the baked world disagrees (a spawn on
a kitchen counter, an item under a crate); the few that moved are the only
divergence left.

### What is solid on a modelled map

Solidity is decided in three places, and each was wrong in its own way.

**Which GLB nodes collide** (`glb-colliders.json`, read by `glb-colliders.ts` and
`world3d.rs`). Both clients used to skip any node whose name contained a
decoration keyword — the browser case-insensitively, native case-sensitively — so
`Perimeter` matched `rim` and every map's outer wall was walk-through in the
browser, along with Mirage's Window Room floor (`Window`), Dust II's bomb-site
crates (`Plant`), and the office's cubicle dividers (`Divide`). A keyword now
only marks a node as decoration, and decoration is skipped **only when it is too
small to matter**: it can neither be stood on (both horizontal sides ≥
`standWidth`) nor block a body (taller than the step height and wider than
`blockWidth`). `NonCol` in a name still always opts out. Name keywords match
case-sensitively and material keywords case-insensitively, in both clients; the
table's `cases` are replayed by `glb-colliders.test.ts` and
`tests/test_glb_colliders.rs`, the second of which also casts rays outward from
each map's centre so a map that can be walked out of fails. `ColOnly` nodes are
hidden, not intangible — the browser used to drop them from collision too.

Two model fixes came out of that test: the office's façade is floor-to-ceiling
*breakable* glass over a drop, so each façade now has an invisible
`Exterior_Barrier_*_ColOnly` behind it; and `maplib.add_safety_floor` puts an
invisible `Safety_Floor_ColOnly` under every map at its lowest surface, because
the bank lays one slab per room and the strip under each doorway had no floor at
all — offline, walking through the west door dropped you out of the world.

**The body that collides** (`physics-rapier.ts`, `physics_rapier.rs`). The Rapier
controller was tuned in metres — a 1.8-tall, 0.45-radius capsule — and was never
touched when the maps went to cubes, so it became a third of the avatar's size
and slipped through gaps the avatar visibly could not. Every length, speed and
acceleration is now multiplied by `UNITS_PER_METRE` (3), which keeps the tuned
feel exactly; the material-penetration model stays in metres and is handed
`thickness / U`.

**What a hosted match simulates.** The server, and the client predicting against
it, never see the GLB: they run the cube heightfield built from the JSON's
`brushes`, which for these maps was a hand-drawn open box with a few dividing
walls, so online every crate, van and building was walk-through.
`tools/blender/bake_collision.py` now rasterises the GLB's colliders (the same
`glb-colliders.json` rule) into that grid and writes the result back as merged
`room` rects, marking the JSON `"collision": "baked"`:

- 4×4 vertical rays per cell, counting solid by face orientation **per object**
  (one tree for the scene steps past coincident faces and unbalances the count),
  with a one-sided plane read as a thin slab, not as rock all the way down.
- A column holds one opening, so the bake walks from the spawns over every
  opening of every column — step, running jump, drop — and keeps the lowest
  opening that was *reached*: the street under a catwalk, never the void under a
  floor slab. An unreached column keeps its top only when it is open to the sky
  (a crate or wall top you can shoot over); anything else closes.
- Footprints are conservative (a cell is blocked if any sample is), floors round
  to the nearest cube and ceilings round down.

`maplint` holds a baked world to what these maps are built for: reachability
allows a **running jump** (`JUMP_CLIMB = 4`, measured against `physics.step` by
`test_maplint_jump_climb`), crossing the unstandable cell that hugs any ledge,
and raised islands — standable tops beside ground someone does reach — are
geometry, not a sealed room. Drawn maps keep the stricter on-foot rule.

Rebuilding a modelled map is therefore three steps, in order: the generator
(`blender --background --factory-startup --python tools/blender/generate_<map>.py`),
the bake (`... --python tools/blender/bake_collision.py -- hd_<map>`), then
`uv run python tools/hassault_fit_placements.py hd_<map>`.
`test_modelled_maps_simulate_their_model` fails if a `gltf` map ships without
baked brushes.

Fixing solidity surfaced traversal the models never had: Dust II's Long A and CT
"ramps" and Inferno's apartment stairs were boxes (cliffs), now wedges via
`maplib.add_wedge`, and Nuke's reactor hall was four unbroken walls — Site A was
unreachable by anyone — and now has three doorways (`maplib.add_wall_with_door`).
...
That capture also declares `systemKeys`, asking for `alt+tab` and `alt+f4` as well.
Asking is not getting: it needs Chromium, *document* fullscreen and the
`keymap.captureSystemKeys` setting, which is off by default — see
[system-key capture](../architecture/keybindings.mdx#system-key-capture). Pointer
lock is requested with `unadjustedMovement: true`, which is the real "raw input" of
a shooter's settings menu — movement before the OS applies pointer acceleration —
falling back to a plain lock where that is unsupported.

### One table, both clients

The native client used to hard-code every key in a `match` in `app.rs`, so the
Controls screen rebound keys for the pane only. The native window is where the game
is played by default, and it ignored every change. It now reads the same
`hassault.controls` setting at launch (`apps/native-fps/src/controls.rs`) and
dispatches through `game_key`:

- **Codes need no translation.** winit's `KeyCode` variants are named after the
  W3C `KeyboardEvent.code` values, so a key's `Debug` name *is* its code, and
  there is no translation table to drift.
- **The defaults are pinned.** A stored diff is relative to the defaults, so both
  clients must agree on them. `tests/browser_parity.rs` parses `DEFAULT_CONTROLS`
  out of the TypeScript at test time and compares it with `controls::DEFAULTS`.
- **A few keys stay fixed** because the Controls screen does not offer them: the
  console (backquote), the menu (Escape), F3/F6/F7, and the buy menu's number
  row. The arrow keys still turn the view natively, but only when nothing is
  bound to them. The shared defaults put strafing there, and a key cannot mean
  both.
- **Pane-only actions do nothing natively.** Ping, drop and noclip have no native
  implementation, so bound there they do nothing rather than something else.

The screen once listed its groups by hand as `['Movement', 'Combat', 'View']`,
which silently left out **Utility**. So the grenade, throw, buy, use and drop keys
were rebindable in code and on no screen at all. It now iterates `CONTROL_GROUPS`,
and a test fails if any action's group is missing from that list. The
**Communication** group adds:

- **Push to talk (`voice`, V).** Held, it tells your team you are on the radio
  (the `voice` indicator event). In lobby voice with the
  `hassault.voice.pushToTalk` setting on, it also opens your mic, which is
  otherwise closed. The native client has the radio indicator only, since lobby
  voice lives in the pane.
- **Chat, everyone (`chatAll`, Y) and team (`chatTeam`, U).** Noclip moved from V
  to N to make room.
...
Two flags, one per team, each on a stand. Take the enemy's, carry it to your own,
and score — **provided your own flag is home**, which is the rule that makes a
full defence worth playing rather than a race between two teams running past each
other.

A flag is a small state machine (`home` / `carried` / `dropped`) and not a
`pickups` item, because an item's whole lifecycle is "taken, then respawns" and a
flag's is not: it comes *back*, and while carried it is somewhere specific.
Running it through `Field.collect` would mean the mode reading state it does not
own out of a structure that cannot express it.

Rules that were each a bug first:

- **Taking and returning are automatic on contact**, and CTF deliberately uses
  none of `Command.use`. A player who has to press a key for a flag they just ran
  onto will not notice the flag is there.
- **A carrier who dies drops it where they fell**, on a timer before it returns
  itself. Teleporting it home instantly removes every fight over a dropped flag,
  which is most of the mode; leaving it forever lets one be parked where nobody
  goes.
- **The drop happens in `on_kill`, not in `tick`.** This one is not a style
  point: `simulate` runs `_respawn_due` *before* `mode.tick`, so a player whose
  respawn timer has elapsed is alive again — and standing somewhere else — by the
  time a poll would look. The flag would stay carried by a dead player, or land at
  their spawn. The `tick` poll remains as the backstop for a death with no killer,
  since `_fall_damage` deliberately does not come through `on_kill`.
- **A carrier who quits drops it rather than taking it with them.** Deleting it
  would make quitting-while-carrying a way to deny a flag outright.
- **`on_kill` does not call `super()`**, and its absence is load-bearing. The base
  implementation adds one to the killer's team score, which is right for
  deathmatch and wrong under a scoreboard labelled "Captures". Inherited, it
  silently mixed the two: a bot match read **9–0 off six captures and three
  kills**. Both numbers were real, which is exactly why it looked plausible — the
  only way to catch it is to assert that the score *equals* the capture count,
  which a test now does over a whole match.

Bots play it through the one `bot_goal` hook: carry it home, chase the enemy
carrier (whose position is public, so this is not knowledge a player lacks), go
and fetch your own dropped flag, or go for theirs. Every goal carries an
`expires_in`, and that is not politeness — a goal with no timeout parks a whole
team against a wall for the match, which reads as "the mode is broken" rather
than "the pathing is dumb". Verified by running bot-only matches on all three
bundled maps: six captures apiece in sixty seconds, with take / drop / return /
capture all firing.

### Bomb defuse

Rounds, a plant, a defuse and a side swap. The phase machine —
`warmup → freeze → live → post → (freeze | over)` — is a **pure function**,
`advance(state, dt, facts) -> (state, emits)`, with everything that touches
`MatchRoom` in the `tick` wrapper around it. That is not tidiness: three clients
have to agree about *when a round ends*, and the cheapest way to keep them
agreeing is for the rule to be a function with no world in it, the way
`physics-vectors.json` binds the three physics ports.

`Facts` is the whole of the world's influence on the machine — two head counts
and two flags — and `Emit` is the whole of its influence back.

Rules with a reason:

- **`HALF_AT = ROUNDS_TO_WIN - 1`, and the minus one is the point.** Set equal, a
  side takes every round of the first half and wins the match without the other
  ever having attacked, which makes the map's balance the entire result. One
  short of winning means the swap always happens. This is CS's structure and the
  same arithmetic is why.
- **The swap changes roles, not teams.** CS moves the players; here it flips one
  boolean and `attackers` is derived from it, so nobody's `team` changes, nobody
  moves spawn, and `wins` stays indexed by the people who earned it — no score to
  reverse. That is sound *only* because the bundled maps place their sites
  neutrally, equidistant from both sides. On a map with a defender-favouring
  site, keeping your spawn across the swap would mean one team attacks the hard
  site twice, and that is the moment to move players instead.
- **A plant swaps the round clock for the fuse**, which is what makes planting
  with four seconds left a winning play rather than a wasted one.
- **Wiping the attackers after a plant does not win the round.** It only means
  nobody is left to stop the defuse, and that asymmetry is the whole reason to
  plant early.
- **Time out with no bomb down is a defender win.** The attackers had a job.
- **The bomb moves to another attacker when its carrier dies**, rather than
  dropping. A dropped bomb on maps this size is a hunt, not a round. The only
  way it reaches the floor is on purpose — the drop key, below.
- **A mid-round joiner waits for the next one.** Dropping them in alive hands one
  side a body the other did not have to shoot, deciding the round on when
  somebody's browser finished loading.
- **`on_kill` does not call `super()`**, same as CTF: rounds are the score.

Freeze time turns damage off (`damage_scale` returns 0) but deliberately does
*not* freeze movement — that would need a physics hook and every mode would pay
for a branch one of them uses. Walking to your side of the map during the buy is
not an exploit.

Friendly fire is **0.35**, and this is the mode that makes `damage_scale` a float
rather than a bool: a teammate you spray through should be a real cost, and one
you cannot hurt at all makes a doorway a place to stand.

#### What a round reset must clear

`room.items.reset()`, every player respawned, `nades` and `zones` cleared — and
**`room.history.clear()`**, which is the one that hides. Every body has just
teleported, so without it a shot in the first quarter-second of the new round is
rewound against last round's positions and registers on somebody who was
somewhere else. It is silent, lasts a few ticks, and is close to impossible to
reproduce deliberately. `PositionHistory.clear` already existed; it simply had to
be called.
...
Money on `MatchPlayer` (not a mode-side dict, which leaks its entries when
`remove` fires), and **every number served** in `welcome_state().config` plus a
`catalog` — the `/weapons` and `/items` precedent. A client with its own copy of
a price is a menu that offers a purchase the server then refuses, and that fails
as money still sitting there with nothing saying why.

The catalogue is `{id, name, kind, slot, price}` rather than raw `WEAPONS`
indices, so armour and grenades ride the same integer as a rifle and
`Command.buy` stays one number. **The index into it is what goes on the wire**,
which is why a duplicate id is a tested-for error: two rows would buy the same
thing while the menu showed two.

`Command.buy` and not a `/ws` event, for the reason `throw` gives: commands are
forwarded *verbatim* by `fabric.handle_input`, so a field crosses the peer wire,
the ranked relay and the console with no changes at all. An event needs a
`channel.py` branch, a `fabric.py` send and handle pair, a new frame kind, a
guest-side normaliser and a `ranked.py` relay — five touch points, four of them
security boundaries, for something that costs nine bytes here. It is `-1` when
absent, the `weapon`/`nade` shape: a default of zero names the first entry and
would make every movement command a request to buy a rifle.

Rules with reasons:

- **A purchase that would give nothing spends nothing** — the `pickups.apply`
  shape. Armour at full armour, or a rifle you hold, must not quietly cost you
  the round's money.
- **A dead player may still buy.** They are buying for the *next* round and the
  freeze is when that is decided, so `alive` is deliberately not one of the
  checks — see the `on_command` note below, which is what makes it possible.
- **A loss pays, and pays more the longer it goes on.** Without it a side that
  loses once buys nothing, loses again, and the scoreline stops being about
  play. That is the failure an economy is most likely to introduce and the one it
  is least obvious it has. Capped at `MAX_LOSS_STREAK`, or a side that never wins
  ends richer than the one beating them.
- **Planting and defusing pay even on a losing round.** Planting and then being
  wiped is still the play that nearly worked. Paid at the objective rather than
  at round end, which also avoids having to remember who the planter was — they
  may not be alive by then.
- **Half time levels the purses**, or the first half's economy decides a match
  whose sides have just swapped.
- **A team kill charges rather than pays**, floored at zero: friendly fire here
  is partial, so a teammate in a doorway is already a cost, and this stops
  shooting through them being free. A debt would follow somebody into a round
  they had nothing to do with.

#### Dropping and selling back

Two more `Command` fields, for `buy`'s reason: `drop` (a flag) and `sell` (a
catalogue index, `-1` when absent). Both are handled in `Defuse.on_command`,
so deathmatch — where everyone already holds everything — ignores them. In
that handler **sell runs before buy**, so one frame can trade a rifle for a
sniper on the strength of the refund.

**Drop** (`X` in the browser) puts down **the bomb if you carry it, and
otherwise the gun in your hands**. The bomb comes first because handing it to a
teammate better placed to plant is what a carrier means by the key.

- Only a bought gun can be dropped, never the knife or pistol (`FREE_SLOTS`),
  and only by a living player.
- A dropped gun keeps its magazine and reserve. A drop that refilled itself
  would be a free reload for two players standing side by side.
- It rides in the shared state as `drops: [{id, slot, x, y, z}]`, **with no
  ammunition**. How full it is belongs to whoever picks it up.
- It is taken by walking over it. One reach, `_standing_on`, covers kits, guns
  and the bomb, so no two of them can be taken from different distances.
- A gun goes to anyone who doesn't already own one of that kind. It moves into
  their hands only if they were holding the knife or pistol: swapping someone off
  a rifle because they brushed past a shotgun would be a pickup that costs a
  fight.
- A dropped bomb (`bomb.state == "dropped"`, with its position) is taken only by
  an attacker. Attacker bots go and fetch it through `bot_goal`.
- **Your own drop ignores you for `DROP_REPICK_DELAY` (1s).** It lands at your
  feet, so without the delay the next tick hands it straight back and the key
  appears to do nothing.

**Sell** refunds the full price, **only in the freeze**. Pressing a row that
`you.mode.sellable` lists sells it back. The menu shows that row as
`SELL +$price` in a colour of its own, because a row that looks finished but
refunds you on a keypress is a misclick waiting to happen.

`sellable` is its own list and not derived from `bought`, because **owning is
not having paid**. `MatchPlayer.purchased` records what you bought, and two rules
keep it honest:

- **A gun that leaves your hands leaves `purchased`.** Without that, one player
  buys a rifle, drops it for a teammate, and both of them sell it back: one
  purchase, two refunds. A picked-up gun is never added to `purchased`, so it is
  owned but not refundable. `test_a_dropped_gun_cannot_be_sold_by_either_player`
  pins both halves.
- **What is sold must still be exactly what was bought**: a grenade not yet
  thrown, armour that has absorbed nothing. Refunding spent utility would make
  the freeze a window in which it is free.

The client decides buy-or-sell in `trade.ts` from the *latest* envelope at send
time, never from its own idea of what it paid for. The native client renders
`dropped` as an ordinary bomb state but does not send either field yet.

#### Two ordering bugs the economy exposed

Both were invisible until something was actually bought.

**`outfit` was never called on respawn.** It ran in `MatchRoom.add` only, so
`reset_loadout`'s full grant won at the start of every round: you paid, you
spawned, and you were holding the whole loadout anyway. There is no error
anywhere in that — the buy menu simply appears not to work. `respawn` now calls
it immediately after `reset_loadout`, and the docstring on both says why that
order is the contract.

**`mode.on_command` sat inside `if player.alive:`.** It was called from
`_handle_combat`, which is gated on being alive, so a dead player's commands
never reached the mode at all and the buy window was closed to exactly the people
using it. It now runs in the queue drain **outside** that guard: what a dead
player's command may do is the mode's decision, not the loop's. Still inside the
drain, so the command's `dt` has already been spent from the budget and a
progress bar accrued from it is bounded by the same clock movement is.

#### The buy menu, in both clients

A **held** panel like the scoreboard rather than a mode you toggle into: it is
read during the freeze while the round is being decided around you, and a menu
you can leave up by accident is one you die behind. `B` holds it; the number row
buys while it is open, which is two jobs for one row and unambiguous precisely
because the menu is held — the other hand is on it.

Three row states, and they are three because they mean three things: owned,
buyable, out of reach. Collapsing the first two gives a menu that keeps offering
what you already have. `canBuy` comes off the wire rather than being a phase
check either client repeats, and a closed window **says so** instead of hiding
the panel — hidden, a player pressing `B` during a live round cannot tell whether
the key is broken or the window is.

Two things the native render caught that no test would have: the key column and
the item name touched (`"2Shotgun"`) at a gutter sized for the digit rather than
for a column, and the scoreboard drew *over* the menu. Both are held panels, so
they rarely overlap in play — but the buy menu is the one being typed into, and a
panel you are pressing keys at must not be the one underneath. It is drawn last
now, and near-opaque, unlike the scoreboard: that one is glanced at while the game
carries on behind it, and this one is read.

#### Bots, honestly

Bots play defuse through the same `bot_goal` hook, and one thing had to move for
it to work at all: **the action key is decided outside the fight-or-roam branch**.
Planting is a commitment, and a bot that breaks off every time an enemy appears
never plants — measured, every round on the larger map timed out with the bomb
still in a carrier's hands until that moved out of the `else`.

With that fixed, bot-only matches on `hd_crossing` play the whole mode: plants,
defuses, a half-time swap and a match won. On `hd_atrium` they still do not
reach the sites — 128 cubes of ring is more than `_steer`'s local avoidance
handles, so those rounds time out. That is a pathing limit on one map rather than
a mode bug (a human plants there fine), and `expires_in` keeps a stuck bot from
parking against a wall for the match.
...
### The mode HUD, in the native client

Top-centre, which is the one region of this HUD nothing else uses — the kill feed
is top-right, health bottom-left, ammo bottom-right. It draws the score under its
own label, a round clock or a bomb fuse, which side you are on, the state of each
flag, and a bar for whatever is being held.

**Everything is drawn from what the server sent and nothing is re-derived.** The
score's label is `scoreLabel` off the wire rather than a match on the mode id, so
a mode that counts something new needs no change in the HUD at all. The fuse and
the phase clock are the server's own countdowns rather than local timers started
on an event, which is what keeps them right after a stall. The progress bar is
`you.mode.progress`, not a local one — a local timer keeps running through the
interruption that resets it server-side, through a stall, and through dying, each
of which is a bar that finishes while nothing happens.

Two collisions the unit tests could never have caught, both found by rendering
`examples/hud_preview` and looking:

- The score line and the bomb line overlapped. A 5×7 glyph is `7 * scale` tall,
  so the `8.0 * scale` line advance left one scale of gap and the two touched.
  `paint_center` already used `9.0`; now so does this.
- The progress bar's label printed straight through the bar. `center_text` places
  a glyph's **top**, so a label at `y - 3.2u` at scale `0.8u` ran from `y - 3.2u`
  down to `y + 2.4u` — across the thing it was labelling.

`hud_preview` takes `defuse` or `ctf` now, and the fixtures it draws are
deliberately the *awkward* moment in each — a planted bomb with four seconds left
while you are defusing it, a CTF with our flag taken and theirs dropped. A HUD
checked against the calm case passes with a layout that collides the moment two
things are true at once.

Objective events all share one banner rather than getting a treatment each: what
a player needs from "bomb planted at A" and "they took our flag" is the same
short sentence, briefly, where they are already looking. It sits **above** the
crosshair because the kill notice sits below it, and both landing in the same
second is the ordinary case — planting while somebody shoots you off the site.
`Fx::RoundLive` deliberately produces no banner: the phase clock already says
LIVE, and a banner that only repeats a readout is one people learn to ignore,
including on the events that matter.

A `Fx::Half` **clears the kill feed**. Its entries are coloured by whether they
were ours, and after the sides change every one from before is coloured for a
side that player is no longer on — which reads as the feed having got the kills
wrong rather than as the colours meaning something else now.

#### The same layer in the browser pane

`panels/ModeHud.tsx`, in the same place on screen and saying the same things, so
the two clients read alike. It draws from the wire the same way — `scoreLabel`
verbatim, the server's own countdowns, `you.mode.progress` rather than a local
timer — and the reasons are the same ones written up for the native client above.

One structural difference worth knowing: this one stacks in **normal document
flow**, where the native HUD advances a `y` by hand. The two collisions the
native version shipped with — a line advance one scale too tight, and a label
placed by its top across the bar it labelled — cannot happen here by
construction. That is not an argument that this file needs less care; it is the
reason it needed less of the *same* care.

`objectiveNote` lives in `session.ts` beside the class that consumes it rather
than in the component, because `absorb` uses it as the **test** as well as the
phrasing: returning `null` for anything that is not an objective event is what
lets one function decide both. A second list of the same fourteen kinds is
exactly the pair that drifts, and a kind with no phrasing renders no banner at
all — silent, and only on the event nobody happened to try.

`__tests__/mode-wire.test.ts` reads **the same captured fixture the Rust test
does**, `apps/native-fps/tests/mode-wire.json`. One artefact, two readers: it is
the only thing that can catch the two clients drifting from each other *and* from
the server at once.

#### The kill feed is a structured record, not a sentence

`panels/KillFeed.tsx` draws the browser pane's feed, and the thing worth knowing is
the shape of what it draws. `KillNote` (`session.ts`) carries `killerName` /
`assisterName` / `victimName` with a `KillFeedTeam` (`CT` | `T` | `FFA`) *each*,
the `weaponId`, and seven independent modifier flags — `isHeadshot`,
`isWallbang`, `isThroughSmoke`, `isNoScope`, `isAirborne`, `isBlind`,
`isNutshot` — plus `isLocalPlayerInvolved`.

It also still carries `text`. Both matter: the flags are what let the component
render a weapon badge and per-modifier icons and colour each name by *that
player's* team, and the pre-rendered string is what anything without a renderer
(a log line, an agent tool, the native HUD's simpler feed) can still read. A feed
that shipped only the sentence could not colour the two names differently; one
that shipped only the flags would force every consumer to learn the grammar of a
kill.

The split between what the server decides and what the client does is the usual
one, and it runs straight through this record. `isThroughSmoke` is **server-side**
— `match.py` resolves whether the shot crossed a smoke volume and puts it on the
wire as `fx.smoke`; the client only reads it. Letting a client decide that would
make a smoke worth exactly as much as the renderer that drew it.
`isLocalPlayerInvolved`, by contrast, is derived **client-side** in
`session.absorb` by comparing `fx.killer` / `fx.victim` / `fx.assister` against
the local `playerId` — which is right, because "am *I* in this line" is the one
question whose answer differs per recipient and needs no authority to settle.
`mine` is the same boolean under its older name, kept because the native HUD and
the older feed both read it.

#### Two bugs the live API found that the tests did not

Worth recording because both were invisible to everything else:

- **`catalog()` reported capture the flag and defuse as free-for-alls.** `teams`
  was declared only inside each mode's `welcome_state`, so
  `getattr(mode, "teams", False)` found nothing on them. The welcome said one
  thing and the catalog another, both plausibly, and the only place it showed was
  a lobby offering one scoreboard for a game with two sides. `teams` is now a
  `GameMode` class attribute and a test asserts the catalog and the welcome
  agree, per mode.
- **`use: ['KeyE']` collided with a controls test's example key.** Rebinding
  steals a key, so `setBinding(defaults, 'jump', 0, 'KeyE')` began producing a
  diff for `use` as well. The behaviour is right and the round-trip case covers
  it; the case about *storing only what differs* now uses a key nothing is bound
  to, so it measures one thing again.

#### The version probe, and why `Extra` could not cover it

`divergence` reports unknown event names and unknown fx kinds. It reports nothing
at all about an unknown key **inside** the mode blob, because the blob is a field
this build *does* declare and `#[serde(default)]` swallows the rest without a
word. An old client would join a capture-the-flag match, render no flags, score
nothing, and say nothing about why.

So the server stamps `v` into every mode blob and `report_welcome` compares it
against `SUPPORTED_MODE_V`, once. That is the only thing there is to compare —
which is also why `tests/mode_wire.rs` asserts the stamp is actually on the wire.

#### `tests/mode-wire.json`

A welcome and a snapshot **captured out of a live defuse room** mid-round with
the bomb planted, not written by hand. Every other test of these types builds the
struct it then asserts on, so all of them would pass just as happily against a
wire shape the server has never sent.

This is the one seam where the two sides can disagree in silence: `serde(default)`
is on every field — it has to be, so an older server does not break a newer client
— and the cost is that a renamed key, a `camelCase` slip or a struct nested one
level differently all deserialize to a default with no error. The symptom is a HUD
drawing zeros over a round that is really happening.

Regenerate it by driving a room to a plant and dumping
`state_payload()` + `snapshot_message(...)` as `{"welcome": …, "snapshot": …}`.

#### Joining a mode

`--game=dm|tdm|ctf|defuse` on the client's command line, carried in the join.
Spelled `--game` and not `--mode` because `--mode` already means how the *client*
was launched (train, host, join, edit, ranked) — two different things that both
want the word.

The server **ignores it when a room id is given**, and that is the right way
round: a room id names a match already in progress, and what mode it is running
is the room's answer rather than the joiner's. An invite is an invite to a game,
not a request to change it. Empty asks for the server's default, so a launcher
that knows nothing about modes keeps working.
...
#### A panel that is read may not be translucent

The native card fills its body with its own **opaque** `CARD_BG`, not the shared
`menu::PANEL_BG`, which is `0.96`. Four percent of a bright glyph sounds like
nothing. It is not: the killfeed, the multi-kill banner, the floating damage
numbers and a held scoreboard all sit behind that card, and every one of them
came through it at **7–12% Weber contrast** against the panel. Brightness is not
what gives it away — *structure* is, and the eye finds readable words at a
contrast where it would never notice a shade.

Its own constant rather than a bump to `PANEL_BG`, which is shared with the pause
menu and every HUD panel that never asked for this; the buy menu already sets its
own alpha, so a panel choosing one is the established shape here. The rule both
follow: **a panel that is glanced at may be translucent, a panel that is read may
not.** The scoreboard is looked at during a lull with the game carrying on behind
it; these are panels somebody stops to read.

Alongside it, **a modal suppresses the scoreboard rather than covering it**
(`app.rs`, where the `HudView` is built). Two reasons, and the first is the one
opacity cannot fix: the scoreboard is *wider than the card*, so its edges stick
out past both sides where only the scrim attenuates them. The second is that it
makes the claim already written beside the summary's draw call — "the card is the
only thing on screen once it is up" — true rather than aspirational, and it hands
back the largest single block of overlay vertices (34.2k → 26.6k, a fifth of the
budget) at the one moment nothing can read them.

Both are pinned by `nothing_behind_the_card_can_be_read_through_it`, which asks
the question geometrically — is every sampled point of the card body covered by an
opaque triangle? — rather than asserting the constant back at itself. It samples
the corners as well as the middle, because a fill that covered the centre and not
the corners would pass a single-point check and still leak a killfeed into the
card's bottom-left. It also asserts something translucent is *still* drawn, since
an overlay opaque out to the window edge would not be a fix but a different bug.
...
`result_for` makes about `won`), and at least one kill, death, point of damage, or
**objective**.
...
#### The fourth term, and why three were not enough

Kills, deaths and damage all describe a **fight**, and in defuse the fight is not
the game. A player who planted the bomb twice, was traded for both times by
somebody else's shot and never landed one of their own has zero of each — and used
to file as "not a match": no row, no card, no XP, for the player who decided two
rounds. The mode most likely to produce that player was the one the predicate had
never been updated for.

So `objectives` — bombs planted or defused, flags captured — is the fourth term.
Three things about it are deliberate:

- **It rides in `you.objectives`, beside `kills`, not inside `you.mode`.** All
  three modes replace the mode blob wholesale, so a counter living in there
  depends on each of them remembering to pass it on, and the one that forgot
  would silently file a defuse specialist as having done nothing.
- **`opponents > 0` still gates it.** Standing on a site until the bar fills is
  something you can do in an empty room. An objective is evidence that something
  happened, never evidence that there was anybody to have played against.
- **It is the *opposite* arrangement from `damageDealt`**, and the contrast is the
  rule. The native client cannot reproduce damage, so it does without it; it
  cannot derive an objective either — `bomb_planted` and `bomb_defused` carry no
  `by` — so it is **served** the number instead. A figure the client can count
  exactly, it counts; one it cannot, it is told or it goes without. Nothing is
  approximated. That also means the objective term needs no care in the shared
  fixture: unlike `hits` versus `damageDealt`, the two sides read the same number.

#### What a row records, and what cannot be recovered later

Two columns arrived with the modes, both through `_ensure_column` because
`CREATE TABLE IF NOT EXISTS` never adds one to a table that already exists:

- **`mode`**, backfilling to `dm`. Not a placeholder — before modes existed a
  room could only be deathmatch, so that is what those rows genuinely were.
  Without the column a 5–3 in rounds and a 5–3 in kills are the same two numbers
  and nothing else in the row tells them apart, which is unrecoverable after the
  fact rather than merely missing.
- **`objectives`**, so the card can show what earned the XP rather than only the
  total.

XP gained two terms with them. **`XP_PER_OBJECTIVE` is worth six kills** — the
same argument `XP_PER_100_DAMAGE` makes, taken to the mode that needs it most:
paying the player who planted under fire by their frag count describes a different
game than the one the scoreline showed. (`outcome_for` weights an objective at
three kills for MVP; XP weights it higher because MVP is a comparison *within* a
team and this is not.) **`XP_ROUND_WIN` is deliberately small** — winning already
pays `XP_WIN`, and a large per-round term would make a 5–4 grind worth more than a
5–0, rewarding length rather than play. It is asked of `mode.rounds_won`, never
read off `room.scores`, because `scores` counts kills in deathmatch and reading it
directly would pay a round bonus once per frag.

Both cards show it. The native card **substitutes** the objectives figure for
`OPPONENTS` rather than adding a seventh cell — the grid is two rows of three and a
third row of one would sit exactly where the leave button is, and `OPPONENTS` is
the cell it replaces because that figure exists to explain **NO CONTEST**, which a
card showing objectives is by construction not. The dashboard card grows to five
columns instead, and labels the number per mode (`Captures`, `Plants/Defuses`),
because three captures and three defuses are not the same achievement.

#### Ranked stays deathmatch, and the absence is enforced

`HassaultReferee.join` takes **no `mode` parameter**, and that is the decision
rather than a leftover. A rated round-based mode is a different project, not a
flag: defuse is scored by *round*, a round is settled between two full sides, and
a server-hosted room people join and leave one at a time has no such thing to
report. The referee would have to hold a side together for a whole match, decide
what a leaver does to the round in progress, and report a per-round result the
store has no shape for — none of which becomes true by accepting a string.

Leaving the parameter out is what makes adding it a decision somebody has to make
on purpose, instead of discovering the problem from a ladder full of half-played
rounds. `mode` is written into the ruleset all the same, so the day it stops being
one value the rows already say which.
...
Inviting does not require that you already be hosting. "Invite Rob" is a complete
intent, so when you are not hosting a match, `inviteFriend` invites them into your
**lobby** (below) and opens one if you have none. It used to host a match and
then invite, which failed twice over: it dropped the inviter into a live map
alone, and the invite never left the node. `session.invite` refuses unless the
status is `joined`, and it was called while still `joining`, so every invite
from the menu was silently discarded. `lobby_invite` opens the lobby and sends
the invite **in one message**, so there is no second step to race. Invites
carry a `kind` (`lobby` | `match`). An older sender sends none, and its invites
read as `match`.

### Lobbies

A lobby (`lobby.py`) is the room in front of a match: the host picks the map,
friends join, chat and mark themselves ready, and **Start** opens a match and
sends every member into it at once. The lobby **outlives the match**. Leaving
the match returns you to the lobby with the same people, and pressing Start
again while that match still runs sends latecomers into the same game rather
than splitting the party. It follows karaoke's shape:

- **Process-global on the host's node.** Every change pushes the whole
  `LobbyState`, stamped with a `revision`, so the pane drops out-of-order
  pushes.
- **Members are renderers.** A member is a local socket or a `PeerLobbyConn`,
  which is `PeerPlayerConn`'s twin (`hassault_lobby` guest to host, carrying an
  `op`; `hassault_lobby_frame` host to guest). `LobbyServer` only calls
  `send_json`.
- **The host leaving closes it.** It is not handed over, because the match it
  would start is simulated on the host's node.
- **Changing the map clears every ready flag.** A ready given for one map is
  not consent to another.
- **Start creates the room empty.** Each member joins it themselves on
  `lobby_start`, which is the only join path. An empty room survives
  `EMPTY_GRACE`. The guest's backend stamps the host node id onto `lobby` and
  `lobby_start`, which is how a remote member's pane knows where to join.
- **Trust gates every inbound peer op**, the same as a match join. Remote
  names are tagged `name@node`.

#### Lobby voice

Voice (`lobby-voice.ts`) is a **mesh of browser-to-browser `RTCPeerConnection`s**:
one audio link per pair of people in voice. The audio rides DTLS-SRTP directly
between the browsers, the same shape as `share/rtc.ts`, and never touches a
backend. The lobby is only the postman:

- **Signalling.** `lobby_signal` carries SDP and ICE to one named member. It
  travels over the fabric as the `signal` op, like chat does. The host stamps
  `from` with the member the socket belongs to, never with what the frame
  claims.
- **Frame limits.** Only `offer`/`answer`/`ice`/`bye` frames are passed, each
  under 32 KB. A handshake is not a state change, so it does not bump
  `revision`.
- **Who is in voice.** Each member carries `voice` and `muted` (`lobby_voice`),
  which is how every pane knows whom to connect to.

Four rules that fail silently if broken:

- **Exactly one side of each pair offers:** the smaller member id. If both
  offered at once (glare), each would reject the other's offer and nobody would
  connect. An offer from the wrong direction is ignored.
- **ICE candidates are queued until the remote description is set.** Adding one
  earlier throws, and dropping it can cost the only route that works.
- **Playback goes through the mixer's `lobby-voice` strip.** It also needs a
  muted `<audio>` element on the stream, because Chrome feeds a remote WebRTC
  track into Web Audio only while a media element is consuming it. Without
  that, the voice is silent.
- **Speaking is metered on a 100 ms timer, not rAF.** rAF stops in a background
  tab, and a lobby is the thing you leave in the background.
- **On Linux the desktop shell has to answer the permission request itself.**
  WebKitGTK sends each `getUserMedia` call to the embedder, and denies any
  request nobody answers. wry does not answer, so the voice bar showed
  "Microphone permission was denied" on Ubuntu while Windows worked.
  `apps/desktop/src-tauri/src/media.rs` grants microphone and device-list
  requests. It does so only on the app's own windows (`main`, `ws-*`), never on
  the embedded browser's webviews.

By default the mic is open while you are in voice, with Mute and Deafen. With
`hassault.voice.pushToTalk` on (a checkbox in the voice bar, or Settings) it is
closed except while the Push to talk key is held. That works in a match, where the
game owns the keyboard, but not in the menu, where the keyboard is the shell's.
Push-to-talk toggles `track.enabled` rather than renegotiating, so each press is
free and cannot drop a link. A window blur releases it, because a key released
elsewhere never sends its key-up. The pane hangs up when it unmounts, since it is the only path handshakes arrive on. A
mesh is right for a party and wrong for sixteen people, which is a known
limit rather than a bug. ICE uses the `network.stunServer`/`network.turn*`
settings, with the same semantics as `share`. Two networks with symmetric NAT
need TURN.

### Match chat

One chat box holds everyone's messages (`ALL`) and your team's (`TEAM`). Every
message goes through **`chat.post`** (`backend/modules/hassault/chat.py`), whether
it comes from a browser on this node, a friend's node over the fabric
(`hassault_chat`, a message type of its own because the rules are not the input
rules), or the native client. So no path can be the lax one. A remote player's
chat used to be dropped outright, and so did chat in a **ranked** match, whose
room is on the game server rather than the node. The node now relays it
(`ranked.relay_chat`) and the game server runs the same `chat.post` against the
referee's rooms (`HassaultReferee.chat`, which passes `server=`). The echo comes
back through the ranked pump like any other event. The server's copy is the only
copy:

- **Stamped and echoed.** Each line gets an `id` and a server `ts` and is echoed
  to its sender too. Both clients render only that echo and drop a repeated
  `id`. The native client used to echo locally *and* receive the server's copy,
  so every message it sent appeared twice.
- **Grapheme limits.** A message is at most 200 grapheme clusters and 1024 UTF-8
  bytes, cut only between clusters (`regex`'s `\X`, UAX #29). `text[:200]` used
  to split a ZWJ family or a flag into garbage. The byte ceiling exists because
  one cluster can be arbitrarily long ("Zalgo" text).
- **Normalisation.** Text is NFC-normalised, and newlines and runs of
  whitespace collapse to one space.
- **Format characters.** Bidi overrides and isolates (U+202A–202E, U+2066–2069)
  are removed, since in a chat box they reorder somebody else's name. ZWJ,
  ZWNJ, variation selectors and tag characters stay, because emoji sequences,
  Persian and subdivision flags are built from them.
- **Rate limit.** Five messages per four seconds per player. A refusal comes
  back as `chat_refused`, and the box says "not sent — slow down".

**The pane** (`ChatBox.tsx`, `chat-text.ts`):

- **Counts graphemes** with `Intl.Segmenter` rather than `maxLength`, which
  counts UTF-16 units and would cut an emoji in half.
- **Ignores the IME's Enter.** An Enter that arrives with `isComposing` set is
  choosing a CJK candidate, not sending.
- **Isolates names and messages** in `<bdi>`.
- **Names every platform's colour emoji font** in its font stack.

While the box is open it owns the keyboard, ahead of even the console's
backquote. Tab switches between everyone and team, and a line fades nine
seconds after it arrives.

**The native client** draws chat in real fonts (`textlayer.rs` and
`textquad.rs`). The HUD's 5×7 bitmap font turned every emoji and non-Latin
character into blank space. Chat is laid out with `cosmic-text` and rasterised
with `swash`, which draws COLR and bitmap emoji in colour, using the **OS's own
fonts**: Segoe UI and Segoe UI Emoji on Windows, Apple Color Emoji on macOS, Noto
on Linux. Nothing is bundled.

- **Fonts load on a thread.** Discovery takes a few hundred milliseconds, and the
  bitmap font covers chat until it lands.
- **Each line is its own buffer, faded while compositing.** `swash` ignores the
  alpha of a glyph's colour, so one old line could not fade inside a shared
  layout.
- **Re-rasterised only on change.** The panel is re-rasterised only when the
  chat's `revision` or a line's eight-step fade changes, then uploaded to one
  sRGB texture and drawn as one pixel-snapped quad after the HUD, but never over
  the menu or the console.
- **Grapheme editing.** Caret, backspace and delete step by grapheme cluster
  (`unicode-segmentation`), so one backspace removes a whole family emoji, and
  the prompt enforces the server's limits.
- **IME input.** Input-method text arrives as `Ime::Commit`, which is the only
  way the Windows emoji panel (Win+.) and CJK input reach a winit window. IME is
  enabled exactly while the prompt is open, and composition text is drawn at the
  caret.

Two platform limits, not bugs: Windows' emoji font has no flag glyphs, so 🇺🇸
reads as "us" (Chrome on Windows does the same), and a machine with no colour
emoji font shows the text font's monochrome glyph. `textlayer`'s tests rasterise
with the real system fonts and assert that an emoji comes out *in colour* and that
Japanese, Cyrillic and Arabic produce ink. The colour check is skipped on a
machine with no colour emoji font.
...
### Surface material: what a mark looks and sounds like

A decal used to be one colour regardless of what it landed on, and a bullet
punching a wall made the same sound as one punching a doorframe. `decals.ts`'s
`surfaceMaterial()` reads the hit cell's texture id off the **served** face
arrays (`wtex`/`ftex`/`ctex` — the map's own `world.ts` data, not a guess) and
buckets it into `concrete` / `metal` / `wood` / `glass` / `default` by a fixed
id table — the same limit the world renderer's own texture-id tinting is under
("No textures yet", further below): map `.cfg` files, which would name a
texture, are not parsed yet, so a material guess is only as good as the colour
guess the surface itself is drawn with, and both wait on the same missing
piece. That classification now feeds three things off one lookup, rather than
three separate guesses that could disagree:

- **`decals.mark`** tints the mark by material (`MATERIAL_TINTS`) instead of a
  fixed white, so a wall of pocked concrete and a shredded vent read differently
  at a glance.
- **`effects.impactSpatter`** spawns a short-lived, material-shaped particle
  burst — ricochet sparks off metal, splinters off wood, shards off glass, dust
  everywhere else — oriented off the served face normal, using the same fixed
  particle pool (`MAX_LIVE`) every other transient effect shares rather than a
  budget of its own.
- **`audio.surfaceImpact`** plays a per-material timbre (`impact_metal`,
  `impact_wood`, `impact_glass`, `impact_concrete`) positioned the same way
  every other sound in the mixer is — bearing plus listener yaw, never a raw
  stereo pan — so a shot's material is audible before the decal is in view.

A **wallbang kill** additionally plays `impact_wallbang`, a distinct low thump
gated on `hit.wallbang` in the snapshot (`match.py` already resolved this per
hit for the kill feed's `isWallbang` flag; this is a second consumer of the
same server-decided fact, not a new one) — audible confirmation that a shot
landed through geometry, which a normal hit marker doesn't distinguish.

This is **browser-only** for now, the same status the smoke wisp carries: the
native client keeps its fixed decal tint and has no particle burst or
per-material audio. `browser_parity.rs` pins what the native side *does* draw
(cap, lifetime, fade, lift, size, face-normal order); it does not yet cover
material, because there is no native material to compare against.

### Bullet smoke channels

A round fired through a smoke cloud used to behave exactly like one fired
through open air on the way in and then vanish into the cloud's opacity on the
way out — the smoke didn't know it had been shot. CS2's mechanic is that a
bullet punches a short-lived visibility tunnel through smoke it passes through,
and it is implemented the same way every other simulation fact here is: **on
the server**, because whether a sightline through smoke is currently open
decides whether a peek is safe, and a client computing that itself could decide
it differently for the shooter and the target.

`grenades.Zone` gained a `channels` list: `(x, y, z, radius, expires_at)`
tuples appended by `match.py` whenever a non-knife shot's endpoint ray crosses
a live smoke zone's sphere, capped at 8 per zone (oldest evicted) and pruned
every tick by `expires_at`. Two things read the list:

- **`sight_blocked_by`**, reached through `MatchRoom.smoked()` — the one call
  both a bot's line-of-sight check and the radar's "is this cloud in the way"
  logic go through, so a cloud is respected the same way by a human's eyes and
  a bot's — now walks a target ray against each active channel first and
  treats a ray passing through one as unblocked, the same way it already
  treats a ray that misses the zone's sphere entirely.
- **`Zone.snapshot()`** serialises the active channels as `[x, y, z,
  remainingSec]`. Internally a channel stores an *absolute* expiry
  (`time.monotonic()` at append time, plus its 0.8 s life), because that is
  what a tick-by-tick prune compares against — `snapshot()` converts it to a
  duration (`expires_at − now`) on the way out, since a raw expiry timestamp
  means nothing to a client that doesn't share the server's monotonic clock.
  The id and radius are fixed constants the client already knows, so the wire
  carries only what varies.

The client mirrors this rather than re-deciding it. `net.ts`'s `ZoneRow`
carries the same `channels` field, and `NadePool.punchSmoke()` (`nades.ts`) —
called from `HorribleAssaultPanel.tsx` on every shot's impact endpoints,
alongside the existing decal/spatter/audio calls — both reproduces a channel
locally on the shooter's own screen (so the hole appears with zero added
latency for the person who made it) and reconciles against the server's list
on the next snapshot the same way `NadePool.sync` reconciles every other zone
field. A channel is stored as a point on the **unit sphere local to the
zone** (`(hit − zoneCenter) / radius`), not a world position, because the
cloud's own bloom/fade animates its radius continuously and a fixed-radius
hole would drift off the surface as the cloud grows or shrinks.

Visually, a channel is a soft-edged void carved into the volumetric shader's
density function (`uChannels[8]`, a `vec4` per hole: local xyz + fade-scaled
radius) rather than a hole in the mesh — the smoke shader already computes a
scalar density per fragment for its noise-driven bloom, so punching a channel
is one more term multiplied in, not a second draw call. `NadePool.smokeDensityAt`
— the same query the camera's own smoke-blindness shroud reads when the player
is standing inside a cloud — subtracts the same channel list, so a channel
punched near your own position also thins the first-person whiteout, not only
the view from outside the cloud.

`backend/tests/test_hassault_smoke_channels.py` pins the channel lifecycle
(creation, expiry, the 8-channel cap) and `sight_blocked_by` respecting an
active channel; `decals.test.ts` pins `surfaceMaterial`'s texture-id buckets.
...
## Two level formats: `cube` and `gltf`

`MapInfo.format` (`backend/modules/hassault/models.py`) is `"cube"` or `"gltf"`,
and it decides which of two entirely separate world and physics paths a match
runs on.

- **`cube`** is everything above: a Cube 1 2.5D heightfield read out of a `.cgz`
  or built from our JSON brushes, walked by the ported AssaultCube physics.
- **`gltf`** is a full 3D mesh with collision hulls. `hd_facility` and
  `hd_junkflea` are the first two, and the pane fetches their geometry from
  `GET /api/hassault/maps/{name}/mesh` — a route that answers `null` for a cube
  map rather than existing conditionally.

A map declares its own format: both new maps carry `"format": "gltf"` at the top
level of their JSON, and `routes.py` reads it. There is also a hardcoded
`name in ("hd_facility", "hd_junkflea")` check immediately before that read, which
is redundant with the declaration and reachable only for those two names — a third
`gltf` map needs the declaration and nothing else. Worth knowing before adding
one, because the shape of that line invites you to append to it.

### The heightfield physics does not run on a `gltf` map

This is the part that is easy to read backwards. The three ported
implementations — `physics.py`, `player.ts`, `physics.rs` — and the
`physics-vectors.json` fixture that binds them all describe the **cube** path.
A `gltf` map runs **Rapier3D** instead: `physics-rapier.ts` in the browser
(`@dimforge/rapier3d-compat`, wasm) and `physics_rapier.rs` natively
(`rapier3d`). `HorribleAssaultPanel` constructs a `RapierPhysicsWorld` only inside
its `if (is3D)` branch, so on a cube map the wasm is never even initialised.

What Rapier buys is what a capsule-against-trimesh controller gives you for free
and the heightfield port cannot: continuous collision against arbitrary geometry,
slope climbing with automatic sliding past the limit, stair stepping, and a
collision capsule that shrinks with a crouch.

**What it does not have is the fixture.** `physics-vectors.json` covers neither
Rapier port; `conformance.test.ts` and `conformance.rs` mention Rapier nowhere.
So the duplication argument made in
[Three implementations, one fixture](#three-implementations-one-fixture) — that
two ports of one rule set are only safe while a shared fixture binds them —
currently holds for the cube path and **not** for the `gltf` one, where the same
rules exist twice with nothing replaying them against each other.
`physics-rapier.test.ts` pins the browser side's own behaviour (slopes, steps,
crouch, momentum) but says nothing about whether Rust agrees. That is a real gap,
...
The browser's smoke is volumetric rather than a billboard — `nades.ts` shades the
puff with self-shadowing and highlights so it reads as a body of smoke you are
looking *into*, which is the difference between a smoke that hides a lane and a
sprite that sits in front of it. It is a rendering choice only: what a smoke
*does* to a shot is resolved server-side, and `isThroughSmoke` on a kill comes
from that resolution, never from what either client drew.
...
## The workspace: three windows and their strips

The game has **three windows**, one per job: **HorribleAssault** (`hassault.play`)
to play, **hAssault Dev Tools** (`hassault.studio`) to build for it, and
**hAssault Server** (`hassault.server`) to run rooms on this node. It used to have
ten — the studio three more times on a different tab, an armory pane duplicating
the main menu's armory section, and four narrow companions each claiming a dock of
their own — and a launcher offering ten ways into one game told nobody which one
they wanted. The companions (match stats, radar, voice, console) are now **region
strips** on the window they accompany (`embedded: true`), so they open at its edge
rather than somewhere else in the frame. Old ids still resolve: `RENAMED_VIEWS`
migrates saved layouts and `VIEW_ALIASES` keeps the agent's vocabulary.

Everything *around* the game still talks across panes and strips —
`workspace-bus.ts` is the channel between them, and it exists because the obvious
implementation does not survive contact with the frame engine.

**The target pane is usually not open yet.** Clicking "inspect in the studio" from
the armory has to open the studio, and the studio mounts a tick later — so an
event broadcast at click time is delivered to nobody. Worse, only an area's active
tab renders, so a pane that *is* open may still be unmounted while you look at
another tab. A plain listener registry loses the request in both cases, silently.

So three of the five channels **park** their request and the consumer drains it on
mount:

| Channel | Parks? | Producer → consumer |
| --- | --- | --- |
| `requestMapLoad` | ✅ `takePendingMapLoad` | studio / console / armory / menu / companion → the play pane |
| `requestMapEditorInspect` | ✅ `takePendingMapEditorInspect` | play / console / menu / companion → the studio |
| `requestWeaponInspect` | ✅ `takePendingWeaponInspect` | armory / play / studio / console → the studio |
| `broadcastWeaponEquipped` | ❌ | armory / studio / console → whoever is listening |
| `dispatchConsoleCommand` | ❌ | console / panel / UI → the play pane |

`HorribleAssaultPanel` shows the pattern exactly: its effect calls
`takePendingMapLoad()` **first**, applies whatever was parked, and only then
subscribes with `onMapLoadRequested`. Drain-then-subscribe, not the reverse — the
reverse leaves the parked intent sitting there until something unrelated fires.

The two that do not park are the ones where a missed event is genuinely nothing:
an equip is a cosmetic broadcast, and a console command aimed at a match that is
not running has nowhere to land anyway.

**Telemetry is a `latest`, not a queue.** `broadcastMatchTelemetry` overwrites, and
`getLatestMatchTelemetry()` reads the current value — because a radar or companion
pane opened mid-match wants the state of the match *now*, and replaying the
history of a firefight to a panel that just appeared would be worse than useless.
`MatchTelemetry` carries the phase, map, roster, spotted list, round time, score,
kill events and the local player's vitals in one object, so a consumer re-renders
from a single snapshot rather than assembling one out of five subscriptions.

**Every request carries a `source`.** `'studio' | 'console' | 'armory' | 'menu' |
'companion'` and friends are not decoration: a pane that both produces and
consumes a channel — the studio does, for weapon inspect — needs to ignore its own
echo, the same problem the [code locus](../modules/editor.mdx) solves the same way.

The bus is **module-global**, not React state or context. A pane's state does not
survive its unmount, and the whole point here is to outlive one.
`resetWorkspaceBusForTesting()` clears every parked intent and listener, which is
what lets `workspace-bus.test.ts` run each channel in isolation.

### The two workspaces

The windows are arranged by `frames:` presets rather than left to the user to
assemble, since the useful combinations are few and specific:

| Preset | Layout |
| --- | --- |
| `hassault_dev` | The game beside the dev tools (55/45). Console and radar are strips on either. |
| `hassault_play` | Just the game; its strips hold the match companion, radar, voice and console. |

`hassault_mapmaker` and `hassault_armory_studio` were retired with the panes they
arranged: the first was `hassault_dev` at different proportions, and the second
paired a pane that no longer exists with the studio.

Each carries a `description`, which is not optional decoration:
`presets-described.test.ts` fails the build without one, because a preset with no
description renders as a bare name in the home launcher and the welcome widget —
the two places a user is offered a way of working.

## Contributions to the layout shell

- **Panels:**

  | Id | Role | Singleton | What |
  | --- | --- | --- | --- |
  | `hassault.play` | document | ✅ | The game. `fullscreen: true`. Strips: companion, radar, voice (right), console (bottom). |
  | `hassault.studio` | document | ✅ | Dev tools: asset viewer, map/scene, materials, animator, rig editor as its own tabs; `params.tab` selects one. Strips: console, radar. |
  | `hassault.server` | document | ✅ | Host rooms, field bots, invite friends, watch the roster — without playing. |
  | `hassault.console` | tool, embedded | — | The developer console. |
  | `hassault.companion` | tool, embedded | — | Telemetry, vitals, team scoreboard. |
  | `hassault.radar` | tool, embedded | — | 2D tactical minimap and callouts. |
  | `hassault.voice` | tool, embedded | — | Voice comms. |

  `hassault.play` is the only `fullscreen: true`. It is a `document` because it
  wants the centre area and a lot of pixels, and it grabs pointer lock, which
  would be hostile in a narrow dock. The `fullscreen` opt-in earns it the `⛶`
  titlebar control and Escape-able presentation over the taskbar and the
  workspace strip — and on the desktop build that escalates to OS-window
  fullscreen, which is what makes pointer lock and the full field of view behave
  the way the game assumes. See
  [desktop-shell.mdx](../architecture/desktop-shell.mdx#presenting-a-pane).

  All three windows are singletons: the studio's tabs are one window's state, so
  opening "the rig editor" switches the open studio's tab rather than making a
  second studio. `openPanel` on an embedded strip id (`hassault.radar`) reveals
  it in its host rather than opening it bare — the same resolution `show` makes.

  **The Server window** is the dedicated-server view of the rooms the main menu
  can also host. It holds no socket: every verb is a REST call the agent tools
  already make — `POST /matches` (host), `GET /matches/{room}` (roster),
  `POST`/`DELETE /matches/{room}/bots`, `POST /matches/{room}/invite` — so the
  window and the agent cannot describe one room two ways. "Join in game" opens
  `hassault.play` and parks the join with `requestJoin`, the invite toast's path.
- **Workspace presets:** `hassault_dev`, `hassault_play` — see
  [The two workspaces](#the-two-workspaces).
- **Commands:** `hassault.open`, `hassault.openServer`, `hassault.openStudio`; the
  retired openers (`openModelViewer`/`openModelEditor`/`openAnimEditor`,
  `openConsole`/`openCompanion`/`openRadar`/`openVoice`) now land on the studio tab
  or strip that replaced each pane.
...
`apps/assault-web` runs the in-browser client **without a node**: a guest with a
callsign plays on the central game server's `/hassault-ws`, and every REST call
goes to that server too (`VITE_BACKEND_URL`). What the server has to serve for
that, and the limits that make it safe to leave public, are in
[deploying the game server](../architecture/game-server-hosting.mdx#the-standalone-web-client).

On the client, `HorribleAssaultPanel` takes a **`standalone`** prop, and it is one
switch rather than a guest check scattered through the pane:

- **Skipped:** the node account (`/session` — identity is the guest callsign),
  the armoury (`/skins/inventory`), the friends roster (`/invitees`), ranked maps,
  the native launcher's status, and the process/debrief poll — which otherwise
  404s twice every 1.5 s for as long as the tab is open.
- **Joins on load.** The page's own buttons already chose to play; the node's main
  menu (ranked, the designer, friends) offers nothing a guest can use. No room id
  means "any room on this map with space", the server's `find_or_create`.

Two fixes the web client surfaced apply to every client. The **map list no longer
overrides a chosen map**: it used to set `hd_assault` unconditionally when it
arrived, racing a join, so a pane could render one map while the server simulated
another. And the **console's REST calls go through `apiUrl`**, so they reach the
backend from the desktop build and the web client instead of the page's own origin.

The web build ships only the game's files from `apps/web/public` (`hassault-*.glb`,
`hd_*.glb`, the icon) — the dashboard's avatar and animation files stay behind —
and fails if it finds no models, since a build without them renders procedural
fallbacks and no weapons without any error.

### Replays are bounded

A `ReplayRecorder` keeps at most the last **ten minutes** of ticks (`MAX_FRAMES`,
a deque): it used to hold every tick of the room's life in memory, ~1 GB an hour
for a full room. Recordings live under `$HORRIBLE_DATA_DIR/hassault/` via
`backend/paths.py` (they were pinned to `<repo>/.data`), and a `MatchServer` can
be built with `record_replays=False` — the game server's is, since nobody could
watch a replay kept there.
...
- ~~**No water, ladder or slide physics.**~~ Water and ladders are implemented
  across all three cube-path ports and bound by thirteen fixture cases — see
  [Water and ladders](#water-and-ladders). **Sliding is not.** `sprint`,
  `isSliding`, `slideTime` and `stamina` exist as fields on `player.ts`'s
  `PlayerState` / `MoveInput` and ride the telemetry bus, but `physics.py`
  simulates none of them, so nothing on the wire can make you slide. The Rapier
  controller names slide-hopping among its capabilities; that is the wasm
  controller's own vocabulary, not a mechanic this game currently has.
...
2. **Bundled maps** — done: five original maps ship with the app, so it needs no
   AssaultCube install to be playable. Three are Cube heightfields; `hd_facility`
   and `hd_junkflea` are [`gltf` levels](#two-level-formats-cube-and-gltf) with
   real 3D geometry.
...
10. **A workspace, not a pane** — done, then consolidated: three windows (game,
    dev tools, server) whose companions are strips, wired together by a typed
    [event bus](#the-workspace-three-windows-and-their-strips) that parks an intent for a
    pane that has not mounted yet, and two
    [workspace presets](#the-two-workspaces) that arrange them.
11. **Full 3D levels** — in progress: `gltf` maps run a Rapier capsule controller
    for slopes, stairs and a shrinking crouch capsule. The
    [conformance fixture does not yet cover that path](#the-heightfield-physics-does-not-run-on-a-gltf-map),
    which is the next thing it needs.
12. **Textures and map models** — the largest remaining gap between this and
    something that looks like AssaultCube. Needs the `.cfg` parser that maps
    texture slots to files.
13. **Pickups** — done: six kinds off the map's own entities, with armour as a
    real mechanic and an item that cannot help you left where it stands. See
    [Items on the map](#items-on-the-map).
14. **Per-weapon sound** — done: a weapon's voice is
    [derived from its own numbers](#a-weapons-voice-is-derived-not-tabulated)
    rather than tabulated, so five weapons sound different with no assets added.
15. **Water and ladders** — done across the cube path's three ports; see
    [Water and ladders](#water-and-ladders). The **scramble state** is the one AC
    friction constant still missing, and sliding with it.
16. **Game modes** — done beyond deathmatch: [capture the flag](#capture-the-flag)
    and [bomb defuse](#bomb-defuse), with a round structure and an economy.
    Ranked play deliberately stays deathmatch.
