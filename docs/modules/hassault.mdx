...
radius at chest height). Maps that already failed when it was written — bank,
assault, inferno, mirage, nuke — are listed in `KNOWN_BAD_SPAWNS`, and the test
fails if one of them starts passing, so the list only shrinks. Items are checked
on office only: elsewhere they sit in crouch-height spaces a standing probe
rejects.
