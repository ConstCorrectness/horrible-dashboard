...
| Symbol Search | `code.search` | singleton, **embedded** | Cross-repo search: Name (live fuzzy) ⇄ Semantic (`Tab`); each hit drives the locus. Same component the quick-open modal hosts. The editor buffer's right strip, and `mod+p` opens it as a modal — a launcher entry was a third home for one search. |
