...
:::warning
Capture is on out of the box, so `app.db` holds whatever your agent passed to a
tool — API keys, file contents, tokens — **without you having opted in**. The
database is local and unencrypted. Turn capture off in the **Datasets** section if
you do not want that.
:::

Trajectories live in `app.db` (SQLite, the same transactional store as the rest of
the node), not in a separate database: a run joins `agent_turns` and
`eval_results` on `turn_id`, and the `database` console's `app` connection queries
them with no new API.

The recorder only writes while some dataset has `capture = 1`. On first start the
app lifespan calls `store.seed_default_capture()`, which creates a
`dashboard-agent` dataset with capture on — unless a dataset is already capturing,
in which case it is left alone. It runs **once per node**, recorded as
`default_capture_seeded` in `traj_meta`, so turning capture off or deleting the
dataset sticks across restarts; "seed whenever nothing captures" would silently
re-enable it every boot. An upgraded node whose datasets all have capture off is
switched on once, by the same rule.

At most one dataset captures at a time. With capture off, the recorder is one
indexed `SELECT` per agent turn. Every caller of `run_agent_loop` is captured —
including evals runs and `agentpedia.fork` replays, which land in the capturing
dataset alongside chat turns; `trajectories.retentionRuns` bounds its size.
...
Agents you write can report here with no SDK of ours: point any OpenTelemetry
exporter at `/api/otel` and each trace becomes a run with `source: external`,
`external_id` equal to the trace id, and `meta.otel`. It lands in the `otel` dataset, or
the one named by the `horrible.dataset` resource attribute. Notebook and training
kernels get the `OTEL_*` variables at spawn. The **Connect** section shows the endpoint,
the ingest token for other machines, and a snippet per framework.

A received run is a **projection** of its spans. It is rebuilt whenever a batch for its
trace arrives, so it goes live, then complete once the root span lands. Nesting survives
as `parent_seq`, which the step list now indents by. Mapping rules, auth and the
built-in agents' own spans are in [OpenTelemetry](otel.mdx).
...
| **Connect** | `o` | Wire an OTLP exporter up: endpoint, ingest token, per-framework snippets, last-span indicator |
...
A run with an OTel trace (a received one, or a built-in turn while `otel.enabled`)
shows a **Trace** view between the Timeline and the I/O lane: the span tree, indented
by parentage, with concurrent bars. The Timeline cannot draw that shape, because it
lays out round by round. Selecting a span shows its attributes and events. The view
renders nothing for a run with no trace (`panels/TraceView.tsx`, geometry in the pure
`panels/trace-model.ts`).

The Connect section *does* poll (`GET /api/otel/ingest`, every 5 s). It is a "did my
exporter reach you yet" indicator for the minute you spend wiring one up, and the
ingest route publishes nothing to watch.
...
| `otel.enabled` / `otel.captureContent` / `otel.injectEnv` / `otel.forwardReceived` | `true` / `false` / `true` / `false` | OpenTelemetry — see [otel.mdx](otel.mdx) |
