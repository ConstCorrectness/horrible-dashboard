...
| --- | --- | --- |
| **Runs** | `r` | Run list + step-by-step detail, with inline grading |
| **Datasets** | `d` | Collections, the capture toggle, headline counts, tool usage |
| **Harness** | `h` | The A/B comparison |
...
Iconography inside the pane is inline stroke SVG (`icons.tsx`) rather than native
emoji. The manifest's `icon:` field is the activity-rail glyph, typed as a string,
and follows the rail's existing emoji convention.
...
- **A frontend surface for import, export and reindex.** All three are routes and
  agent tools today; the pane has no buttons for them.
