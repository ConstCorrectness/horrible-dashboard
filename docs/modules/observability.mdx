...
- **Panels:** `observability.logs` (singleton, `defaultPlacement: bottom`) — the
  ...
  Clear button. A toolbar **filter** narrows the rows live (client-side, over the
  same in-memory list): a free-text query over method+target, a source selector
  (all / client / inbound / outbound), and an **errors only** toggle; the count
  reads `shown / total` while a filter is active. Rows with captured detail show a
  caret and expand on click to the redacted headers/bodies.
