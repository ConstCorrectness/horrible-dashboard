...
batches of other programs' spans, already stored in full in `otel_spans`. On these
paths the inbound capture also **blanks credential-shaped request headers**
(`telemetry.store.is_sensitive_header`). A remote exporter authenticates with the
node's ingest token, and on a LAN-bound node this ring is readable by the same network
the token exists to keep out. See [OpenTelemetry](otel.mdx).
