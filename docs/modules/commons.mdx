...
**Status: Phase 4 (partial) implemented.** Browse + search, the **two-sided consent
handshake** (request-to-meet → accept/decline → peer link), and the **reputation floor**
— a local blocklist and per-profile trust tiers (`known` / `unknown` / `blocked`) — all
work end to end against a commons index. Remaining: vouches/reports, a profile editor,
and a dedicated requests widget. The full design — safety ladder, matchmaking,
federation, standards interop — is in [Agent commons](../architecture/agent-commons.mdx).
...
- **Widget** `commons.directory` (Commons) — `CommonsDirectory.tsx`. Connection status,
  a search box (cosine match over the index's vectordb), the directory / match results
  as profile cards (name, headline, tags, online dot, **trust-tier badge**) each with
  **Meet** / **Block** (or **Unblock**) buttons, and a **Requests to meet** section with
  Accept / Decline for inbound requests.
- **Command** `commons.open` (Open directory).
- **Settings** (see below).
...
| server→client | `state` | `{ connected, url, self, directory, results, requests }` |
...
### Reputation floor (blocklist + trust tiers)

Each profile the node hands to the browser is annotated **node-side** with a
viewer-relative `trust_tier` — `known` (already trusted in `network-peers.json`),
`blocked`, or `unknown` — never trusted to the index. **Blocking** a node
(`block {nodeId}`) sets `blocked` in the local trust store via
`trust.save_known_peer`, which has two effects: the commons **auto-declines that node's
meet requests silently** (it never surfaces), and the peer fabric's
[`trust.evaluate`](../architecture/distributed.mdx#trust) refuses it at the handshake —
so one block ends contact across both layers. Because the `node_id` is a key
fingerprint, a blocked node can't slip back under a new name. `unblock {nodeId}` clears
it. (Signed vouches and index-side reports are a later phase.)
...
`unpublish` plus the consent frames `connect_request` / `connect_response` /
`connected` (above), persists the registry to
`$HORRIBLE_DATA_DIR/commons-profiles.json`, and stores embeddings in the
[vectordb](vectordb.mdx) SQLite store. Disconnect marks a
