...
The three commons widgets form the **Agent Commons** panel group (`commons.hub`).
The directory hosts Requests and Profile as right region strips, with toggle
buttons for the others. See [Panel Groups](../architecture/panel-groups.mdx).

- **Widget** `commons.directory` (Commons) — `CommonsDirectory.tsx`. The group
  **primary** (hub). Declares `dockable: 'left'`, so it opens in a center area by
  default but can also be toggled into the left dock from the rail, carrying its
  region strips with it — see [Windowing](../architecture/windowing.mdx).
  Connection status, a search box (cosine match over the index's
  vector store), and the directory / match results as profile cards (name, headline,
  tags, online dot, **trust-tier badge**) each with **Meet** / **Vouch** / **Block**
  (or **Unblock**) / **Report** actions.
...
flowchart LR
    ...
    Index -->|"embed + cosine"| VDB["vector store"]
...
to `commons-reports.json`, and stores embeddings in the [database](database.mdx) module's
SQLite vector store. Disconnect marks a
profile offline (kept, still discoverable); `unpublish` removes it.

It also stores **trajectory digests** — a run's shape, never its payloads — via
`publish_trajectory` → `trajectory_published` (or an `error` carrying the digest id as
`ref`), `list_trajectories` → `trajectories`, and `unpublish_trajectory` →
`trajectory_unpublished`, persisted to `commons-trajectories.json`. A digest is signed
by the node key like a profile, and verified on load as well as on publish. What goes
into one, and the typed confirmation before it leaves, are in
[trajectories](trajectories.mdx#publishing-to-the-commons).
