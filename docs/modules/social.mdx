...
| --- | --- | --- |
| Resolve | `@ben` → person key, from the game server's directory (fingerprint-checked) | The account-to-person binding ran only when a username was first *claimed*, so any other account was never findable. It now runs on every sign-in and at startup (`server_auth.schedule_person_binding`) |
| Find | person → their machines, from the **same directory entry**: device certificates signed by the person key, each dialable as `relay:<node_id>` | Machines came only from the Atlas presence record, which an ordinary install has no credentials for, and which held only LAN and tailnet addresses |
...
- It may send and be sent only `social_hello`, `social_friend_request` and
  ...
  else. `social_device_cert` is deliberately **not** open, because it replaces this
  machine's identity.
...
### A username lists its machines

Resolving a username used to yield only a *person*. Which machines that person runs
came from the Atlas presence directory, whose credentials live in a developer's
`.env`, so on an ordinary install `@ben` resolved and then reached nobody. The game
server now keeps each bound person's machines (`person_devices`, capped at the 8
newest):

- A node **posts its device certificate** to `POST /directory/devices` after binding
  its account, and at startup (`server_auth.schedule_person_binding`). No sign-in is
  needed: the certificate is signed by the person key, and the server accepts it
  only if the node id is the fingerprint of the node key and **the person key is
  the one bound to an account**. So it can only restate something true, which also
  lets a linked machine without an account list itself.
- `GET /directory/resolve` returns those certificates as `devices`. The node checks
  each one against the entry's person key (`handles.verified_devices`), because a
  lying directory could otherwise list someone else's machine under your friend's
  name, and records each with a `relay:<node_id>` address that `_dial` then uses.
- The certificate bytes are computed twice, by the node and by the game server, and
  `test_games_person_devices.py` pins the two together.
...
works even with the presence directory down.
...
- **A username lists machine ids publicly.** The directory entry carries each
  ...
  username reach you. It carries no hostname: the certificate published there is
  minted with an empty label (`handles.directory_cert`). A linked machine can only
  present the certificate its owner issued, label included.
