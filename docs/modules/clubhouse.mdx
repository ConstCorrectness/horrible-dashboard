...
The account widget exposes a `getAgentContext()` snapshot of the connection state
(connected, name, username, or the current onboarding step), and the live-rooms
panel exposes the rooms it lists (topic, club, speaker/listener counts, people).
Both are read-only — the agent can reason about who's connected and what's live,
but there are no Clubhouse `agentTools` yet. See
