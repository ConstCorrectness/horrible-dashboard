...
`pty.py`; the default shell is PowerShell on Windows, `$SHELL` elsewhere. Each
PTY's blocking read runs in a thread so it never stalls the event loop; sessions
are killed when the socket closes (resume-on-reconnect is a later enhancement).
