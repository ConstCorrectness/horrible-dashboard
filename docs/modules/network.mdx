...
The Peers tool hosts the other members as bottom region strips — toggle one into the
workspace, click again to close ("rail") it. See
[Panel Groups](../architecture/panel-groups.mdx) for the architecture.

- **Widget** `network.peers` (Peers) — `PeersWidget.tsx`. The group **primary** (hub).
  Shows this node's identity and connected peers with status dots; dial a peer by
  `ws://…/peer-ws` address, generate an invite link, or redeem one to pair. Exposes
  presence to the local agent via `useAgentContext` (so the model can fill
  `agent.ask_peer(peerId)`).
