...
or ask for an agent-generated briefing. Handled by `remote_control.py`, which
drops any command from an untrusted peer; the reverse direction is the `mobile.*`
agent tools in `mobile_tools.py`. The [Android companion
app](mobile-android.mdx) is the primary consumer of both — it pairs by QR and
appears on the fabric as an ordinary peer carrying the `mobile` capability.
