...
The first-run wizard around it (`packages/ui/src/desktop/Oobe.tsx`) is **four
steps**: welcome, *how the desktop starts*, look, setup. "How should the desktop
start?" is deliberately its own step ahead of "Pick a look": choosing between the
home surface, a widget board and a plain wallpaper decides whether the app has a
front door at all, and it used to sit fifth of seven in a list of wallpapers,
weighted identically to a decorative gradient. Backdrop choices in both steps now
carry a selected state — they had none, so clicking one confirmed nothing on the
one screen whose job is to teach the app.
...
It teaches the user their own name, a backdrop, a theme and three credential steps
— and nothing about the product. There is no tour, no help menu and no in-app docs
pane, so the moment it closes was the only moment anyone could be told what the app
_is_.

Two deliberately small things close that, rather than a fifth step:

- The final step signs off by naming the **workspace launcher** the user is about to
  land on (the wizard closes onto the `splash` home surface, where the tiles sit
  directly under the ask bar).
- The Start menu's footer carries a **Help** entry (`shell.help`) opening the
  published documentation. It checks `openExternal`'s boolean return and toasts the
  URL when it fails: `window.open` returns `null` behind a pop-up blocker and the
  Tauri invoke rejects on an older shell, and neither throws — a Help button that
  silently does nothing teaches the user that the docs do not exist.

What it explicitly does **not** do is offer a boot-workspace picker. Booting into an
empty floating desktop is a decision this app made on purpose
(`DEFAULT_BOOT_WORKSPACE`, see [windowing](../architecture/windowing.mdx)); the fix
for the presets being invisible was to surface them, not to change where the app
starts.
