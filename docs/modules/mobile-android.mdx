...
**Status: implemented.** App in `apps/mobile-android/` (Kotlin + Jetpack Compose);
its desktop-side counterparts are `backend/modules/network/remote_control.py` and
`backend/modules/network/mobile_tools.py`.
...
| Remote control | `ui/RemoteControlScreen.kt` | Buttons issuing `remote_command`: open a pane, play media, send a spoken notification. |
| Agent | `ui/AgentScreen.kt` | Chat with the desktop agent over `agent_request`, with `agent_stream` tokens and reasoning rendered live. Opens on a `get_summary` briefing. |
| Friends | `ui/FriendsScreen.kt` | The games friends list, fetched via the `get_friends` command. |
...
| `say` | `text` | Emits `agent`/`notification`. |
