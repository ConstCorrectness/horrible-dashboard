...
Spacing requests isn't the whole story — arXiv also rate-limits over longer
windows and answers **429** (or **503** while shedding load), which no local
throttle can predict. Both statuses are retried up to three times with a
backoff that honors `Retry-After` (falling back to 10 s, doubling per attempt),
and the resulting cooldown is **process-wide**: a deep-research run fanning out
a dozen queries would otherwise burn a full retry budget per query. A caller
arriving during a cooldown longer than 30 s fails fast instead of parking a
request handler, and a rate-limited lookup falls back to a **stale** cache entry
when one exists — a ten-minute-old abstract beats an error. Exhausted retries
raise `ArxivRateLimited`, which the routes pass through as a real **429 with a
`Retry-After` header** rather than flattening it into "arXiv unreachable".
...
The **arXiv** section of the Research window (`research.console`, sections
**Deep research** and **arXiv**; the view is `research.arxiv`, now `embedded`) is the
human surface:
...
:::info Rounds, verification and steering
The pipeline above is the v1 single pass. It now runs as
`plan → [subagents → critique]* → synthesis → verify → citations → export` — see
[Depth: rounds, verification, steering](#depth-rounds-verification-steering).
:::
...
## Depth: rounds, verification, steering

### Gap-filling rounds

A single pass answers only what it thought to ask at the start. The **critique** step
closes each round: it reads what actually came back, names the gaps, and either
declares the findings sufficient or writes new subagent specs targeting only what is
missing. The second round asks questions the first round *taught* it to ask — that is
where the depth comes from.

Rounds are capped by effort tier (quick 1, standard 2, deep 3). `research.maxRounds` is
an **override, not a lid** — it defaults to 0 meaning "use the tier", so a non-zero
default can't silently stop deep runs reaching their third round.

The current round is always derived from the database (`max(round)` over subagent
steps), never from a loop counter. That is what makes a restart mid-round resume into
the right round instead of starting over. Steps are keyed by `(kind, round)`
throughout; keying by kind alone silently collapses several critiques into one.

A failed critique is treated as "sufficient" rather than failing the run — the same
principle as a failed subagent. Throwing away a completed round because the reviewer
couldn't produce JSON would be the worst available trade.

### Verification: what is actually supported

Between synthesis and citations — not after, because the citation pass rewrites the
report and appends `## References`, so the audit has to inform that rewrite.

The interesting half needs no model. Claims are extracted by the lead, then their
citations are grouped by **registrable domain**: three citations to `openai.com`,
`platform.openai.com` and `openai.com/research` are one publisher wearing three hats
and count as **one** independent source. That is the honest reading of "two independent
sources", it is deterministic, and it is unit-tested.

Verdicts are `supported` (two or more publishers), `single-sourced` or `unsupported`.
A separate single pass over the findings flags where sources genuinely **contradict**
each other — one call over the whole block, not pairwise comparison of subagent
outputs, which is quadratic and mostly surfaces differences of emphasis.

The citation step then marks single-sourced claims inline and appends a **Confidence &
caveats** section listing contradictions and unsupported claims. It never deletes or
softens content — the reader decides what one source is worth.

`research.verifyDepth` (`off` | `cheap` | `corroborate`) gates it; `cheap` is the
default and costs two model calls.

### Steering a run in flight

**Review the plan first.** Starting a run with `approval_mode: "plan"` (the "review
plan" checkbox) parks it at `awaiting_plan` after planning, before any subagent has
spent a token. `POST /runs/{id}/plan` approves it, optionally with an edited plan.

The gate **returns** rather than blocking, and that matters twice over:
`research.maxConcurrentRuns` defaults to 1, so a blocking wait would halt every other
run on the node, and it wouldn't survive a restart either. `awaiting_plan` is excluded
from the resume pass — a run waiting on a *person* must not be re-enqueued on every
boot, or it spins through the gate forever.

An edited plan is written to **both** the run row and the plan step's output. The
pipeline reads it back from the step on resume, so writing only the run row would
silently discard the edit the moment the run picked up again.

**Ask follow-ups mid-run.** `POST /runs/{id}/followup` queues a question; the next
critique folds unconsumed follow-ups into its "what's still missing" prompt, so they
shape the next round rather than being lost.

**Watch the tools live.** Each subagent tool call streams on the `research` channel as
a `tool` event and persists to `research_tool_calls`. Without it a subagent shows
nothing for minutes — a step's transcript is only written when the step *finishes*,
which is exactly the window where you want to know whether it is searching sensibly or
looping. `GET /runs/{id}/tool-calls` is how a browser that wasn't open catches up.

### Schema migrations

`init_research_db` was `CREATE TABLE IF NOT EXISTS` only, which is a no-op on an
install that already has the tables — so new columns would have silently never
appeared and every write would have failed on an unknown column. `_ensure_column`
(PRAGMA + `ALTER TABLE`) now backfills `research_steps.round`,
`research_runs.approval_mode` and `research_runs.rounds_used`, and there is a test that
builds the *old* schema first, because a fresh-database test cannot see this class of
bug.
...
## Distributing a wave to friends

A research wave is embarrassingly parallel, and both halves of distributing it
already existed: a subagent is an isolated unit of work with a spec in and a report
out, and `agent.ask_peer` is an authenticated agent-to-agent RPC. So this needs
**no tunnel, no lease and no new wire type** — `research.distributeSubagents`
(default off) hands at most two steps per wave to trusted, connected peers
advertising the `agent` capability.

Four things keep it honest:

- **A remote turn runs under the *peer's* permissions.** The callee gates on its own
  `network.allowRemoteAgent` and `network.remoteAgentMode`, which defaults to
  read-only `plan`. A remote subagent can therefore have **fewer tools than a local
  one**, so the brief asks for read-only research and nothing that acts.
- **A decline is never fatal.** A peer that refuses, times out, or answers with
  nothing returns `None` and the step runs locally instead. Distribution can make a
  wave faster; it cannot make one fail. An *empty* answer counts as a decline —
  recording it as done would let synthesis proceed believing that angle was covered.
- **Provenance travels with the report** (`ran_on`). The verification pass grades
  claims by *independent publisher*, so two peers citing the same domain must count
  as one source rather than two, and that arithmetic needs to know where each
  finding came from. The console prints `ran on <node>` beside such a step, and only
  such a step — its absence already means the work stayed here.
- **Remote tokens are not charged to this run's budget.** They were spent on the
  peer's, and charging them here would throttle a run for work it did not pay for.

Research uses its own `PEER_SUBAGENT_TIMEOUT_S` rather than raising
`agent_bridge`'s: a deep subagent turn is minutes of tool calls, while `ask_peer`'s
default is sized for a conversational question, and a chat user waiting on "ask
Rob's agent" should not inherit a ten-minute ceiling.

Assignments are keyed by **step id**, not index — a follow-up posted mid-wave joins
the round (see [Depth](#depth-rounds-verification-steering)), and indices would
shift under it.
