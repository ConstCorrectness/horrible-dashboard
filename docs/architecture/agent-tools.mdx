...
live catalog via a `list_tools`→`tools` WS round-trip (the backend's `_tools_for`,
the exact set the model sees). Remaining work is incremental per-module polish (e.g.
the editor's desktop-only `osfile:` source), not foundational.
...
`tool-exec.ts` boundary, not in the engine: `split_pane` accepts
