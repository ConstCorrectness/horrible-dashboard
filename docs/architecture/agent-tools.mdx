...
## A tool's group is its name prefix

`_group_of(name)` takes the namespace **before the first dot** — `github.searchCode`
is in group `github`; a dotless name (the layout verbs) is in the always-present
`layout` core.

`AgentTool.group` does **not** name the group. It's only a flag: `provider_tools(grouped=…)`
checks whether it is set, to split always-core tools from progressively-disclosed
ones. So a tool named `game.getObservation` that declares `group="games"` really lands
in a group called `game` — and if nothing describes `game`, it shows up in the catalog
as a bare `"game tools"` fallback with no blurb and no guide. That exact mismatch
existed for the games tools until the connectors work added an invariant test
(`test_connectors.py::test_agent_tool_names_match_their_declared_group`).

**Keep the name prefix and the declared group identical.** For a
[connector](../modules/connectors.mdx), both must also equal the connector's `id` —
that's what lets one definition feed the home tile, the group blurb, and the guide.

:::danger Forgetting `group=` is worse than getting it wrong
A mismatched group costs a blurb. An **omitted** one costs a slot on every turn of
every agent: `provider_tools(grouped=False)` treats a tool with `group=None` as
**always-on core**, so its schema is charged to each round whether or not the turn has
anything to do with it — and the `_GROUP_DESCRIPTIONS` entry naming it can never fire,
because `_group_catalog` only walks the dynamic pool.

That is exactly what `research.*` (5 tools) and `arxiv.*` (3) did: `app.py` claimed
they were "grouped under research/arxiv", `_GROUP_DESCRIPTIONS` had entries for both,
and `researcher`'s `preload_groups=[…, "research", "arxiv"]` was partly a no-op —
while all 8 sat permanently in the main orchestrator's core. Fixing it took the main
agent's always-on core from **34 tools to 26**, nearly doubling the room left under
`TOOL_BUDGET` for capability groups.

`test_connectors.py::test_every_namespaced_agent_tool_declares_a_group` now fails on a
dotted tool with no `group=`, alongside the older mismatch test.
:::

### Two tiers of disclosure

`list_tool_groups` shows each group's one-line blurb (`_GROUP_DESCRIPTIONS`, falling
back to a connector's `blurb`); `load_tools` delivers the group's full **guide** — a
SKILL.md-style markdown doc. The blurb answers "would this help?"; the guide answers
"how do I use it without getting it wrong?". Guides ride the tool-result message, so
they need no orchestrator plumbing of their own.

**Any group can ship a guide**, not just connectors. `_group_guide` resolves
`backend/modules/agent/guides/<group>.md` first (`agent/guides.py`, `lru_cache`'d),
then a connector's `resolve_guide()`, then the MCP bridge. For a long time only the
latter two existed, so every *built-in* group — `layout`, `files`, `editor`,
`database`, `games` — had a one-line blurb and nothing else; detail that belonged in a
guide had nowhere to go but the system prompt, where it was charged to every round of
every turn.

Guides are plain markdown so they review and diff like code. Put anything in one that
a model gets wrong repeatedly and that would be too expensive to say on every turn:
which argument is an instanceId rather than a view id, which combinations are useless,
what the module calls a thing.

### `show`: the intent verb

Reaching a surface used to cost three rounds — `list_available_panes` (a row per
registered view), `open_pane`, then `get_pane_context`. Three chances to emit
malformed JSON and a large tool result, for a model whose whole window may be 8k.

`show(target, where?)` collapses that to one call. `target` is loose text — a pane
title, a view id, a **section** label, a side-strip name, a workspace name — and the
**host** does the matching (`layout/show.ts`, pure and unit-tested; `showTarget` in
`layout/controller.ts` acts on the result). It opens or focuses (never a second copy of
something already visible), switches to the named section, reveals a region inside its
host, or switches workspace, and returns the resulting pane's context snapshot inline,
so no `get_pane_context` follow-up is needed.

Because panes are being **merged** rather than deleted, `show` resolves a section
label directly: the user says "friends", not "the People pane, Friends tab", and so
should the model. An `embedded` view resolves to *where it lives* — its host pane and
section — because it has no standalone home to open.

Its argument is deliberately **one string**, not a discriminated union: `_coerce_args`
turns an unparseable payload into an error, and a wide schema is more of them. The way
to shrink a tool list is to move tools out of the always-on core, not to widen the ones
that remain.

`VIEW_ALIASES` (in `layout/controller.ts`) maps retired ids and titles to where their
content lives now. It is deliberately **separate from `serialize.ts`'s
`RENAMED_VIEWS`**, which repairs saved layouts: a stored arrangement is disposable and
reseeds from its preset, but a name the agent has ever used should never stop
resolving. Add an entry whenever a pane is merged away.

## The model: one tool path, one gate

Every **frontend-declared** agent tool call flows through the **frontend relay** over
the shared `/ws` `agent` channel — there is no relay bypass, even for tools whose
resource (a PTY, the filesystem) lives in the backend. Those tools' handlers run in
the frontend and call the backend's existing HTTP/WS API, so the agent has exactly
**one** tool model to reason about.

:::note Backend-registered tools are the exception
Tools contributed via `backend/sdk` (`host.add_agent_tool`) run their handler in the
backend and never touch the relay — used where a tab may not exist at all (a cron run,
the `dash` REPL, `agent.ask_peer`) or where the resource is a server-held secret the
browser must never see. Today: games, training, and the
[connectors](../modules/connectors.mdx). They pass through the same permission gate,
so the sentence that matters is unaffected: **one gate**, not one path.
:::
...
- A pane component registers a snapshot provider via the `useAgentContext(() =>
  ...
  without a core→ui cycle; `packages/ui`'s `PaneHost` supplies the live instance id
  through `PaneInstanceContext` and, around a section body it renders itself, the
  section id through `SectionInstanceContext`. Providers are keyed by the **pair**.
- Two built-in read tools (ungated) expose it: `list_open_panes()` (the
  ...
  `hasContext` flag) and `get_pane_context(instanceId, section?)`, which invokes the
  registered provider and returns its JSON snapshot.

For a pane with **sections**, the active section id is stamped onto every snapshot by
`readPaneAgentContext` — centrally, so a module can't forget it and hand back a
snapshot that is ambiguous about which tab it describes. Passing `section` reads a
different one, which **switches the pane to it first**: only the visible section is
mounted, so there is no way to read a background section without bringing it forward,
and pretending otherwise would return the wrong section's data as if it were the one
asked for. A pane-level provider and the mounted section's provider **merge**,
section last — see
[Embedded views and sections](windowing.mdx#embedded-views-and-sections).

A section with no provider is reported as `hasPayload: false` rather than as an
empty object. That distinction matters more than it looks: a pane that returns
nothing does not make a model answer "I don't know", it makes it answer from
whatever else is in the context window.
...
### Validation stub (retired)

A dev-only `stub` module once exercised the whole surface — a read tool, a gated
tool with a `specifierTemplate`, and a `getAgentContext` snapshot — and validated
all four permission modes live. It was removed once every real module carried
`agentTools` of its own: it appeared in the dev build's launcher as "Agent stub",
which is a pane nobody wanted, and any shipped module is now the shape to copy.

## `load_tools` reads whatever the model spelled

An *array* parameter comes back from small models as a string more often than not.
Measured against llama-3.2-3b: `{"groups": "[\"github\"]"}` and
`{"groups": "['files', 'editor']"}`, the second not valid JSON at all.

The dispatcher used to do `if isinstance(requested, str): requested = [requested]`,
which turned `'["github"]'` into a group named `["github"]`, matched nothing, and
answered `loaded: []`. The model had asked for exactly the right group; the
capability simply never arrived, and it read as a model that could not work
progressive disclosure.

`_coerce_group_list` accepts a real list, a JSON array in a string, a Python-style
list, a bracketed comma list, or a bare name — and deliberately does **not** run a
bare name through a list parser, because `github` is a group name and must stay one.

`_coerce_args` handles the same class of problem one level up, for the arguments
payload as a whole. This one slipped past it because the payload was valid JSON; it
just had a string where the schema said array.
