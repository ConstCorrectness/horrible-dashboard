...
- **View type** — a `PanelDecl` (multi-instance) or `WidgetDecl` (singleton) in the
  registry.
- **Pane instance** — a live container slot in the frame's layout tree hosting a View.
...
- **Ambient / backdrop panes** — render *behind* the dock as a living backdrop (the
  ...
  A new layer in `Frame.tsx` below the center grid.
...
| Ambient backdrop | engine | new layer under the center grid | low |
