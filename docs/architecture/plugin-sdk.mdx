...
`SettingDecl`, `SettingType`, `Capability`, `WsMessage`) live in the SDK and are
re-exported by `@horrible/core`, so plugins and built-in modules share one
contract.
...
the `settings` contribution in SDK `0.2.0`, and `WidgetDecl.defaultPlacement` in
`0.3.0`) do **not** bump it, so existing plugins keep loading.
