...
`PaneRole`, `WsMessage`) live in the SDK and are re-exported by
`@horrible/core`, so plugins and built-in modules share one contract. A `KeybindingDecl` can be **focus-scoped**: set `scope` to a pane
