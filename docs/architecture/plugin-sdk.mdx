...
export default definePlugin({
  setup(host) {
    return {
      widgets: [{ id: 'my-plugin.thing', title: 'Thing', component: Thing, role: 'widget' }],
      commands: [{ id: 'my-plugin.doIt', title: 'My Plugin: Do it', run: () => {} }],
      settings: [{ key: 'my-plugin.size', title: 'Size', type: 'number', default: 5 }],
      // A browser inside the Explorer pane, rather than a pane of its own.
      explorerSources: [{ id: 'my-plugin.stuff', label: 'Stuff', component: Browser }],
      // panels?, keybindings?, backdrops? — same shapes built-in modules use
    };
  },
});
```
...
- **Namespacing is enforced:** every contributed command/panel/widget id **and
  ...
  otherwise.
- **`backdrops`** contribute a desktop backdrop (`BackdropDecl` — an id, a title
  and a component taking that desktop's stored `params`). It is a first-class
  registry collection, so a plugin's backdrop appears in the desktop's right-click
  menu, the first-run picker and the agent's `desktop.set_backdrop` catalog with no
  further registration — and its `desktop.backdrop:<id>` command is synthesized, so
  there is no second list to keep in step. See
  [desktop-shell.mdx](desktop-shell.mdx#backdrops).
- **`explorerSources`** is the one place a contribution lands inside *someone
  else's* pane: a tab in [Explorer](../modules/explorer.mdx), beside Files and
  Notebooks. It exists so a plugin that wants to let you browse something does not
  have to mint another near-identical left-dock strip — the failure mode that made
  five built-in browsers into five rail glyphs. `ExplorerSourceDecl` is a
  `SectionDecl`, so it inherits the tab strip, the persisted active tab, the pick
  key, and the agent's `show("stuff")`. A source declaring its own
  `useAgentContext` would silently overwrite the host pane's single provider — see
  the agent-context rule in [windowing](./windowing.mdx#embedded-views-and-sections).
...
`SettingDecl`, `SettingType`, `Capability`, `CollabDecl`, `RegionViewDecl`,
`PaneRole`, `PaneCaptureDecl`, `WsMessage`) live in the SDK and are re-exported by
`@horrible/core`, so plugins and built-in modules share one contract.

A `KeybindingDecl` can be **conditioned**: `when` takes a clause over a closed
context-key vocabulary (`when: "paneFocus == 'my.pane'"` is active only while that
pane is focused, and beats a plain global on the same key), `override: true` makes
a binding unshadowable, and `hosts`/`platforms` restrict a default to the builds
where its chord is actually reachable. `key` accepts sequences (`mod+k mod+s`) and
physical keys (`code:KeyW`). A `PanelDecl`/`WidgetDecl` can declare `capture` to
take the keyboard while focused. See
[keybindings.mdx](keybindings.mdx).

A plugin's panes are first-class frame panes, just like a built-in module's.
Every `PanelDecl`/`WidgetDecl` declares a **`role`** — `document` (tabs into a
center area), `widget` (a center area of its own), or `tool` (lives in a dock;
`defaultDock` picks the side) — plus an optional `icon`, `regions`
([per-pane strips](panel-groups.mdx)) and `sections` (in-pane tabs). There is no
separate widget board — see [windowing.mdx](windowing.mdx).

A view may also declare a **`category`** (`PaneCategory`: `research`, `agents`,
`build`, `data`, `play`, `people`, `system`), the Start-menu band it is filed
under. A plugin that declares none lands under **Plugins**; built-in modules set it
once on their manifest. See
[desktop-shell.mdx](desktop-shell.mdx#the-taskbar) for how the launcher groups.

A view that only ever renders **inside** another pane — as a region strip or a
section body — declares `embedded: true`. It stays registered and reachable, but
stops being a destination: no opener command, no rail glyph, no type-switcher
entry, and no top-level row in the agent's `list_available_panes`. See
[Embedded views and sections](windowing.mdx#embedded-views-and-sections) for the
full contract, including the one-provider-per-pane agent-context rule that
`sections` imposes.
...
`SDK_API_VERSION` (currently `2`) is bumped on **breaking** contract changes
...
the `settings` contribution in SDK `0.2.0` and the `collab` pane field) do
**not** bump it. Version `2` (SDK `0.4.0`) is the frame-engine contract: pane
`role` became required, `defaultPlacement` and panel groups were removed, and
views may declare `regions`. The later `sections` and `embedded` fields are
additive and optional, so they do not bump it either.
