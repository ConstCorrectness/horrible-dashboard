...
`defaultDock` picks the side) — plus an optional `icon` and `regions`
([per-pane strips](panel-groups.mdx)). There is no separate widget board — see
