...
out of scope** — that requires a WebRTC datachannel transport (future); until then
the relay is the guaranteed fallback.
...
| WebRTC hole-punching (symmetric NAT over TCP) | 🚧 future (needs a datachannel transport) |
