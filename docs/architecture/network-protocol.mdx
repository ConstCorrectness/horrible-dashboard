...
## The lobby system (proposed)

Today peers find each other by **manual invite link**, **direct address**, or **LAN
mDNS**. The target adds an **official lobby**: a client/server intermediary that is
more than a dumb relay — it's a **presence directory + room listing + signaling**
service, with P2P as the preferred data path and relay as the fallback.

**Status: design.** The relay broker (`relay_broker.py`) and `network.directoryUrl`
setting are the seeds; the lobby extends them. Nothing below is implemented yet.
...
### Lobby wire protocol (proposed, over a `/lobby-ws` socket)
...
| **Lobby** (directory + rooms + signaling) | 🚧 proposed (this page) |
