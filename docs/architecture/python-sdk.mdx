...
| --- | --- | --- |
| `dash.layout.describe()` | `get_layout` | the whole frame: center split tree (areas, tabs, regions), docks, floating, fullscreen/focus |
...
| `dash.layout.move(instance_id, area_id=None, direction=None)` | `move_pane` | into an exact area, or toward a neighbor |
...
| `dash.layout.float(instance_id)` / `dock(instance_id)` | `float_pane` / `dock_pane` | pop out to a floating card / put back |
