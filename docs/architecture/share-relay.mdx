...
flowchart LR
    ...
    subgraph Relay["horrible-share"]
        REG["Registry<br/>code and token to stream"]
    ...
    CAP -->|"WHIP /whip/token: SDP offer"| ROOM
    STRANGER -->|"opens the link"| PAGE
    PAGE -->|"WHEP /whep/code: SDP offer"| ROOM
    ROOM -->|"encoded frames"| STRANGER
```

## What a public viewer is

A viewer holds a watch code and gets **pixels and chat**. Never a grant, never a
...
Every stream is minted with two values, and they are **never interchangeable**:

| | Looks like | Addresses | Authority |
| --- | --- | --- | --- |
| **Code** | `k7m2x9qp` — 8 Crockford base32 characters, ≈40 bits | `/<code>`, `/s/<code>`, `/whep/<code>`, `/chat/<code>` | Watch and chat |
| **Token** | 43 URL-safe characters, 256 bits | `/whip/<token>` (and the key-gated `/streams`, `/restream`) | Publish video |

The link a host hands out is the short one, `https://horrible-share.fly.dev/k7m2x9qp`,
served in place (no redirect) so the URL a viewer sees stays the one they were given.
The viewer page carries the code and **never** the token.

**Why two.** Until this split a stream had one token, used in both the viewer URL
(`/s/<token>`) and the ingest URL (`/whip/<token>`). WHIP is gated on nothing but that
token, and `Room.publish` closes whoever is publishing and takes over — so anyone
holding a public link could POST to `/whip/<token>` and **replace the host's screen with
their own video**. `Registry.get` (token) and `Registry.get_by_code` (code) now refuse
each other's value, and `test_a_viewer_cannot_publish_with_the_code_from_their_link`
pins it over HTTP.

**Why 40 bits is enough for the code.** It would be thin for a permanent secret. It is
not thin for a value that dies within hours, can carry a passphrase, and sits behind a
**miss limiter** (`ratelimit.py`): an address that names 20 well-formed codes that do not
exist — or supplies 20 wrong passphrases — in a minute is answered **429** for the rest
of that minute, on every code-addressed route, including the real code, since a limit a
guesser walks past once they land on a hit is a filter, not a limit. Three rules that are
quiet if wrong:

- **Only well-formed codes count.** A browser requests `/favicon.ico` beside every page;
  charging it as a guess would lock out a viewer who reloaded twenty times.
- **`Fly-Client-IP` is trusted only on Fly** (`FLY_APP_NAME` is set). Fly's proxy
  overwrites it; anywhere else it is a header the caller wrote, and trusting it would
  hand a guesser a fresh "address" per request.
- **A wrong passphrase is a miss too.** It is a guess, and each check is a PBKDF2 run —
  an unlimited stream of them is also a cheap way to spend the relay's CPU.

A code is normalised the way people type it: case-insensitive, hyphens ignored, and
Crockford's look-alikes folded (`i`/`l` → `1`, `o` → `0`).

## The token

The token is publish authority, and the rules below apply to both values:

- **Opaque and random, not signed.** A JWT would let the relay verify
...
| Watch (page, WHEP, chat) | The code (+ passphrase) | This is the public path — meant to be openable by a stranger, and never able to publish |
| Guess a code | — | 20 misses per address per minute, then 429 |
...
`ShareSession.link` (the short view URL) is broadcast to every guest. The **ingest
URL is not, and must never be** — anyone holding it can push their own video into
the host's stream. It lives on the node's session object and is served only from
`GET /api/share/link`, a route no guest can reach. That protection was hollow for
as long as the view URL carried the same token; see
[two keys per stream](#two-keys-per-stream).

A node talking to a relay that predates short codes still works: `link.mint` reads
`short_url` with `.get` and falls back to `view_url`, because the node and the relay
deploy separately and a `KeyError` there would break minting outright.
...
### Memory is a queue-discipline bug, not a sizing problem

The 4 GB machine was OOM-killed too, 2m21s into a **single** share. That kill is
what established the real mechanism, and it is worth stating precisely because
every instinct here points at the wrong lever: it was not the viewer count, and
raising the memory only moves the deadline.

**Every queue on aiortc's receive path is unbounded, and nothing anywhere drops a
frame.** `decoder_worker` decodes each inbound frame and `put_nowait`s it onto
`RemoteStreamTrack._queue` whether or not anyone is reading it, and
`MediaRelay.subscribe()` defaults to `buffered=True`, giving each consumer its own
unbounded queue filled the same way. A decoded frame is freed only when somebody
pulls it, and the host is never told to slow down. A 1080p frame is ~3 MB of
YUV420; at the capture's 15 fps that is ~45 MB/s of accumulation from any consumer
that is slow — or absent.

Absent is the sharper half, because it needs no viewers at all. `MediaRelay` does
not read a source until some proxy of it calls `recv()`, so a host who mints a
link and starts sharing *before anyone opens it* was not idling. The relay was
filling memory at line rate while reporting a perfectly healthy stream.

So `fanout.py` holds two rules, both pinned by
`backend/tests/test_share_relay_backpressure.py` against real aiortc:

1. **The ingest is pumped from the moment it is published** (`Room._start_pump`),
   not from the moment somebody watches. The pump discards what it reads; the
   point is the side effect of keeping `MediaRelay`'s reader running.
2. **Video subscribers are unbuffered** (`_buffered_for`), so a slow encoder skips
   to the newest frame instead of growing a backlog — one frame of memory per
   viewer regardless of how far behind they fall. Audio stays buffered, because
   dropped audio is *audible* where a dropped video frame is merely coarser, and
   a decoded Opus frame is three orders of magnitude smaller.

For live video, skipping is the correct semantics rather than a compromise: a
frame that arrives late is worthless, and the alternative is latency that climbs
until the process dies. Re-encoding removes the usual hazard — dropping *decoded*
input cannot desynchronise a downstream decoder, because each viewer's encoder
emits its own keyframes on PLI.

The test file asserts the failure mode as well as the fix. If aiortc ever bounds
its own queues, the control case goes red, and that is a signal to revisit the
approach rather than a test to repair.
...
to "a handful" than to 25. The ceiling refuses the next viewer with a 503 and a
plain sentence, because accepting them and degrading the stream for everyone
already watching is the worse failure. An HLS branch is the answer when it
matters (Phase 6).

The number is **six**, and it is six because the twenty-five it replaced was
disproved rather than doubted: the deployed relay was OOM-killed 2m21s into a
single share on a 4 GB `performance-2x` (`exit_code=137, oom_killed=true`). It is
`SHARE_RELAY_MAX_VIEWERS`, set in `fly.share.toml` next to the machine it was
sized for, because the right value is a property of the box — how many software
encodes it carries at the host's capture resolution — and not of the code.
`DEFAULT_MAX_VIEWERS` in `tokens.py` is the same six for a relay run with no env
at all. Raise it from a measurement, not from a demo that happened to survive.

Note the ceiling is **per stream**, not per machine: two concurrent shares at six
viewers each is twelve encodes, and nothing enforces a machine-wide total. Six is
therefore a floor on what one popular link costs, not a cap on what the relay can
be asked to do.
...
## A viewer says 'live' only when a frame has decoded
...
Moving `live` onto `connectionstatechange` fixed half of that and left the other
half, because **`connected` is not a picture either**. It means ICE and DTLS
completed — that the two ends can talk. The relay answers a WHEP offer for a
room whose track has stopped yielding frames exactly as happily as for a live
one, so a host who stopped sharing left viewers on the same black rectangle
under the same green chip, with the overlay that would have explained it hidden
by an event that had proved nothing about the video.

`live` therefore comes from the **first decoded frame** and nothing else:
`requestVideoFrameCallback` where it exists, `loadeddata` everywhere else. Both,
because neither fires for a track that carries no media — which is the whole
point. Until one does, the chip reads `connected` and the phase says "waiting for
video".

The 12s deadline is guarded on that same frame rather than on the connection
state, and reports which of the two failures happened: "could not reach the
stream" when no path formed, "connected, but no video" when one did and nothing
came down it. Guarding it on `connectionState === 'connected'` is what let the
silent case run out the clock unreported — the one watchdog that could have
named it disarmed itself on the evidence of the thing that was not wrong. The
deadline is needed at all because ICE can sit in `checking` for a long time and
may settle on `disconnected` rather than `failed`, in which case the failure
handler never runs.
