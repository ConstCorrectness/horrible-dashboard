...
The server is also the **username directory and peer relay** the friends layer
depends on (see [social](../modules/social.mdx#adding-someone-anywhere-by-username)):

| Route | Purpose |
| --- | --- |
| `POST /account/devices` | Enroll the calling machine in the signed-in account: bearer + a node-key signature over `store.enroll_challenge`. Returns a certificate signed by the account's person key (held server-side in `account_identities`) and the account's other machines |
| `DELETE /account/devices/{node_id}` | Unlist a machine on sign-out |
| `GET /directory/resolve` | `@handle` → person key and its machines (`devices`) |
| `WS /relay-ws` | The peer relay broker (`backend/relay_broker.py`): challenge-signed registration, frames pinned to their sender, `undeliverable` for offline targets |
| `GET /relay/health` | Relay client count |

The broker is imported from `backend/relay_broker.py`, not from
`backend.modules.network`, whose package init would pull the node's settings store
into this process. A test asserts the game server never imports it.
