...
| Route | Purpose |
| --- | --- |
| `POST /account/person` | Bind the signed-in account to a person key (bearer + person signature) |
| `POST /directory/devices` | List a machine under its bound person, as a device certificate. No sign-in: the certificate must verify under the bound key |
