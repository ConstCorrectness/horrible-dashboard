...
| `host.add_agent(AgentSpec(...))` | a **specialized agent** in the roster (own system prompt, tool-group scope, per-agent `agent.<id>.*` settings and chat sessions) — addressable from the chat widget's picker and `agent.delegate`. Built-in ids (`main`, `coder`, `dba`, `researcher`) cannot be shadowed. See [agent chat](../modules/agent-chat.mdx#specialized-agents--delegation) |
...
| `host.add_training_provider(provider)` | one trainable-environment source for the [training](../modules/training.mdx) module (Kaggle, HF, Gymnasium, …) |
| `host.add_search_provider(provider)` | a web-search provider, joining the existing fan-out and rank fusion |
| `host.add_dataset_source(source)` | one place training material lives — search, splits, peek, locate. See [datasets](../modules/datasets.mdx) |
| `host.add_recipe_backend(backend)` | one **training framework**: its tasks, its knobs, and the code they become. Inherits the venv-introspection safety net |
| `host.add_eval_runner(runner)` | one way of executing an eval suite; results land in the same tables as every other runner's |
...
## The AI-research seams

Four of the methods above exist so an outside contributor can extend the research
pipeline without touching its core:

- **`add_dataset_source`** — a new place datasets come from. It appears in the browser
  beside the Hub and a recipe can point at it like any other.
- **`add_recipe_backend`** — a new training framework. The important part is what it
  does *not* have to write: `recipes.introspect()` asks the backend for
  `probe_classes(task)` and interrogates those classes in the project venv, so a new
  framework inherits the whole `ok | renamed | unsupported | unvalidated` field
  resolution. A backend that wrote its own validation would be a backend whose
  "dropped field" report drifts from every other one's.
- **`add_eval_runner`** — a new way to execute a suite.
- **`add_search_provider`** — a crawled corpus joining the same rank fusion.

Built-ins win id conflicts in all four, matching the connector rule. See
[the AI-research pipeline](./ai-research.mdx) for how they fit together.
