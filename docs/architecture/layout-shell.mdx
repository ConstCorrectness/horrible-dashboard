...
| `ctrl+space` (Esc exits) | fullscreen the focused area (in-window) |
...
- **`window.perWorkspace`** — a workspace tab's context menu gains **Open in new
  ...
  The new window is labelled `ws-<id>` and loads the same frontend with
  `?workspace=<id>`; `main.tsx` reads that param and boots `AppShell` straight into
  that workspace (skipping home). All windows share one backend and persist their
  own workspace server-side, so no per-platform fork. The `ws-*` capability glob
  grants secondary windows the same titlebar permissions as `main`. (v1 caveat:
  opening the *same* workspace in two windows is last-write-wins on autosave, and
  the server "active" flag tracks the most recently switched workspace in any
  window.)
