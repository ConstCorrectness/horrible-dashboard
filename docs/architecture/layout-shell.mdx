...
- **`workspace`** — a **dockable window manager**: module panes (panels _and_
  ...
  Several **named workspaces** exist, switchable via a tab strip; the Dashboard
  is the default seeded one. Each workspace's layout persists. Built on dockview,
  wrapped so the registry stays the public API. See
  [windowing.md](windowing.mdx) for the model, pane types vs instances, named
