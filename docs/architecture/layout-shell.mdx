...
- **`workspace`** — the **frame**: a hybrid VS Code/Blender layout engine.
  ...
  live in three fixed **docks** behind a VS Code-style **activity rail**, and
...
view, the frame adds the **activity rail** (one glyph per tool view; click opens
it in its dock, click again hides the dock; command palette at the bottom).
Individual panes are opened from the palette, the rail (tools), or each area's
...
- **Keybinding service** — maps keys to command ids with **focus scopes**.
  Modules declare defaults; the shell resolves them against the focused pane (see
  below) and (on desktop) registers any global shortcuts.
...
- **`scope`** — a pane view id (e.g. `terminal.instance`). The binding is only
  active while a pane of that view is focused, and it **shadows a plain global**
  binding for the same key. So a focused terminal can bind `mod+k` to
  `terminal.clear` while `mod+k` opens the command palette everywhere else.
- **`override`** (on a *global* binding) — when true, the global wins even if the
  focused pane has a scoped binding for the same key. The escape hatch for
  shortcuts that must never be shadowed.

The shell tracks the **active scope** — the focused pane's view id — in
`packages/core/src/keybindings.ts`: the frame's pane host calls `setActiveScope`
...
Key specs are `+`-separated modifier tokens plus the key — `mod+k`,
`alt+shift+left`, `ctrl+space`, plain `t` — and modifiers match **exactly**
(`mod+b` and `mod+alt+b` bind independently). `mod` is ctrl or cmd.
...
| `ctrl+space` (Esc exits) | fullscreen the focused area |
...
| `mod+1..9` | switch workspace by position |
| `mod+k` | command palette (override-global) |
...
| `window.perWorkspace`  | no                                 | planned (phase-2 native shell): a workspace per OS window |
| `window.fullscreen`    | no (in-window fullscreen-area only) | planned (phase-2 native shell)  |
| `chrome.workspaceTabs` | no (the frame renders its own tab strip) | planned (phase-2 native shell): tabs in the native titlebar |
| `shortcuts.global`     | no                                 | yes (summon window, quick chat)  |
| `tray`                 | no                                 | yes                              |

The three `window.*`/`chrome.*` **phase-2** rows are the contract for the
planned native shell (workspace-per-OS-window, native chrome hosting the
workspace tab strip, OS fullscreen). They exist in the `Capability` union today
so the frame can branch on them, but no shell grants them yet.
