...
### `hint` beside, `detail` below

An item can carry two kinds of secondary text and they are not interchangeable.
A **`hint`** shares the row with the label, right-aligned: a keybinding, a parent
directory, a one-word state. A **`detail`** is a sentence and is drawn on its own
line underneath.

The row can only lay out one of those well. The label column is
`minmax(0, 1fr)`, so it is what gives way when the row is crowded — passing prose
as a `hint` took the whole 340px and ellipsized the label to nothing. That is
literally what happened to the theme picker: every entry showed a full sentence
about a theme and none of them showed its *name*, in a menu whose only job is to
let you pick one by name. Both the theme and backdrop pickers now pass their
description as `detail`; a `hint` is additionally capped at half the row.
...
| `area.tab`        | `areaId`, `instanceId`, `viewId`, `title`  | shell               |
| `editor.selection`| `uri`, `text`, `empty`                     | **llamacpp**        |
| `library.chunk`   | `text`, `title`, `sourceId`, `chunkIndex`  | **llamacpp**        |
| `training.project`| `projectId`, `name`, `owner`               | training            |

`files.background` is the empty space below the last row of the tree, which the
tree used to swallow. It is a **separate kind** rather than `files.node` with a
null path, because the two genuinely differ: there is no row, so every row-scoped
verb (open, rename, delete, copy path) is absent, and the create verbs fall back to
the first writable root instead of the clicked directory. Reusing the node kind
would have produced a menu whose every second item was permanently grey — exactly
what *absent, not disabled* exists to prevent. The tree's row handler runs first
and this one checks `defaultPrevented`, so a row click never opens both.

`training.project` is a row in the training projects list, and is the clearest case
of the menu paying for itself. That row used to carry five inline buttons — open,
recipe, two pushes and delete — on one flex line in a 280px dock; the row is the
open action now and the rest live here, with `Kaggle`/`Colab` folded into a
`Push a copy to ▸` submenu. A project another module owns keeps every item but
**disabled with a `detail`** saying why, rather than absent: the explanation is the
point, and hiding them would make an inert project look identical to a healthy one.
Delete stays enabled, because reclaiming the disk is exactly what still applies.
The row also exposes a `⋯` button that opens this same menu — a menu nobody knows
is there is the actions being gone.

`area.tab` is a document tab in a centre area's strip. Its bulk-close items read
the tab list from the store **when they run**, and close by instance id rather than
by index: every close is guarded and may prompt, so the strip can shift under a
half-finished *Close Others*, and closing by position would then start eating the
wrong panes.

`editor.selection` and `library.chunk` are the two **text** kinds, and both exist
because a surface owned by one module holds text another can do something with:
the llama.cpp module puts *Trace this* on both, and neither the editor nor the
library has any idea it did. The editor reads the selection from the **live
CodeMirror view** rather than the buffer's persisted bytes — what is on screen is
what was right-clicked, and acting on anything else is worse than declining. An
`empty` selection is a real target rather than an absent one, following the same
convention "run selection" tools use: no selection means the whole file.
...
1. Call `openContextMenu(event, target)` from `onContextMenu`, and
   `preventDefault()` **and `stopPropagation()`** only when it returns `true`.

   :::warning A nested surface must stop propagation
   `preventDefault()` suppresses the *browser's* menu; it does nothing about the
   event continuing up the tree. Where a surface sits inside another one that also
   has a menu — a taskbar window button inside the taskbar, a file row inside the
   tree — the ancestor's handler runs next and calls `openContextMenu` again, and
   because the store keeps a single `open` menu the ancestor's simply **replaces**
   the one just opened.

   The failure is quiet and looks like a missing feature rather than a bug: for a
   while a right-click on a taskbar window button showed the taskbar's own menu
   (Position / Auto-hide / Reset), so *Close* appeared not to exist even though
   `windowButtonMenu` had always offered it.
   :::
