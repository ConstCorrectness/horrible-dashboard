...
- **Three pane roles = default placement.** Every view (panel or widget
  ...
  **Which role a view should declare.** The three are easy to pick between by
  what the pane *is*, rather than by where you first imagined putting it:
  - a **document** is a surface you work *in* — one per subject, several open at
    once, and it benefits from tabs;
  - a **tool** is a *companion* to whatever you are working on — narrow,
    singleton, always the same thing, useful alongside a document;
  - a **widget** is a *readout* — glanceable, no interaction depth, sized to a
  ...
  that a frame preset *tabs* with others cannot be one. `lab.hub`,
  ...
  A view that has become *purely* a companion says so with **`embedded: true`**,
...
- **Workspaces are full frames (Blender model).** A workspace persists
  *everything*: the center tree, dock visibility/sizes/contents, windows, its own
  `mode` and `backdrop`,
  ...
  **Desktop is first and deliberately empty**: floating mode, an aurora backdrop,
  ...
  anything, and the presets beside it stop reading as *choices* once one of them
...
resolve an embedded view to *where it lives*: `show("Outline")` reveals it in its
...
`role` and `embedded` decide where *new* opens go; neither touches a layout that is
...
region is a *companion* beside the main content; a section *replaces* it. Each
...
**merge**, section last, and reads are pinned to the *active* section so a provider
...
there because many sections still have no provider at all (only the *mounted* body
...
whose entire purpose is to be the one place you go to find *something*, wherever it
...
- **The snap zone is stored alongside the rect, not derived from it.** A snapped
  ...
  *today's* desktop rather than to stale pixels from a differently sized one.
...
- **Area header** (Blender-style, collapsible per-area for chrome-less
  ...
  the strip grew a `+` — the affordance for adding a pane *to this group* has to
...
- **Tab gestures**: a tab is `draggable`, and dropping one on another tab in any
  ...
  *Minimize* names the taskbar in its hint, because a minimized tile leaves no
  ...
  *Split Right* on a tab goes through `moveTabToSplit`, not `splitAreaBy`. The
...
- **Keys are workspace-scoped.** `paneSeq` lives on `FrameState`, so instance ids are
  unique only *within* a frame: two workspaces can each hold a
