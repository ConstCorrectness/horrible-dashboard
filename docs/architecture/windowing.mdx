...
spawns a second OS window (`ws-<id>`, loaded with `?workspace=<id>`) that boots
straight into that workspace, all windows sharing one backend. See the capability
table in [layout-shell.mdx](layout-shell.mdx).
