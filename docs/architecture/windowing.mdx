...
its provider registers under the **real, enumerable** pane instance id. What
follows is one rule: **exactly one live `useAgentContext` provider per pane** (the
host component or its section bodies, never both), since they key on the same id.
...
there because most sections have no provider at all (only the mounted body can
provide, and at most one of them does): a bare `{section}` gave the model nothing to
answer from, and a pane that returns nothing does not produce "I don't know" — it
produces a confident answer sourced from whatever else was in the context window.
