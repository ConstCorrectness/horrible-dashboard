...
- **Three pane roles, strict zones.** Every view (panel or widget declaration)
  ...
  - **tool** — lives **only in the docks**, one visible per dock (file explorer,
    outline, terminal, REPL, agent chat, observability…); `defaultDock` picks
    its side;
...
- **Workspaces are full frames (Blender model).** A workspace persists
  ...
  **Coding Harnesses**, plus module-contributed ones (Training, Notebook,
  Orchestration); `layout.reset` restores the preset.
...
(no per-platform fork). The floating layer and fullscreen mode are in-window
today; the phase-2 native shell claims the OS-level versions through new
capabilities — `window.perWorkspace` (a workspace per OS window),
`chrome.workspaceTabs` (the tab strip in the native titlebar), and
`window.fullscreen` — declared in the SDK now, implemented by no shell yet. See
the capability table in [layout-shell.mdx](layout-shell.mdx).
...
versioned serialization schema, and the rewritten agent layout tools. Not yet:
pointer tab drag-docking (deliberate), OS windows / native chrome (phase 2 —
capability contract only), region view cycling on repeat-press, and workspace
