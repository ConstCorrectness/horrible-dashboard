...
**Changing a pane's type:** each tab carries a **▾ switcher** (a custom
`defaultTabComponent`, `packages/ui/src/PaneTab.tsx`, wrapping dockview's
`DockviewDefaultTab`) listing every registry pane. Picking one calls
`LayoutController.changePaneType(instanceId, paneId)`, which swaps the content
**in place** — same instance id, so geometry and autosave are untouched. `PanelHost`
re-renders the new type by tracking `panelId` from `api.onDidParametersChange`.
The menu is portaled to `document.body` and positioned from the tab's viewport
rect; since the pane catalog grows long, its `max-height` is capped to the space
available below the tab (and it **flips above** the tab when there's more room
there) so the full list always fits on-screen and scrolls within that height. A
sticky **filter input** at the top narrows the list by title/id substring (the
command-palette match) and is keyboard-drivable — it auto-focuses on open, ↑/↓
move the highlight, Enter picks it, Escape closes.
...
**▾ pane-type switcher** (`changePaneType`). Built-in presets: **Dashboard** and
**Scripting**. `scratch.note` is the reference non-singleton panel. Not yet: a
pane-type picker for the split target (it duplicates the current pane today),
