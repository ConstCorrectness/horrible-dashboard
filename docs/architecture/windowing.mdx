...
- **Pane instance** — an active container slot running a panel _or_ widget view.
...
**Changing a pane's view:** each tab carries a **▾ switcher** (a custom `defaultTabComponent`, [PaneTab.tsx](file:///home/horrible/horrible-dashboard/packages/ui/src/PaneTab.tsx), wrapping dockview's `DockviewDefaultTab`) listing every registry view. Selecting one calls `LayoutController.changePaneType(instanceId, viewId)`, which swaps the view content **in place** — keeping the same pane instance ID, geometry, and autosave state. `PanelHost` re-renders the new view by tracking the `panelId` (view ID) reactive parameter updates from `api.onDidParametersChange`.
