...
- **Tab gestures**: a tab is `draggable`, and dropping one on another tab in any
  ...
  to the Right / Close All, then Split Right / Split Down / Float).
...
- **Drag-out** (`layout/drag.ts` + the gestures in packages/ui): drag a **rail
  ...
  Tabs now drag between center areas as well (see **Tab gestures** above). What
  is still missing is dropping onto an area *edge* to create a split in one
  gesture — `move_pane`/`join_area` can't create a new split either, so an agent
  that opened a pane in the wrong place still has no way to fix it.
