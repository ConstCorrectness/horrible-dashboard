...
- **Named workspaces.** A workspace is a named, savable dockview layout; several
  can exist and you switch between them via a tab strip. The **Dashboard** is
  just the default seeded workspace (a layout of common widget-panes), not a
  special container.
...
1. A rail button or command calls `registry.openPanel(id)` (works for panel _or_
   widget ids). Every widget also gets a synthesized `widget.open:<id>` command,
   so any widget is openable from the palette ("Open widget: &lt;title&gt;"). A visual
   "Add widget…" picker in the tab strip does the same.
...
## Named workspaces & switching

`Workspace` loads the collection (`getWorkspaces()`), renders a **tab strip**
above the dock (one tab per workspace, a "＋" to create, "×" to delete), and
tracks the active id. Switching a tab flushes the current layout
(`saveWorkspace`), then `api.fromJSON(target.layout)` (or seeds, for the
Dashboard, or clears, for a fresh one) — guarded by `swappingRef` so the
programmatic swap doesn't trip autosave. Commands switch tabs through the
`registry.setWorkspaceSwitcher` seam (AppShell enters the workspace view, then
signals via a `pendingWorkspace` nonce — mirroring `pendingOpen`);
`dashboard.open` is just `switchWorkspace('dashboard')`.

On first run (no workspaces) the frontend **seeds** a "Dashboard" workspace —
opens the preset widgets (`dashboard.welcome`, `dashboard.backendStatus`,
`observability.io`) as panes in a 2-column arrangement — and saves it under the
stable id `dashboard`.

## Agent-driven layout

The agent orchestrator can arrange the workspace too (open/close panes, create
and switch workspaces). The mutations it needs beyond the existing
`openPanel`/`switchWorkspace` seams go through `registry.layoutController`
(`setLayoutController`), which `Workspace` installs over its dockview api. So the
agent stays decoupled from the engine, same as every module. See
[../modules/agent-chat.md](../modules/agent-chat.mdx).
...
default Dashboard arrangement is the frontend's job (engine-shaped).
...
Implemented: widgets-as-panes, named workspaces with a tab switcher, splits, tab
groups, floating windows, per-workspace layout persistence/restore, the
singleton/instance distinction, and the seeded default Dashboard. `scratch.note`
is the reference non-singleton panel. Not yet: real OS windows on desktop,
per-instance pane state (scratch instances share one store), multiple instances
of the same widget in one workspace, and workspace reorder/import-export.
