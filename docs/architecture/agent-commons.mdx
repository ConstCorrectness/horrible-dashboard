...
**Status: design, with Phases 1–4 (partial) landed.** Implemented: the standalone index
server (signed profiles + vectordb matchmaking — `backend/modules/network/commons_server.py`,
...
accept/decline → `connected` → peer dial), **and the reputation floor** — a local
blocklist (auto-declines a blocked node and refuses it at the peer fabric too) and
per-profile trust tiers (`known` / `unknown` / `blocked`); see
[Module: commons](../modules/commons.mdx). Still design: vouches/reports, a profile
editor, a dedicated requests widget, and DHT/PEX. It builds entirely on shipped
primitives — the
...
## 2. Matchmaking (the vectordb seam)

Keyword filtering on `tags` is the floor. The real win reuses the
[vectordb module](../modules/vectordb.mdx): the commons server embeds each profile
...
sequenceDiagram
    ...
    participant V as vectordb (cosine)
...
clean seam — the commons server owns one vectordb collection of profiles; nothing
...
- **Vouches — design.** A node you already trust publishes a **signed attestation** for
  another (`vouch {subject_node_id, sig}`), weighted by *your* trust graph (not a global
  count, for Sybil resistance), adding a `vouched` tier between `unknown` and `known`.
- **Reports — design.** Abusive nodes reported to the index for de-listing; the local
...
- **Semantic search** (the vectordb matchmaking) — DHTs do exact-key lookup, not ranked
  similarity.
...
3. **Treat the commons index as an A2A curated registry** — same vocabulary, plus our
   consent gate and vectordb ranking.
...
**Scope of v1:** a hosted, federatable index (profiles + vectordb matchmaking) + the
...
`$HORRIBLE_DATA_DIR/commons-profiles.json` (embeddings live in the vectordb SQLite
...
- **Match index → vectordb, reused directly.** The server process imports
  `backend.modules.vectordb.database` and `embeddings`:
...
1. **Server profiles + search** (`commons_server.py` + vectordb index) — testable via
...
4. **Reputation floor** (block/report, trust-tier display) + profile editor + requests UI.
   **◐ partial** — blocklist + trust-tier display (`known`/`unknown`/`blocked`) shipped
   (`commons` channel `block`/`unblock`, node-side `trust`-backed; directory badges +
   Block/Unblock; verified live across two nodes). Remaining: vouches/reports, a profile
   editor, and a dedicated requests widget.
5. **v2 (separate effort):** DHT/PEX, signed vouches, federation between indexes.
...
- Matchmaking engine: [Vector DB](../modules/vectordb.mdx).
