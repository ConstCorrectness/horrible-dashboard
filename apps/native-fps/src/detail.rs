...
///
/// Mipmapped would be better and is deliberately not done yet: the UVs are in
/// cube units, so a floor seen down a corridor samples hundreds of tiles per
/// pixel row, and without mips it can boil into moire while walking. The browser
/// gets mips and anisotropy for free from three; here they would have to be
/// generated by hand, which is a blit chain this change does not need in order
/// to stop the world reading as coloured cardboard.
pub fn bind_group(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
