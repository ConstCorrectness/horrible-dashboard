...
import { installShims, installTextureLoader, resolveFlips } from './lib/three-headless.mjs';
...
// Headless shims
//
// Shared with `build_hassault_weapon.mjs` — see `scripts/lib/three-headless.mjs`
// for what they are and why nothing here rasterises. They live there rather than
// here because two of them are subtle enough that a second copy would drift into
// being wrong without ever failing: the `FileReader` shim must not resolve
// synchronously, and `resolveFlips` has to bake every flip out before the
// exporter runs.
// ---------------------------------------------------------------------------
