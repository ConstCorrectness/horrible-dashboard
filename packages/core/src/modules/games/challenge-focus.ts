 ...
 * buffered (the panel may not be mounted yet — regions mount lazily)
 * and also broadcast live for an already-open panel. See docs/architecture/panel-groups.mdx.
 */
...
/** Open the Challenges region focused on `gameId`'s scenario set. */
export function requestChallenges(gameId: string): void {
  pending = gameId;
  revealRegionView('games.challenges');
