    ...
    async with httpx.AsyncClient(timeout=60) as client:
        try:
            async with client.stream("POST", url, json=payload) as res:
                ...
                print(f"[{name}] Has thinking: {has_thinking}, Has content: {has_content}, First content: {repr(first_content)}")
                return has_thinking
...
async def main():
    ...
    print(f"Found original payload. Model: {payload.get('model')}, Number of tools: {len(original_tools)}")
    ...
    await test_payload(f"10 dot tools", test_p)
